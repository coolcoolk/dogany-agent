...
제품은 하나의 포함관계로 짜여 있습니다 — 작은 단위가 큰 단위 안에 중첩됩니다.
**스킬**은 **모듈** 안에, 모듈은 **팩** 안에, 팩은 **킷** 안에 듭니다. 가장
작은 단위부터 바깥으로 읽어 나가면:

<p align="center"><img src="docs/img/structure-tree-ko.png" width="700" alt="트리와 정의 표로 본 구조. 스킬(가장 작은 단위 — 한 가지 동작을 수행, 예: 리마인더 스킬은 밤 9시에 약 먹으라고 한 번 알림)이 모듈(스킬 + 데이터로 한 능력을 완성해 킷 데이터계약에 기록, 예: 식단기록은 '닭가슴살 200g' 입력을 받아 칼로리를 계산해 라이프킷에 저장) 안에, 모듈이 팩(모듈 + 지식 + 화법 규율을 묶어 스태프 에이전트를 빚음, 예: 헬스트레이너 팩은 루틴 짜고 챙기는 운동 코치 스태프 에이전트를 만듦) 안에, 팩이 킷(한 도메인 전체의 데이터계약 — 그 도메인 모듈·팩이 같은 스키마 공유, 예: 라이프킷은 일상 전체 식사·운동·일정의 데이터계약) 안에 든다. 스킬 ⊂ 모듈 ⊂ 팩 ⊂ 킷. 중첩 밖: 코어 모듈은 킷을 건너뛰어 메인 에이전트에 직접 붙고, 팩을 설치하면 메인 아래에 스태프 에이전트가 생기며, 메인 에이전트가 킷을 장착한다"></p>

- **스킬(skill)** — 가장 작은 단위입니다. 한 가지 동작을 수행하며, 하나의
  `SKILL.md`가 담습니다. 예를 들어 리마인더 스킬은 "밤 9시에 약 먹으라고
  알림 한 번"을 보냅니다. 더 큰 단위들은 모두 이 스킬을 원자로 조립됩니다.
- **모듈(module)** — 스킬과 그 데이터로 한 능력을 완성해 킷 데이터계약에
  기록하는 단위입니다. 스킬은 모듈 안에 듭니다. 예를 들어 식단기록(diet-log)
  모듈은 "닭가슴살 200g" 입력을 받아 칼로리를 계산하고 라이프킷에 저장합니다.
  모듈은 킷 종속(특정 킷에 씀, 예: 식단기록)이거나 코어 모듈(킷 무관 — 아래
  예외 참조)입니다.
- **팩(pack)** — 모듈 + 지식 + 화법 규율(rubric)을 묶어 스태프(부하)
  에이전트를 빚는 번들입니다. 모듈은 팩 안에 듭니다. 팩에는 페르소나가
  없습니다 — 말투·톤은 나중에 온보딩에서 정해집니다. 예를 들어 헬스트레이너
  팩은 루틴을 짜고 챙기는 운동 코치 스태프 에이전트를 만듭니다.
- **킷(kit)** — 가장 큰 단위입니다. 한 도메인 전체의 데이터계약으로, 그
  도메인의 모듈과 팩이 같은 스키마를 공유합니다. 팩과 모듈은 킷 안에 듭니다.
  어느 킷을 장착하느냐가 에이전트의 도메인을 정의합니다. 예를 들어
  **라이프킷(LifeKit)**은 일상 전체(식사·운동·일정)의 데이터계약입니다.
  (개발/PM용, 사업용 등 다른 도메인 킷은 예정입니다.)

중첩 밖에 놓이는 것이 둘 있습니다. **코어 모듈**은 킷을 통째로 건너뛰어 메인
에이전트에 직접 붙습니다(예: 리마인더·기억검색처럼 아무 에이전트나 쓰는 능력).
그리고 팩을 설치하면 메인 아래에 **스태프 에이전트**가 생깁니다. **메인
에이전트** 자신 — 정체성과 기억을 갖는 단 하나뿐인 본체, 페르소나는 온보딩 때
인스턴스별로 결정 — 이 바로 킷을 장착하는 주체입니다.
...
- 핫 인젝트: `USER.md`와 `PROFILE.md`는 매 턴 컨텍스트에 로드됩니다
...
오래 남을 지식은 네 갈래 중 하나로 분류됩니다: 에이전트 정체성(`PROFILE.md`),
당신의 프로필(`USER.md`), 재사용 절차(`SKILL.md` — 도메인 기록은 서비스
레이어를 거쳐 `database.sql`에 쌓입니다 — 킷 데이터계약 레인, 예: LifeKit),
그리고 나머지
전부는 `memories/`로. 앞의 셋은 에이전트가 수동으로 라우팅하고, 마지막
갈래는 AUTO — 엔진이 알아서 관리합니다.

<p align="center"><img src="docs/img/routing-ko.png" width="560" alt="기억 라우팅: 사용자 대화가 PROFILE.md·USER.md·SKILL.md(service→database.sql, 킷 데이터계약 레인, 예: LifeKit)·memories/ 토픽 파일로 분류됩니다. 수동은 에이전트가, AUTO 갈래는 엔진이 처리하며 다음 도식에서 확대됩니다"></p>
...
- **`agents/main/`** — 레포 내부 scaffold 민트 대상(dev 모드/폴백; 엔드유저
  기본 설치 경로는 `~/.dogany/main`)으로 레포 콘텐츠가 아닙니다. 새 클론에는
...
- **`skills/`** — 에이전트 간 공유 프레임워크 스킬(`dogany-cron-register`,
  ...
  복사본을 갖습니다. 라이프킷 도메인 스킬은 이제 이 레포에 없습니다 —
  라이프킷은 독립 팩(별도 레포 `dogany-lifekit`)이며, 민트 후
  `scripts/pack/pack_install.sh`로 설치되면 `.claude/skills-bundle/` 아래에
  DORMANT 상태로 배치되고 `dogany-lifekit-setup` 스킬로만 활성화됩니다.
- **`database/`** — 인스턴스 정형 데이터 마운트 지점. 라이프킷 정형 레인
  (`lifekit.py`/`lifekit.sh`/`schema.sql`: 로컬 SQLite "생활 OS" — 식사,
  운동, 사람, 일정)은 라이프킷 팩으로 이동했고 킷 활성화 시점에 여기로
  설치됩니다. 이 레포에는 엔진측 복원 도구(`restore-data.sh`)만 남습니다.
  `*.db` 데이터는 미포함.
- **`service/`** — 스킬이 raw 데이터 레이어 대신 임포트하는 안정적 SDK
  표면. `service.lifekit` 파사드는 라이프킷 팩으로 이동했습니다.
- **`scripts/`** — `agents/.template`과 공유 루트에서 독립 에이전트를 생성하고
  인스턴스 `.env`를 작성하는 `mint.sh` (단일 생성기; 봇 토큰은 argv가 아니라
  `DOGANY_BOT_TOKEN` 환경변수로 전달). 이메일은 앱 비밀번호가 아니라 구글
  온보딩(원클릭 OAuth 로그인)으로 연결됩니다.
...
- 민트된 인스턴스는 그 자체가 `PROJECT_ROOT`입니다. rules, 프레임워크 스킬,
  서비스 표면의 실제 복사본(심링크 아님)을 갖습니다.
- 브릿지는 환경에서 `PROJECT_ROOT`를 읽습니다(`start.sh`가 launchd plist로
  설정). plist와 훅은 민트 시 `__PROJECT_ROOT__` 플레이스홀더를 치환합니다.
- `pack_install.sh`로 설치되는 킷 콘텐츠(예: 라이프킷 팩)도 동일한 인스턴스
  레이아웃을 유지합니다. 팩 코드는 부모 트리가 아니라 자기 설치 위치 기준으로
  DB와 동료 파일을 해석합니다.
...
2. 마법사가 언어·타임존·봇 토큰(BotFather에서 발급)·소유자 확인을
   ...
   `~/.dogany/main`, 홈 폴더의 히든 디렉터리). 자동시작 서비스 설치와
...
터미널에서 `dogany`를 실행하면 같은 에이전트를 만납니다 -- 같은
정체성, 같은 기억, 같은 훅과 스킬:
...
직접 폴더를 열어 대화하는 방법도 있습니다:

    cd ~/.dogany/main && claude

노트북 팁: 에이전트는 기기가 깨어 있는 동안만 동작합니다. 전원을 연결하고
시스템 잠자기를 꺼두세요. macOS: 시스템 설정 > 배터리 > 옵션 > "디스플레이가
꺼져 있을 때 전원 어댑터 연결 시 자동으로 잠자기 방지"를 켜세요(macOS 버전에
따라 메뉴 이름이 다를 수 있습니다). 또는 터미널에서 `sudo pmset -c sleep 0`
을 실행하세요. 노트북 덮개를 닫으면 외부 모니터를 연결한 클램셸 모드가
아닌 이상 기기가 잠자기에 들어가므로, 덮개를 닫지 마세요. 화면 잠자기(화면만
꺼짐)는 무관합니다.

## 업데이트 방법

에이전트 프레임워크는 주기적으로 업데이트됩니다. 업데이트 방법:

- 텔레그램에서 에이전트에게 요청: "업데이트해줘"
- 또는 로컬에서 실행: `dogany update`

최신 릴리즈 버전으로 프레임워크 스킬이 갱신됩니다. 기억, 정체성, 개인
데이터는 변경되지 않습니다.
...
- 선택적 `lifekit` 레인은 독립 팩(레포 `dogany-lifekit`)입니다. 정형 레인
  (`lifekit.db`)은 킷 활성화 시(`pack_install.sh`) 팩의 `schema.sql`로
  초기화됩니다(빈 정형 레인, 사용자 데이터 없음). 킷 없이 민트하면
  라이프킷 파일이 전혀 설치되지 않습니다.
