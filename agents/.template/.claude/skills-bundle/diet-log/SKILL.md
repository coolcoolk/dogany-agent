...
## Aggregate finality contract (DGN-669)

- `$PROJECT_ROOT/database/lifekit.sh agg-day <date>` appends `as_of=` / `last_entry_at=` / `closed=` KEY=VALUE lines (`agg-week`: `as_of=` + per-day `[closed]`/`[prov]` suffix). `closed=0` = provisional: the day is not closed and numbers may still change (late backfill).
- Day-close rule (derived at read time, never stored): closed(D) = now >= max(last_entry(D) + 6h quiet, D+1 12:00 local floor). A late backfill insert re-opens the day automatically.
- Marker contract: any surface presenting a PAST date's aggregate MUST attach `(잠정, HH:MM 기준)` (HH:MM from `as_of`) when `closed=0`; `closed=1` may render bare or as 확정. Current-day surfaces (diet card, today retro) are "so far" views: exempt from the marker, but must never claim finality.
- `closed=1` with `meal_cnt=0` means NO RECORDS for that day, not a confirmed fast -- never voice it as a confirmed 0 kcal / fasting day.
- Known limitation: deletes and date-move-out edits on an already-closed day are undetected (day stays closed with changed numbers) -- tombstone detection is a separate ticket.

<!-- DGN-669 consumer-rule: Skull-supplied (TBD, design-split 1-a). The behavioral
     consumer rule (live-read at speech time; never re-cite aggregate numbers from
     old messages; provisional vs 확정 phrasing) lands here VERBATIM from Skull's
     delivery. Do not invent or paraphrase the wording before it arrives. -->
