...
- Branch discipline (DGN-887): NEVER run `git checkout <branch>` or any
  HEAD-moving operation in a live instance repo (the running agent
  workspace). Branch work in live instance repos MUST use worktree
  isolation (`git worktree add`) only. Any branch restore applies only
  to canonical/release repos, never to the live agent working tree.
...
# Release note authoring: ko_detail / en_detail (DGN-742)
When writing releases/vX.md, add `ko_detail` and `en_detail` keys in the
`<!-- user-summary ... -->` block alongside `ko` and `en`. These feed the
second expandable fold ("Detailed changes") in the update-completion notice.

Rules:
- Derive content from the Changes section of the same release note.
- De-jargonize using the display-name map (sot/ESTATE-TAXONOMY.md):
  use surface names the user meets, not internal component/process labels.
- Strip ALL DGN ticket numbers (no "DGN-NNN" refs in detail text).
- `ko_detail` is the primary; `en_detail` mirrors it in English.
- Best-effort: hotfix/patch releases MAY omit both keys -- the completion
  notice falls back gracefully to summary-only (no second fold rendered).
- Detail should be longer and more explanatory than the `ko`/`en` summary,
  but still jargon-free and readable by a non-developer user.

Format (inside the existing user-summary HTML comment block):
  <!-- user-summary
  ko: |
    <short summary>
  en: |
    <short summary>
  ko_detail: |
    <longer jargon-free Korean explanation of what changed and why>
  en_detail: |
    <longer jargon-free English explanation of what changed and why>
  -->
...
6. Canonical repo HEAD check: for every canonical repo touched during this
   release, run `git symbolic-ref -q HEAD`. If the command exits nonzero
   or returns nothing, the repo is in detached HEAD state -- check out
   main (or the repo's documented default branch) before finishing. Note
   any restore in the close report. A release close MUST NOT leave any
   canonical repo detached.
   Branch discipline (DGN-887): NEVER run `git checkout <branch>` or any
   HEAD-moving operation in a live instance repo. Branch work in live
   instance repos MUST use worktree isolation (`git worktree add`) only.
7. Rider open-ticket scan (DGN-867 Gate B, B1 -- READ-ONLY):
   Scan the DGN worklog tickets listed as riders for this release.
   For each rider whose worklog status is NOT done/resolved/closed,
   record it in the close report as a "shipped-but-open" candidate.
   DO NOT edit any ticket status -- that call belongs to the main
   session (one-writer rule; Done=earned evidence gate). Output goes in
   the report only; no writes outside the existing 3-place edit scope.
8. Report: files edited, item mapping (ticket -> changelog line),
   mismatches flagged, preflight report path + verdict summary, HEAD
   check result for each canonical repo, shipped-but-open rider
   candidates (from step 7), and the exact release.sh command the
   dispatcher still needs user approval to run.
