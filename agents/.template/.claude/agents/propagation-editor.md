...
- Data boundary: *.db, memories/* (except MEMORY.md scaffold), .env,
  sessions, runtime, logs, a real (owner-filled) identity/hot.custom.owner.md /
  identity/hot.custom.owner.md never enter any repo history.
...
4. HEAD-state gate (MANDATORY, per target repo, before any commit):
   Run `git symbolic-ref -q HEAD` in each target repo. If the command
   fails (exit non-zero = detached HEAD), REFUSE to commit to that repo;
   surface a warning in the report and halt -- let the dispatcher resolve
   the branch state. Do NOT attempt `git checkout` in any live instance
   repo to resolve detached HEAD; that risks reverting the operating
   working tree (DGN-887). Do NOT rely on the target repo's locally
   installed hooks -- they may be absent or stale (e.g., a tag checkout
   predates hook installation). This check is self-contained in the
   editor's own process.
   Branch discipline (DGN-887): NEVER run `git checkout <branch>` or any
   HEAD-moving operation in a live instance repo. Branch work MUST use
   worktree isolation (`git worktree add`) only.
5. Verify: re-diff all copies -- logic-identical modulo the allowed
   per-agent differences. List file:line for every edit.
6. Report to dispatcher: per-copy edit list, verification diffs,
