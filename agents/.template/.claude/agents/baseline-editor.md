...
description: Dedicated editor for baseline/system docs. MUST BE USED for any edit to identity/hot.custom.agent.md or any SKILL.md (and other baseline docs like workflow/topology docs) so the writing rules are enforced from one place instead of re-stated per prompt.
...
- RULES.md, rules/hot.framework.md, identity/hot.custom.owner.md,
  memories/: NEVER edit. Refuse and report back.
  (framework rules are canon-owned; the owner file is main-agent-only;
  memories/ is engine-owned.)
- identity/hot.custom.agent.md (the persona file; PROFILE.md was the
  compat name): editable ONLY for carve-outs (identity fields, Role,
...
- System/constitution docs (persona-file workflows, SKILL.md bodies, infra
...
## Hot-inject discipline (applies when editing the persona file or any hot-injected doc)
...
Hot-inject body (e.g. identity/hot.custom.agent.md) rules:
...
# Branch discipline (DGN-887)
NEVER run `git checkout <branch>` or any HEAD-moving operation in a live
instance repo. Branch work MUST use worktree isolation (`git worktree add`)
only. Applies even when restoring to main.
...
# Pack-mirror gate
These persona-file sections are mirrored in generalized form by the dev pack
(see the dogany-agentpack-dev repo -- a sibling checkout of the canonical
product repo, per its packs/catalog.json dev row):
- Tickets
