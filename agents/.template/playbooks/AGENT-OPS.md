...
The framework refresh channel that installs this file (upstream section 3k2)
mechanically asserts the contract on every refresh; the asserting code lives
upstream, not in this instance. -->

# AGENT-OPS -- framework operations reference

Cold reference for framework procedures. Read this when you need the full
mechanics; the hot pointer in identity/hot.custom.agent.md carries only the entry points and the
...
`--trigger user|auto` (WHO asked: `user` = explicit owner restart command -- idle guard
bypassed, immediate, no menu; `auto` = autonomous/self restart after a fix or
self-update -- idle guard applies, and on refusal the restart silently defers to the
next natural restart, never surfacing a menu. Default `auto`),
`--force` (bypass alias for the idle guard).
...
Restart is baked into self-update.sh -- no separate restart step or menu is needed
after update. On success the script triggers restart automatically:
- Interactive path (CLAUDECODE env set): --trigger user -- idle guard bypassed,
  immediate restart.
- Autonomous/cron path (CLAUDECODE absent): --trigger auto -- idle guard applies;
  silently deferred to next natural restart if guard refuses.

Agent rule: place the self-update.sh call as the last tool call in the turn and
close the turn in one line directly after (mitigates mid-turn session cut).
...
- baseline-editor -- any edit to identity/hot.custom.agent.md or a SKILL.md (and other baseline docs).
...
Heavy dispatches (Workflow / Agent / Task / deep-research) are gated MECHANICALLY by
the PreToolUse hook `routines/usage-gate.py` against plan-specific 5h/7d utilization
thresholds (plan slug in `config/agent.conf` PLAN=). Denies above threshold; fully
silent below; non-blocking 7d burn-rate hint when pace is high. Owner approval
bypasses the gate one-shot (the single next heavy call). The gate is machinery,
not judgment: do not reflexively defer heavy work "to save the window" without an
actual gate denial -- dispatch first, let the gate decide.
