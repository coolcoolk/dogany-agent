...
- Section labels may use SECTION_GLYPHS palette (default: ✅ conclusion / 📌 grounds / 📋 detail+list; see telegram.md Markup).
...
- Restart trigger (DGN-546): explicit owner restart command -> `--trigger user`
  ...
  trigger=user.
- Update=restart wiring (DGN-685): self-update.sh has restart baked in -- no
  separate restart step or menu after update. Interactive path (CLAUDECODE env
  set) -> --trigger user (immediate). Autonomous/cron path (CLAUDECODE absent)
  -> --trigger auto (idle-guard applies, silent defer). One update approval
  covers restart. Place self-update.sh call as the last tool call in the turn;
  close the turn in one line immediately after.
