...
# Unknown identities still hard-block in every mode.
...
for a in "$@"; do
  case "$a" in
    ...
    --outbound-diff)
      ...
      ;;
    -h|--help)
      sed -n '2,63p' "$0"; exit 0 ;;  # header doc lines 2-63
    -*)
...
if [ "$MODE" = "outbound-diff" ]; then
  ...
  # Collect added lines only from git diff base..HEAD
  ADDED_LINES="$(git -C "$REPO_ROOT" diff "${OUTBOUND_BASE}..HEAD" -- 2>/dev/null \
    | grep '^+' | grep -v '^+++' )"
  ...
  while IFS= read -r dline; do
    ...
    case "$dline" in
      ...
      '+'*)
        ...
        content="${dline#+}"
        if printf '%s\n' "$content" | grep -qE "$OUTBOUND_ERE"; then
