...
- Tier 2: option / pros-cons comparison -> blockquote list, NOT image.
  ...
  "rec: reason" footer. Option labels = short token only -- no separators, no description
  phrases, no recommendation markers. Recommendation marker in body child line or footer
  only (ko "(추천)" / en "(rec)"). Label overflow (>31 chars): button falls back to
  number token (ko "N번" / en "No.N"). Stack items vertically per option (never
  one-line compressed -- cognition collapses).
