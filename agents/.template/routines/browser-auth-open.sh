...
# --------------------------------------------------------------------------
# agent-browser resolution -- do NOT rely on PATH alone (2026-08-23).
# DGN-963 dropped $HOME/.npm-global/bin from PATH; agent-browser lives there
# and nowhere else, so every consumer bound to PATH went dark.  Resolve from
# the known install dirs too, and FAIL LOUD -- a silent skip here is
# indistinguishable from "auth is fine", which is the worst possible lie for
# an auth-check script to tell.
# --------------------------------------------------------------------------
AB="${BROWSER_AUTH_AGENT_BROWSER:-agent-browser}"
if ! command -v "$AB" >/dev/null 2>&1; then
  for _cand in "$HOME/.npm-global/bin/agent-browser" \
               "$HOME/.local/bin/agent-browser" \
               "/opt/homebrew/bin/agent-browser" \
               "/usr/local/bin/agent-browser"; do
    if [ -x "$_cand" ]; then AB="$_cand"; break; fi
  done
fi
if ! command -v "$AB" >/dev/null 2>&1 && [ ! -x "$AB" ]; then
  echo "$PROG: ERR agent-browser not found (PATH + known install dirs)" >&2
  echo "$PROG: cannot verify session auth -- refusing to report success" >&2
  exit 6
fi
