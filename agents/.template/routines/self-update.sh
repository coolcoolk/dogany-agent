...
# ---------------------------------------------------------------------------
# DGN-687 helpers: release-note extraction + HTML escaping for the Telegram
# parse_mode=HTML push path (tags are added AFTER escaping, so only the
# dynamic note text is escaped).
# ---------------------------------------------------------------------------
...
maybe_restart() {
  ...
  # English CHANGELOG scrape (which is model-facing per DGN-210). Fallback: when
  # a release predates the summary block (empty extraction), fall back to the
  # existing changelog_section(English) behaviour -- no old release breaks.
  # DGN-725: changelog_section() strips DGN ticket refs before returning text.
  _notes="$(user_summary "$_src_root/releases/${_vdisp}.md" "$DOGANY_LANG")"
  if [ -z "$_notes" ]; then
    _notes="$(changelog_section "$_src_root/CHANGELOG.md" "$_vplain" | head -n 30)"
  fi
  # DGN-742: Korean extended detail fold (COMPLETION notice only).
  #   Reads <lang>_detail key from the same user-summary block. If present,
  #   rendered as a SECOND expandable blockquote after the summary fold.
  #   Hard-capped at ~2500 chars; truncated output gets a "...see more" line.
  #   If detail is empty, notice is byte-identical to pre-742 output (parity).
  _detail="$(user_summary "$_src_root/releases/${_vdisp}.md" "${DOGANY_LANG}_detail")"
  if [ -n "$_detail" ]; then
...
…(이하 생략 — 더 궁금하시면 물어봐 주세요)"
  ...
  if [ "$DOGANY_LANG" = "ko" ]; then
    ...
    _fold="이번 업데이트 요약"
    _detail_label="자세한 변경 내용"
  else
    _notice="Restart complete · ${_vdisp} update complete"
    _fold="This update at a glance"
    _detail_label="Detailed changes"
  fi
  # DGN-766: summary + detail render as ONE expandable blockquote, not two
  # separate quote bubbles (mirrors DGN-718 single-block principle, applied to
  # the COMPLETION notice). Summary section first, detail section after a blank
  # line. Either may be absent -> fail-open to whichever is present.
  if [ -n "$_notes" ] || [ -n "$_detail" ]; then
    _body=""
    if [ -n "$_notes" ]; then
      _body="${_fold}
$(printf '%s\n' "$_notes" | html_esc)"
