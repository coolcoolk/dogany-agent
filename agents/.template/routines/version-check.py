...
def _build_user_notice(remote_version, notes, lang):
    ...
    PLAIN lines OUTSIDE the fold; ONLY the fold label + release-note summary
    sit inside one <blockquote expandable> so the notes collapse while the
    call-to-action stays plainly visible above the fold:
    ...
    With no notes the fold is dropped entirely -- header + action, 2 plain
    lines, no blockquote.
