...
def _build_user_notice(remote_version, notes, lang):
    ...
    (owner-approved 2026-08-01); en mirrors the structure. The <blockquote>
    tags are Telegram HTML the bridge renders; the expandable fold carries
    the release-note summary and is present only when notes were found."""
    ver = "v" + remote_version.lstrip("v")
    if lang == "ko":
        header = "<blockquote>도가니 업데이트 있어요 · {}</blockquote>".format(ver)
        fold_label = "이번 업데이트 요약 …"
        tail = "<blockquote>지금 업데이트하실래요?</blockquote>"
    else:
        header = "<blockquote>Dogany update available · {}</blockquote>".format(ver)
        fold_label = "This update at a glance ..."
        tail = "<blockquote>Update now?</blockquote>"
    parts = [header]
    if notes:
        parts.append(
            "<blockquote expandable>{}\n{}</blockquote>".format(fold_label, notes))
    parts.append(tail)
    return "\n".join(parts)
