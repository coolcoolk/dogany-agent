...
def _build_user_notice(remote_version, notes, lang):
    ...
    DGN-718). en mirrors the structure. The ENTIRE notice is ONE Telegram
    <blockquote expandable> so it renders as a single collapsible bubble --
    NOT three separate quote blocks (DGN-718 regression: later notice work,
    DGN-738/742, rebuilt on the old 3-block shape and dropped the merge).
    Collapsed preview = 3 lines (header / action / fold label); the
    release-note summary sits below the fold. With no notes, downgrade to a
    plain 2-line blockquote (header + action), no fold."""
    ...
    if notes:
        return "<blockquote expandable>{}\n{}\n{}\n{}</blockquote>".format(
            header, action, fold_label, notes)
    return "<blockquote>{}\n{}</blockquote>".format(header, action)
...
def main():
    ...
    if repo_root:
        ...
        if repo_version and repo_version != "unknown" and _is_newer(repo_version, built_version):
            ...
            # fall back to CHANGELOG scrape when summary is absent (fail-open).
            ...
            try:
                ...
                notes = ""
            if not notes:
                notes = _release_notes_local(repo_root, repo_version)
    ...
    if (remote_version
        ...
        # fall back to CHANGELOG scrape when summary is absent or fetch fails.
        notes = ""
        try:
            ...
            notes = ""
        if not notes:
            notes = _release_notes_remote(remote_version)
