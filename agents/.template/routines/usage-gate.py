...
# routines/lib/conf_reader.py only.  Guarded import keeps the hook
...
# Baseline write-rules FORCE these subagents (persona file "Baseline doc
...
def read_plan(cwd):
    ...
    Parsing via the single conf parser (routines/lib/conf_reader.py,
    DGN-916).  First candidate with a non-empty PLAN value wins; empty
    value / missing key / missing file falls through to the next
    candidate, then to DEFAULT_PLAN.
    """
    candidates = [
    ...
    for path in candidates:
        slug = conf_get("PLAN", path)
        if slug:
            return slug if slug in PLAN_THRESHOLDS else DEFAULT_PLAN
    return DEFAULT_PLAN


# --- DGN-925: gate master switch + 7d owner-exempt scope ---------------------
# USAGE_GATE     master on/off. off -> whole hook is a no-op pass-through for
#                heavy dispatches too (kill switch). Unset/anything-else -> on.
# USAGE_GATE_7D  scope of the 7d (weekly) block.
#                  full (default) -> 7d blocks every session.
#                  auto-only      -> 7d blocks ONLY autonomous/cron sessions;
#                                    the owner-driven live session skips the 7d
#                                    block (5h still applies + the burn-rate
#                                    hint still fires, so the owner stays aware).
#                                    The 7d window is the days-long lockout, and
#                                    when the owner is present they ARE the
#                                    approval authority.
# Session kind: the live bridge session carries PROJECT_ROOT (set in
# bridge/sdk_bridge.py) and it reaches this hook; headless auto runners
# (autonomous-loop.sh / metal-shift.sh spawn `claude -p` under launchd with
# only PATH+HOME) do NOT. Explicit DOGANY_SESSION= override wins. Fail-safe:
# unknown session -> auto, so the 7d block is never silently skipped.
# Env (USAGE_GATE / USAGE_GATE_7D / DOGANY_SESSION) overrides agent.conf.
GATE_OFF_VALUES = {"off", "false", "0", "no", "disabled", "disable"}
GATE_7D_AUTO_ONLY_VALUES = {
    "auto-only", "auto_only", "auto", "owner-exempt", "interactive-exempt",
}


def _conf_first(cwd, key):
    """First non-empty lowercased value for `key` across the payload-cwd conf
    then the workspace conf (mirrors read_plan's candidate order), or ''."""
    for path in (
        os.path.join(cwd, "config", "agent.conf"),
        os.path.join(SCRIPT_DIR, "..", "config", "agent.conf"),
    ):
        val = conf_get(key, path)
        if val:
            return val.strip().lower()
    return ""


def gate_disabled(cwd):
    """True if the whole gate is switched off (env wins, then agent.conf)."""
    val = os.environ.get("USAGE_GATE", "").strip().lower() or _conf_first(cwd, "USAGE_GATE")
    return val in GATE_OFF_VALUES


def gate_7d_auto_only(cwd):
    """True if the 7d block is limited to autonomous sessions (owner-exempt)."""
    val = os.environ.get("USAGE_GATE_7D", "").strip().lower() or _conf_first(cwd, "USAGE_GATE_7D")
    return val in GATE_7D_AUTO_ONLY_VALUES


def session_is_interactive():
    """True if this dispatch is from the owner-driven live bridge session (vs
    an autonomous/cron headless runner). DOGANY_SESSION= override wins; else
    PROJECT_ROOT presence (bridge-only). Fail-safe: unknown -> auto (False)."""
    override = os.environ.get("DOGANY_SESSION", "").strip().lower()
    if override in ("interactive", "owner", "live"):
        return True
    if override in ("auto", "autonomous", "cron", "headless"):
        return False
    return bool(os.environ.get("PROJECT_ROOT"))
...
def main():
    ...
    if gate_disabled(cwd):
        sys.exit(0)  # DGN-925 master switch off: no-op pass-through, silent
    ...
    # DGN-925: when USAGE_GATE_7D=auto-only and this is the owner-driven live
    # session, waive the 7d block (5h still applies; the burn-rate hint below
    # still fires). Autonomous/cron sessions always keep the full 7d block.
    apply_7d = not (gate_7d_auto_only(cwd) and session_is_interactive())

    if fh >= thr_5h or (apply_7d and sd >= thr_7d):
