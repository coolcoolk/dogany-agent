...
first contact. Portfolio has no class gate (dec-035 준용): a domain instance
whose lifekit offer is class-suppressed still gets the portfolio offer.
READ-ONLY here too: dogany-portfolio-setup flips the state.
...
def main():
    ...
    if not onboarding:
        ...
        ctx = None
        try:
            # Lifekit: pending AND class gate passes (the lifekit bundle mounts
            # on main-class (front-door) agents only; domain staff agents never
            # get the offer). Activation-time gate only -- LIFEKIT already on is
            # untouched.
            ...
            # Portfolio: no class gate (dec-035 준용). Fires only when the
