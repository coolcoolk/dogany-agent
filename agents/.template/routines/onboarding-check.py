...
ONBOARDING_CTX = {
    ...
    "ko": (
        ...
        "'회원님'·'사용자' 같은 라벨은 절대 쓰지 마세요. 마지막 질문(역할)은 '제가 맡을 역할이 "
        ...
        "실제 통합 config로 판정한다(DGN-284 #3) -- 본인 config/agent.conf의 HANDOFF_PEER_AG를 "
        "조용히 확인. "
        "도메인 에이전트 -- 마이그레이션 경로(HANDOFF_PEER_AG가 설정됨 -- 메인 에이전트에서 "
        "민팅됐고 이관할 기존 기록이 있음): "
        "[[OPTIONS]] 메뉴 없음. 대신: "
        ...
        "제가 먼저 첫 상담을 제안드릴게요' "
        "폴백 (HANDOFF_PEER_AG는 있으나 handoff_cli.py 없음 -- 패키지 부분 적용): "
        "단계 (a) 건너뜀, 대신 안내: "
        "'아그(메인 에이전트) 방으로 돌아가면 기존 기록 이관이 이어집니다.' "
        "(메인 에이전트 이름을 아는 경우 사용, 모르면 '메인 에이전트') "
        "도메인 에이전트 -- 신규 직접 민팅 경로(HANDOFF_PEER_AG 없음 -- 독립 인스턴스, 이관할 "
