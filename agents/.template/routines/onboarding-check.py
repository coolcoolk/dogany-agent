...
ONBOARDING_CTX = {
    "en": (
        ...
        "instead. (Only Q6/role is exempt from re-asking when the Role slot was pre-filled "
        "at mint with a real domain role.) "
        ...
        "config/agent.conf for HANDOFF_PEER_AG and derive own slug from the workspace "
        "directory name. If HANDOFF_PEER_AG is set AND "
    ...
    "ko": (
        ...
        "해당 질문을 그대로 물어서 사용자에게 확인받으세요. (Q6/역할만 예외: Role 슬롯이 민팅 시 "
        "실제 도메인 역할로 미리 채워진 경우에 한해 역할 질문을 생략 가능.) "
        ...
        "[[OPTIONS]] 메뉴 없음. 대신: "
        "(a) 마이그레이션 요청 시도 (결정론적, 모델 없음): 본인 config/agent.conf에서 "
        "HANDOFF_PEER_AG를 읽고 워크스페이스 디렉토리 이름으로 슬러그를 도출. "
        "HANDOFF_PEER_AG가 설정돼 있고 routines/lib/handoff/handoff_cli.py가 있으면 실행: "
        "  python3 <own-root>/routines/lib/handoff/handoff_cli.py submit "
        "  --to-root <HANDOFF_PEER_AG> --from <own-slug> --to ag "
        "  --type migration.request "
        "  --payload-json '{\"domain\":\"<role domain>\",\"target_root\":\"<own root>\"}' "
        ...
        "제가 먼저 첫 상담을 제안드릴게요' "
        "폴백 (handoff_cli.py 또는 HANDOFF_PEER_AG 없음 -- 패키지 미적용): "
        "단계 (a) 건너뜀, 대신 안내: "
