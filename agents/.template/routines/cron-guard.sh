...
' "$USAGE_DEFER_MARKER" 2>/dev/null)"
    ...
    # Job display phrase (owner-approved copy 2026-08-12): "밤새 기억 정리" is
    # CONSOLIDATE-ONLY wording. DEFER_WHAT = subject form (…가/이), DEFER_WHAT_OBJ
    # = object form (…를/을), so both approved sentences read naturally. Any
    # other usage-deferring job falls back to its friendly name; registering a
    # new job here means supplying its own owner-approved phrases (UX gate).
    case "$LABEL" in
      *.consolidate-0430)
        DEFER_WHAT="밤새 기억 정리가"
        DEFER_WHAT_OBJ="밤새 기억 정리를"
        ;;
      *)
    ...
    if [[ -n "$DEFER_LAST_SUCCESS_EPOCH" ]] \
      ...
      if defer_dedup_hit stale; then
        ...
        # Owner-approved copy (형님 2026-08-12). DEFER_WHAT = "밤새 기억 정리가"
        # for the consolidate job (the only stale-eligible job today); other
        # jobs fall back to their friendly-name phrase (see DEFER_WHAT above).
    ...
    if [[ "$DEFER_WINDOW" == "seven_day" ]]; then
      ...
      # Owner-approved copy (형님 2026-08-12 13:43). "밤새 기억 정리" wording
      # is consolidate-only -- see DEFER_WHAT above. Button label: "지금 실행".
