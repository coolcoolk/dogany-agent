...
if [[ "$RETRO_HEALTH_SOURCE" == "warg" ]]; then
  ...
  if [[ -n "$WARG_LIFEKIT_SH" && ( -x "$WARG_LIFEKIT_SH" || -f "$WARG_LIFEKIT_SH" ) ]]; then
    ...
    # DGN-669: TODAY read = "so far" semantics, exempt from the provisional
    # marker contract; appended finality keys are ignored by the ^key= parsing.
...
" "$_created_utc" 2>/dev/null || true)"
  ...
  # DGN-669: reads TODAY -> inherently "so far" semantics; exempt from the
  # provisional marker contract (must not claim finality either). The appended
  # agg-day finality keys (as_of/last_entry_at/closed) are ignored here.
