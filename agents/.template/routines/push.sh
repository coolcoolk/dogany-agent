...
HTML_MODE=""  # --html: send with parse_mode=HTML; supported tags: <b> <i> <u> <s> <code> <pre> <blockquote> <blockquote expandable> <a href>
...
# NOTE (DGN-688): In --html mode, trimming at a fixed byte offset may split a
# tag mid-token. The 4000-char budget leaves 96 chars of headroom vs the 4096
# hard limit, which is usually sufficient. Callers are responsible for keeping
# HTML bodies short; deeply-nested tags that fall on the cut boundary may
# produce a Telegram 400 which triggers the plain-text fallback below.
BODY="${BODY:0:4000}"
...
if [[ -n "$HTML_MODE" ]]; then
  # DGN-688: --html mode -- attach parse_mode=HTML; fall back to plain on HTTP 400.
  HTTP_CODE="$(_send_message --data-urlencode "parse_mode=HTML")"
  if [[ "$HTTP_CODE" == "400" ]]; then
    echo "[push] HTML send failed (HTTP 400); retrying as plain text" >&2
