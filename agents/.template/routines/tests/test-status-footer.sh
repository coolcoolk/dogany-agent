...
# ===========================================================================
# DGN-1013 item D: _ticket_frontmatter scan-depth relaxation
# ===========================================================================
# A YAML block scalar (">" / "|") or multi-line list value in the
# frontmatter pushes the closing "---" fence past the old 20-line hard
# bound, silently dropping an otherwise-valid ticket from _collect_recent_done
# (a real ticket confirmed to close at line 28: id/status/... + a
# multi-line "superseded_parts: >" block + a multi-line "grounded_on:"
# list).  This block reproduces that shape directly against
# _ticket_frontmatter (unit-level; no Stop-hook plumbing needed).
echo "=== DGN-1013 item D: frontmatter scan-depth relaxation ==="
echo ""

FM_DIR="$WORK/frontmatter-overflow"
mkdir -p "$FM_DIR"
FM_TICKET="$FM_DIR/DGN-9001-overflow.md"
{
  echo "---"
  echo "id: DGN-9001"
  echo "status: done"
  echo "updated: 2026-08-24"
  echo "superseded_parts: >"
  for i in $(seq 1 20); do echo "  filler line $i of a long block scalar value."; done
  echo "grounded_on:"
  echo "  - worklog/DGN-1.md"
  echo "title: overflow ticket"
  echo "---"
} > "$FM_TICKET"
FENCE_LINE="$(grep -n '^---$' "$FM_TICKET" | sed -n '2p' | cut -d: -f1)"

out="$("$PYTHON" - "$FOOTER_PY" "$FM_TICKET" <<'PY'
import importlib.util, sys
spec = importlib.util.spec_from_file_location("sf", sys.argv[1])
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
fm = m._ticket_frontmatter(sys.argv[2])
assert fm.get("id") == "DGN-9001", "id not parsed: %r" % fm
assert fm.get("status") == "done", "status not parsed: %r" % fm
assert fm.get("title") == "overflow ticket", "trailing key past old 20-line bound lost: %r" % fm
print("FMTEST-OK %r" % fm)
PY
)" && rc=0 || rc=$?
if [[ "$FENCE_LINE" -gt 21 ]]; then
  ok "fixture closing fence past old 20-line bound (line $FENCE_LINE)"
else
  fail "fixture too short to exercise the old bound (fence at line $FENCE_LINE)"
fi
assert_eq "frontmatter parse unit exit 0" "$rc" "0"
assert_contains "id/status/title all survive the long block scalar" "$out" 'FMTEST-OK'
echo "  $out"
echo ""

# A file whose fence NEVER closes must still terminate (bounded scan, not
# read-to-EOF) -- corrupt/fenceless files must not turn the Stop hook into
# an unbounded read.
FM_NOFENCE="$FM_DIR/DGN-9002-nofence.md"
{
  echo "---"
  echo "id: DGN-9002"
  for i in $(seq 1 500); do echo "prose line $i, no closing fence."; done
} > "$FM_NOFENCE"
out2="$("$PYTHON" - "$FOOTER_PY" "$FM_NOFENCE" <<'PY'
import importlib.util, sys
spec = importlib.util.spec_from_file_location("sf", sys.argv[1])
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
fm = m._ticket_frontmatter(sys.argv[2])
assert fm == {}, "unclosed fence should yield {} (bounded scan): %r" % fm
print("NOFENCE-OK")
PY
)" && rc2=0 || rc2=$?
assert_eq "unclosed fence still bounded (exit 0, no hang)" "$rc2" "0"
assert_contains "unclosed fence -> {} (bounded, not unbounded read)" "$out2" 'NOFENCE-OK'
echo ""
