...
# cat7 (DGN-1058); replaces the .sweepignore path allowlist for this file.
...
rc1=0
# DGN-1122: guard sees this tests/-path harness in ancestry; curl/claude are
# stubbed, so opt in explicitly (override receipt lands on stderr).
PATH="$STUB_DIR:$PATH" PUSH_GUARD_OVERRIDE=send-anyway \
  "$PUSH_SH" --env "$FAKE_ENV" --prompt "test" 2>"$STDERR1" || rc1=$?
...
rc2=0
PATH="$STUB_DIR:$PATH" PUSH_GUARD_OVERRIDE=send-anyway \
  "$PUSH_SH" --env "$FAKE_ENV" --prompt "test" 2>"$STDERR2" || rc2=$?
