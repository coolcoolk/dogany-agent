...
# --- B2: jsonl very stale (beyond new ceiling) -> STALLED, not dropped -----
# DGN-1015 (Rev 13): before this ticket the agent silently vanished here --
# identical on screen to a normal completion (the 2026-08-22 09:33 incident
# shape). It must now surface with a distinct dead-marker instead.
print("\n--- B2: jsonl beyond ceiling -> shown STALLED (DGN-1015 Rev 13) ---")
ceiling = sf.LIVE_STALE_SECS
touch_jsonl(AID, age_secs=ceiling + 60)
write_transcript([launch(AID)])
descs = sf._collect_active_subagents(TRANSCRIPT)
check("agent NOT silently dropped when jsonl exceeds the ceiling", len(descs) == 1)
check("stalled entry carries the dead-marker label",
      len(descs) == 1 and sf.STALLED_LABEL in descs[0])
print("  descs: %r" % descs)
...
# ---------------------------------------------------------------------------
# DGN-1015 Rev 13: silent-death visibility
# ---------------------------------------------------------------------------

# --- D1: jsonl NEVER written (launch race) -> stays silently dropped -------
print("\n--- D1: jsonl never observed -> silently dropped (ambiguous, not dead) ---")
jsonl_path = os.path.join(SUB, "agent-%s.jsonl" % AID)
if os.path.exists(jsonl_path):
    os.remove(jsonl_path)
write_transcript([launch(AID)])
descs = sf._collect_active_subagents(TRANSCRIPT)
check("no jsonl ever written -> not shown at all (no false 'dead' guess)",
      len(descs) == 0)
print("  descs: %r" % descs)

# --- D2: one live + one stalled -> both shown, live first, stalled labeled -
print("\n--- D2: live agent + stalled agent both surface, live sorts first ---")
touch_jsonl(AID, age_secs=5)                    # AID: fresh -> live
touch_jsonl(AID2, age_secs=sf.LIVE_STALE_SECS + 60)  # AID2: stale -> stalled
write_transcript([launch(AID), launch(AID2)])
descs = sf._collect_active_subagents(TRANSCRIPT)
check("both agents surfaced (none silently dropped)", len(descs) == 2)
check("live agent has no dead-marker", not any(
    sf.STALLED_LABEL in d for d in descs[:1]))
check("stalled agent carries the dead-marker",
      any(sf.STALLED_LABEL in d for d in descs))
print("  descs: %r" % descs)
