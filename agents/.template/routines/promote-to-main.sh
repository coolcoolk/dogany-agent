...
#
# In-place promote an EXISTING implicit-main agent to explicit `main`
...
#                   persona file (identity/hot.custom.agent.md, compat name
#                   PROFILE.md). Agent-to-agent minting has no human onboarding
...
#      implicit-main -> explicit `main`. Idempotent no-op if already main.
...
#   - class-stamp uses tmp+mv atomic writes.
...
# stamp and (opt-in) the domain's L1_DB keys, via the atomic conf helper.
#
...
# ---------------------------------------------------------------------------
# idempotency note: class stamp is a no-op if already main.
# ---------------------------------------------------------------------------
if [ "$current_class" = "main" ]; then
  info "parent already promoted (DOGANY_AGENT_CLASS=main) -- class stamp is a no-op"
fi
...
# ---------------------------------------------------------------------------
# step 1: class stamp (atomic).
# ---------------------------------------------------------------------------
# Mirrors install.sh dgn227_postmint: parent .instance.conf DOGANY_AGENT_CLASS
# implicit-main -> explicit `main`. Idempotent no-op if already main.
if [ "$DRY" -eq 1 ]; then
  info "step 1/5 (dry-run): would set DOGANY_AGENT_CLASS=main in $INSTANCE_CONF"
else
  if [ "$current_class" != "main" ]; then
...
if [ -n "$PACK_ID" ]; then
  ...
  if [ "$DRY" -eq 1 ]; then
    ...
    # Template defers all generic-brief plists (plists.defer). The class-stamp
    # step above does not load them; this block does.
    #
