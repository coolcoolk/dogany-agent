...
async def test_messages_during_fastpath_buffer_and_drain(tmp_path, monkeypatch):
    ...
    # serialization point), not spawn a concurrent SDK turn. DGN-911: the
    # default in-flight buffer is now the debounce buffer; the fast-path
    # task's drain flushes it the moment the handler completes.
