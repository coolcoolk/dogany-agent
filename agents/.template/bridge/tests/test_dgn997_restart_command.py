...
Covers:
  ...
  - Duplicate /restart before the process dies: single launch (latch).
    (DGN-1050: the cross-path latch test with the DGN-994 authsync CTA was
    removed with the CTA itself -- /restart is now the latch's only
    trigger; the latch constant lives on as RESTART_LATCH_S.)
...
class TestRestartCommandLaunch(unittest.IsolatedAsyncioTestCase):
    ...
    # DGN-1050: test_authsync_cta_tap_then_restart_command_shares_latch was
    # removed here -- the DGN-994 authsync CTA it exercised is retired with
    # /authsync (its file->keychain sync was the estate auth-death vector).
    # The shared latch attribute (_restart_launch_started) survives with
    # /restart as its only trigger; single-trigger dedup is covered by
    # test_duplicate_command_launches_once above.


class TestRestartErrorCopy(unittest.TestCase):
    """RESTART_ERROR is a NEW string (dec-094 owner-confirm gate) --
    restart-scoped in content ({error} placeholder shape)."""
