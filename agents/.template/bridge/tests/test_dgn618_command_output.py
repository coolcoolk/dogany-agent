...
class HistoryRemovalTest(unittest.TestCase):
    ...
    def test_no_history_in_bot_command_menu(self):
        # DGN-919: menu is built from COMMAND_MENU_SPEC; verify /history absent.
        from bridge.bot import COMMAND_MENU_SPEC
        cmd_names = [cmd for cmd, _ in COMMAND_MENU_SPEC]
        self.assertNotIn("history", cmd_names)

    def test_history_absent_from_help_text_both_locales(self):
        # DGN-919: help_text is now split into header+footer; check both parts.
        for key in ("help_text_header", "help_text_footer"):
            self.assertNotIn("/history", ko.STRINGS.get(key, ""))
            self.assertNotIn("/history", en.STRINGS.get(key, ""))
