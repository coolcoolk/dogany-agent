...
class TestOverflowHandleLocalized:
    ...
    def test_mixed_run_only_overflowing_buttons_degrade(self, ko_locale):
        """Mixed run: only the overflowing options degrade; short ones stay."""
        options = ["실행", "가" * 22, "대기"]
        texts = _button_texts(build_option_keyboard(options))
        assert texts[0] == "1. 실행"
        assert texts[1] == "2번"
...
class TestTwoDigitPrefixWidth:
    def test_prefix_width_flips_overflow_at_position_ten(self, ko_locale):
        ...
        (3.4) -- the overflow decision is per-button, prefix included."""
        body = "가" * 19  # 28.5; +2.4 = 30.9 fits; +3.4 = 31.9 overflows
        options = [body] * 10
        texts = _button_texts(build_option_keyboard(options))
        assert texts[0] == f"1. {body}"
        assert texts[8] == f"9. {body}"
