...
async def test_inflight_runs_alone_pending_two_merged(monkeypatch):
    ...
    # Two more /queue while first is blocking on the barrier (DGN-911: the
    # no-interrupt merge path is now the explicit coalesce=True /queue path).
    await b._enqueue_text_task(user_id, "pending1", ts1, _upd(ts1), coalesce=True)
    await b._enqueue_text_task(user_id, "pending2", ts2, _upd(ts2), coalesce=True)
