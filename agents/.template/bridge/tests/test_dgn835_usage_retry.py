...
def test_preflight_threshold_aligned_with_defer():
    # Pins the BRIDGE half of the seam. It is deliberately a literal: inside a
    # live instance memory-engine is not importable from bridge/, so a test
    # that tried to read the other side would have to skip -- and a skip reads
    # exactly like a pass.
    #
    # This assertion alone is NOT a lockstep, and DGN-818-DESIGN section 5.3
    # was right to call L3 false: memory.py could move to 95.0 and this would
    # still be green. The cross-side comparison lives where BOTH trees exist,
    # in tests/dgn818_lockstep_l3_selftest.sh (DGN-818 C4), which reads
    # memory-engine/memory.py CONSOLIDATE_USAGE_EXHAUSTION_PCT and
    # routines/usage-gate.py CAP_EXEMPT/RELIEF_CAP and fails when they part.
