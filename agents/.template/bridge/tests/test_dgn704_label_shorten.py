"""DGN-704 / DGN-704b / DGN-779: OPTIONS button label shortening forcing point.
...
else as 1.0 -- capped at _BUTTON_LABEL_MAX_WIDTH.

DGN-779 (owner iPhone 13 mini re-measurement, 2026-08-07): whitespace weight
corrected from 1.0 to 0.4 (on-device actual ~0.25; 1.0 was 2.7x overcount).
Budget raised from 30.0 to 31.0 (on-device 1-line boundary ~31.5; -1.0 for
ellipsis reserve).
...
class TestLabelWidth:
    ...
    def test_budget_is_thirty_one(self):
        # DGN-779: raised from 30.0 to 31.0 (on-device 1-line boundary ~31.5,
        # minus 1.0 reserved for the ellipsis glyph).
        assert _BUTTON_LABEL_MAX_WIDTH == 31.0

    def test_space_weight_is_point_four(self):
        # DGN-779: "a b" = 1.0 + 0.4 + 1.0 = 2.4 (space was 1.0, corrected to 0.4)
        assert _label_width("a b") == 2.4

    def test_cjk_space_cjk_weight(self):
        # "가 나" = 1.5 + 0.4 + 1.5 = 3.4
        assert _label_width("가 나") == 3.4

    def test_ellipsis_still_counts_as_one(self):
        # U+2026 is East-Asian-Ambiguous and not whitespace -> weighted 1.0
        assert _label_width("…") == 1.0
...
class TestSkullLengthNowFits:
    ...
    def test_eighteen_korean_chars_fit(self):
        """18 pure-Korean body chars + '1. ' fits under the 31.0 budget -> unchanged.

        DGN-779: '1. ' = '1'(1.0) + '.'(1.0) + ' '(0.4) = 2.4; body = 18*1.5 = 27.0;
        total = 29.4 (was 30.0 when space counted as 1.0).
        """
        label = "1. " + "가" * 18
        result = _shorten_button_label(label)
        assert result == label
        assert _label_width(result) == 29.4
