...
class TestDgn876FoldTrim(unittest.TestCase):
    ...
    # (A) interim == final -> no fold prepended (compose path)
    def test_a_compose_full_dup_no_fold(self):
        whole = "step one"
        msgs = [
    ...
    # (B) compose path: interim superset of final -> fold composed with survivors
    def test_b_compose_superset_keeps_extra_progress_para(self):
        # Single-interim capture (no bubble) -- compose-fallback path.
        # Interim: "extra step\n\nfinal answer" (superset).
        # Final: "final answer".
        # After subtract: "extra step" survives -> fold composed.
        msgs = [
            _make_assistant_msg(
                "tool_use",
        ...
        response, handler, req = self._run(msgs)
        self.assertIsNone(req.fold_msg_id)
        # Fold must be prepended and contain the extra step.
        self.assertTrue(response.content.startswith(">!"))
        self.assertIn("extra step", response.content)
        # Final answer still present, not duplicated.
        self.assertIn("final answer", response.content)
        self.assertEqual(response.content.count("final answer"), 1)
...
class TestGrowingFoldReaderLoop(unittest.TestCase):
    ...
    def test_dgn710_compose_fallback_partial_dup_body_kept_whole(self):
        # DGN-710 + DGN-777 final-sacred: opening streamed as a non-terminal
        # chunk (captured as interim), then the end_turn body re-included it.
        # The body keeps every word; the duplicated opening is subtracted
        # from the FOLD side instead. Here the capture holds nothing else,
        # so no fold is attached and the opening shows exactly once (body).
        ...
        self.assertNotIn(">!", response.content)  # fold emptied -> not attached
        self.assertEqual(response.content, final_body)

    def test_dgn777_compose_fallback_fold_keeps_nonoverlap_paras(self):
        # DGN-777: the capture holds an overlapping AND a non-overlapping
        # paragraph. The overlap leaves the fold; the survivor still composes;
        ...
        response, handler, req = self._run(msgs)
        self.assertIsNone(req.fold_msg_id)
        # Body whole: both final paragraphs intact.
        self.assertIn("P1 shared para", response.content)
        self.assertIn("P3 new answer", response.content)
        # Fold attached, holding ONLY the non-overlapping paragraph.
        self.assertTrue(response.content.startswith(">! P2 fold only"))
        # Shared paragraph shows exactly once (body), never in the fold.
        self.assertEqual(response.content.count("P1 shared para"), 1)
