...
Success criteria (task spec):
  (a) messages arriving during an in-flight turn coalesce into ONE combined
...
async def test_inflight_arrivals_coalesce_no_overflow_notice(monkeypatch):
    ...
    # next three arrive while first is in flight -> buffered, no new turn each.
    await b._enqueue_text_task(user_id, "second", ts1, _upd(ts1))
    await b._enqueue_text_task(user_id, "third", ts2, _upd(ts2))
    await b._enqueue_text_task(user_id, "fourth", ts3, _upd(ts3))
...
async def test_hard_cap_surfaces_notice(monkeypatch):
    ...
    for i in range(cap):
        await b._enqueue_text_task(user_id, f"m{i}", ts0, _upd(ts0))
    assert len(b._user_pending_texts.get(user_id, [])) == cap
    assert b.application.bot.sent == []

    # One past the cap: notice fires, message not appended.
    await b._enqueue_text_task(user_id, "overflow", ts0, _upd(ts0))
    assert len(b._user_pending_texts.get(user_id, [])) == cap
