...
STRINGS = {
    ...
    # conversation). The flag previously reused stop_interrupted -- /stop
    # copy for an action the user never took. Same vocabulary rule as the
    # DGN-1015 approval above: no developer terms. DRAFT copy -- owner
    # confirmation pending; the flag must not ship enabled before this
    # line is approved.
    "auto_interrupt_notice": "하던 작업을 멈추고 방금 주신 메시지를 처리합니다.",
    ...
    # UX-gate item (draft).
    "cmd_desc_health": "에이전트·에스테이트 헬스 점검",
    "cmd_desc_help": "도움말",
    # --- /health command (DGN-986 [c]) ---
    "health_failed": "헬스 점검에 실패했습니다: {error}",
    # --- /btw command (DGN-902) ---
    ...
    # DGN-759 동기화 문구 일괄 제거: file->keychain 덮어쓰기가 회전된(신형)
    # refresh token을 구형으로 되돌려 서버측 패밀리 revoke -> estate 전면
    # 인증 사망의 원인이었음. 남는 문구는 폐기 안내 1건뿐 (숨김 스텁 핸들러가
    # 회신). Copy status: 미확정 (형님 확인 대기, dec-094 gate).
    "authsync_retired": (
        "/authsync 명령은 폐기되었습니다 (DGN-1050). 파일->키체인 동기화가 "
        "자격증명을 반복적으로 무효화시킨 원인이었습니다. 자격증명 갱신은 "
        "CLI가 스스로 관리하며, 자격증명 파일이 낡아 보이는 것은 정상입니다. "
        "재로그인은 터미널에서 `claude auth login` 실행 후 /restart 하세요."
    ),
    ...
    # new bridge terminal-closes the restart instead. Copy status: 미확정
    # (형님 확인 대기) -- proposed wording, owner approval pending.
    "restart_backstop_notice": "재시작 확인: 완료 통지가 중간에 끊겨 브릿지가 대신 알려드립니다. 현재 정상 동작 중입니다.",
    ...
    # UNCONFIRMED (형님 확인 대기) -- interrupt-fold ticket. Caption for the
    # auto-interrupted PLAIN answer draft collapse (distinct surface from
    # fold_caption_stopped above, which is the dev-agent growing-fold caption).
    # Candidates (owner picks one, then this comment + the fallback in
    # formatting.py should be updated to match and this note removed):
    #   1. "중단됨"              -- bare, most conservative (current fallback)
    #   2. "중단됨 · 답변"        -- parallels "중단됨 · 진행 기록" naming
    #   3. "답변 중단됨"          -- reads as a full sentence fragment
    "interrupt_fold_caption": "중단됨",
    ...
    # DGN-1141 stage 4: identical to the en catalog by contract (the
    # model-facing channel grammar is locale-independent; the release lint /
    # test suite asserts en == ko for this key). See en.py for the
    # code-ownership rationale.
    "system_prompt": (
