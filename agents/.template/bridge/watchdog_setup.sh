...
write_service_marker() {
  ...
  # The bridge service plist is the first non-watchdog *.plist in this
  # directory. Note: install.sh install_launchd takes the FIRST *.plist glob
  # match with NO watchdog exclusion (it relies on alphabetical order,
  # newbridge < watchdog); this script excludes *.watchdog.plist explicitly
  # since it registers that one itself.
