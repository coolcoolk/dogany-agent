...
|------------------|-----------------|----------|---------|
| `step_buttons`   | dict step->rows | yes      | per-step keyboard. Key = step "1".."N". Value = `[[label, value], ...]` (one row). |
...
| `confirm_fmt`    | str             | no       | confirmation headline. Positional `{1}`/`{2}`; `{sec}` named extra for hold arms. Fallback: bridge default. |
| `confirm_fmt_skip`| str            | no       | headline for the skip path. |
| `confirm_fold`   | dict            | no       | DGN-939 item6 expandable blockquote appended below the headline. `{"summary": str, "body": str}`. Both may carry `{1}`/`{2}` tokens. Empty/blank summary -> no fold. |
...
Emit `[[IDRILL:<arm_id>]]` on its own line in the model turn's body. The
bridge strips the marker and renders `step_buttons["1"]` + `step_text["1"]` as
an inline keyboard on that message (homomorphic to `[[OPTIONS]]`; no file
