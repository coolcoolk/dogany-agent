...
class TelegramBot:
    ...
    def _idrill_confirm_base(arm: dict, val1, val2: str) -> str:
        """The confirmation headline (DGN-918/924 confirm_fmt path, unchanged)."""
        try:
            val1_num = int(val1)
    ...
    def _idrill_confirm_fold(arm: dict, val1, val2: str) -> str:
        ...
        Both summary and body may carry positional {1}/{2} tokens (step values).
        Returns the neutral `fold:: ... fold::end` typed block (transpiled to a
    ...
    def _idrill_build_step_keyboard(cls, arm: dict, arm_id: str, step: str):
        ...
        rows = [
            [
                InlineKeyboardButton(lbl, callback_data=f"idrill:{arm_id}:{step}:{val}")
                for lbl, val in buttons
            ]
        ]
        # Back button is OPT-IN (arm nav_back: true). DGN-918/924 two-step arms
        # that do not declare nav_back keep the exact single-row keyboard --
        # the arm consumer (Skull) opts into « Back navigation explicitly.
        decl = arm.get("step_buttons")
        first_step = None
        if isinstance(decl, dict):
            digit_keys = sorted(
                (k for k in decl.keys() if isinstance(k, str) and k.isdigit()),
                key=int,
            )
            first_step = digit_keys[0] if digit_keys else None
        if (arm.get("nav_back") is True
                and first_step is not None and step != first_step):
            back_label = arm.get("step_back_label")
            if not isinstance(back_label, str) or not back_label:
                back_label = messages.IDRILL_BACK_LABEL
            rows.append([
                InlineKeyboardButton(
                    back_label, callback_data=f"idrill:{arm_id}:{cls._IDRILL_BACK_ACTION}"
                )
            ])
        step_text = arm.get("step_text")
    ...
    async def _idrill_fire_final(
        ...
        # Confirmation render: keep the DGN-918/924 (val1, val2) signature for
        # 2-step arms; for N-step arms pass step1 + this final value.
        val1 = captures[0][1] if captures else "?"
