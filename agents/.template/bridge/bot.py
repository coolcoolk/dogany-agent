...
class TelegramBot:
    ...
    async def _set_bot_commands(self) -> None:
        commands = [
            ...
            BotCommand("authsync", messages.CMD_DESC_AUTHSYNC),
            BotCommand("btw", messages.CMD_DESC_BTW),
    ...
    async def _try_fastpath(self, user_id: int, text: str, update) -> bool:
        ...
        async with self._get_user_queue_lock(user_id):
            if self._prune_user_tasks(user_id):
                # Turn in flight -> let the coalescing path buffer this message.
                return False
    ...
    async def _cmd_btw(
        ...
        async def run_fork_task() -> None:
            ...
            # Edit the thinking placeholder to the real answer, or send a new
            # reply if the edit fails (e.g. message too old or deleted).
            try:
                ...
                try:
                    new_msg = await self.application.bot.send_message(
                        chat_id=chat_id,
                        text=marked,
                    ...
                    # Re-register fork with the new message_id as anchor so
                    # reply-to routing still works.
                    new_anchor = new_msg.message_id
                    fork.anchor_message_id = new_anchor
                    self._btw_forks.register_fork(user_id, fork)
                except Exception as send_err:
                    logger.error(
                        "btw fallback send also failed for user %s: %s",
                        user_id,
                        send_err,
                    )

        # Fire the fork task as a tracked background task (tracked for /stop
        # compatibility; the fork does NOT go through the coalescing path since
        # it is independent of the main session queue).
        task = asyncio.create_task(run_fork_task())
        self._track_user_task(user_id, task)
    ...
    async def _maybe_route_btw_reply(
        ...
        async def run_fork_continuation() -> None:
            ...
            marked = f"{messages.BTW_MARKER}\n\n{response}"
            try:
                # Reply-to the user's follow-up message so the chain is clear.
                await self.application.bot.send_message(
                    chat_id=chat_id,
                    text=marked,
                    reply_parameters=ReplyParameters(
                ...
                logger.error(
                    "btw fork continuation send failed for user %s: %s",
                    user_id,
                    e,
                )

        task = asyncio.create_task(run_fork_continuation())
        self._track_user_task(user_id, task)
        return True
