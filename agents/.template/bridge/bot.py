...
class TelegramBot:
    ...
    async def _send_content_artifacts(
        ...
        if idrill_arm_id is not None:
            await self._idrill_send_initial_keyboard(message, idrill_arm_id)

    async def _idrill_send_initial_keyboard(self, message, arm_id: str) -> None:
        """DGN-924: render the initial step1 keyboard for an [[IDRILL:..]] marker.

        Reads the arm file, builds an InlineKeyboardMarkup from
        arm.step_buttons["1"] (callback_data idrill:<arm_id>:1:<value>) and sends
        arm.step_text["1"] as the message text. The arm file is the single source
        of the buttons and the copy (contract v3). Fail-soft: an invalid arm_id
        or a missing / button-less / text-less arm drops the keyboard silently
        (the marker was already stripped from the body).
        """
        ...
        if arm is None:
            ...
            return
        step1_buttons = self._idrill_step_buttons(arm, "1")
        if not step1_buttons:
            logger.warning("idrill: arm %s declares no step_buttons[1]", arm_id)
            return
        step_text = arm.get("step_text")
        text = step_text.get("1") if isinstance(step_text, dict) else None
        ...
        rows = [
            [
                InlineKeyboardButton(
                    lbl, callback_data=f"idrill:{arm_id}:1:{val}"
                )
                for lbl, val in step1_buttons
            ]
        ]
        try:
            await message.reply_text(
    ...
    async def _send_smart(
        ...
        if force_options and has_marker:
            ...
            if kb:
                # DGN-932 mechanism B: silent button appendix (see
                # _send_content_artifacts for rationale).
                # Turn-level 1-loud guarantee: if no loud send preceded
                # (list-only, non-streamed, no drafts), buttons are promoted
                # to loud so the turn is never completely silent.
                silent = (
                    notify_silent("options_prompt")
                    if body_was_loud
                    else False
    ...
    @staticmethod
    def _idrill_step_buttons(arm: dict, step: str):
        """Return the [[label, value], ...] button list an arm declares for a
        step, or None if the arm declares no buttons for that step.

        The arm file is the single source of the step keyboard (contract v3):
        the bridge holds no hardcoded button set. Malformed declarations
        (non-list rows, non-2-tuples, non-string label/value) are dropped so a
        bad arm cannot inject arbitrary structure into the keyboard.
        """
    ...
    @classmethod
    def _idrill_declared_values(cls, arm: dict, step: str):
        """Set of tap values the arm declares as buttons for a step (whitelist).
        Empty set if the arm declares no buttons for that step.
        """
        buttons = cls._idrill_step_buttons(arm, step)
        if not buttons:
            return frozenset()
        return frozenset(value for _label, value in buttons)
    ...
    async def _idrill_fire_cmd(self, arm: dict, val1, val2: str) -> bool:
        ...
        arm-declaration-whitelisted values and executes the list verbatim:
        list-form subprocess, no shell, no PATH lookup, no cwd change,
        no re-parsing of declared elements.  Returns True on exit 0.
        """
        key = "cmd_skip2" if val2 == self._IDRILL_SKIP_VALUE else "cmd"
        ...
        # Whitelist the substitution values against the arm's own declaration
        # (contract v3): a value may enter the argv only if it belongs to the
        # step's declared buttons or passes the step's declared validate regex.
        # The bridge holds no hardcoded numeric set -- the gate is preserved,
        # only its source moved from bridge constants to the arm file.
        # Everything else in the argv is the file's own declaration and passes
        # through untouched (no re-parsing).
        val1_str = str(val1)
        if not self._idrill_value_allowed(arm, "1", val1_str):
            logger.warning("idrill: step1 value rejected by whitelist: %r", val1_str)
            return False
        val2_sub = None
        if val2 != self._IDRILL_SKIP_VALUE:
            if not self._idrill_value_allowed(arm, "2", val2):
    ...
    async def _handle_idrill_callback(self, query, data: str) -> None:
        ...
        if action == "1":
            if value == self._IDRILL_OTHER_VALUE:
                # Free-text fallback (declared special value "other"): prompt
                # the user; leave arm file for TTL cleanup.
                try:
            ...
            # Gate the tapped step1 value against the arm's own declaration
            # (contract v3): only a declared/validated value may be captured.
            if not self._idrill_value_allowed(arm, "1", value):
                logger.warning(
                    "idrill: step1 tap value %r not allowed by arm %s",
                    value, arm_id,
                )
                try:
                    await query.edit_message_text(messages.IDRILL_ERROR)
            ...
            # Build the step2 keyboard from the arm's declared step_buttons["2"]
            # (contract v3): the arm file is the single source of the keyboard.
            step2_buttons = self._idrill_step_buttons(arm, "2")
            if not step2_buttons:
                logger.warning(
                    "idrill: arm %s declares no step_buttons[2]", arm_id,
                )
                try:
                    await query.edit_message_text(messages.IDRILL_ERROR)
                except Exception:
            ...
            # DGN-924: when the arm declares step_text["2"], swap both the text
            # and the keyboard so the step2 prompt copy shows (the arm file owns
            # the copy). Otherwise keep the markup-only swap (backward-compat).
            step_text = arm.get("step_text")
            step2_text = step_text.get("2") if isinstance(step_text, dict) else None
            step2_markup = InlineKeyboardMarkup(step2_rows)
            if isinstance(step2_text, str) and step2_text:
                try:
                    await query.edit_message_text(
                        step2_text, reply_markup=step2_markup
                    )
                except Exception as e:
                    logger.warning(
                        "idrill: edit_message_text (step2 swap) failed for arm_id=%s: %s",
                        arm_id, e,
                    )
                return
            try:
                await query.edit_message_reply_markup(reply_markup=step2_markup)
            except Exception as e:
                logger.warning(
        ...
        if action == "2":
            ...
            # Gate the tapped step2 value against the arm's own declaration
            # (contract v3): the value must be a declared button / pass the
            # declared validate regex. The special "skip" value is itself a
            # declared button, so it flows through the same gate.
            if not self._idrill_value_allowed(arm, "2", value):
                logger.warning(
                    "idrill: unexpected step2 value %r for arm_id=%s",
                    value, arm_id,
                )
                try:
                    await query.edit_message_text(messages.IDRILL_ERROR)
