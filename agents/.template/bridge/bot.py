...
class TelegramBot:
    ...
    async def _cmd_restart(
        ...
        # Duplicate-launch latch: SHARED with _handle_authsync_restart_callback
        # (the DGN-994 CTA tap) via the same _restart_launch_started attribute
        # -- both paths launch the identical self_restart.sh --trigger user
        # operation, so a rapid double /restart, or a /restart immediately
        # after an authsync CTA tap (or vice versa), must not double-fire the
        # script. Window-based, not sticky, so an aborted restart self-heals
        # (see AUTHSYNC_RESTART_LATCH_S).
        now = time.monotonic()
        ...
        def _run_restart() -> subprocess.CompletedProcess:
            ...
            # here (mirrors _handle_authsync_restart_callback).
            return subprocess.run(
                [
                    str(script),
                    "--trigger", "user",
    ...
    async def _handle_authsync_restart_callback(self, query) -> None:
        ...
        """
        # Duplicate-launch latch: the keyboard-clearing edit below is async, so
        # two rapid taps can both arrive before the button disappears. First
        # trigger inside the window wins; the rest are dropped silently.
        # Window-based so an aborted restart self-heals (see
        # AUTHSYNC_RESTART_LATCH_S). DGN-997: this latch is SHARED with
        # _cmd_restart (the /restart slash command) via the same
        # _restart_launch_started attribute -- both paths launch the
        # identical self_restart.sh --trigger user operation, so a tap on
        # this CTA followed immediately by a /restart (or vice versa) must
        # not double-fire the script.
        now = time.monotonic()
        started = getattr(self, "_restart_launch_started", None)
        if started is not None and now - started < AUTHSYNC_RESTART_LATCH_S:
            return
        ...
        # for the window so a second tap/command cannot double-fire mid-restart.
