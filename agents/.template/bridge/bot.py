    ...
    async def _send_content_artifacts(
        ...
        if force_options:
            ...
            if has_marker:
                ...
                # (strip_consumed_options first, then the plain extractor so
                # a non-adjacent/failed run still renders its buttons).
                button_options = options
                if not button_options:
                    _, button_options = strip_consumed_options(clean)
                if not button_options:
