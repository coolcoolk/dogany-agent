...
# DGN-991 stopgap (rev3): background-consequence note appended on /stop soft
# success -- the soft interrupt itself aborts the in-flight turn's tree, so
# its subagents die at the FIRST stop (see the i18n rev3 comment) -- and the
# honest hard-teardown result copy (see _cmd_stop / _hard_stop).
STOP_BG_NOTE = t("stop_bg_note")
...
AUTHSYNC_RESTART_BUTTON = t("authsync_restart_button")  # DGN-994 restart CTA
...
# only (script missing / launch exception / non-zero exit), same shape as
# AUTHSYNC_ERROR but restart-scoped so the copy stays accurate.
...
IDRILL_NAV_ROOT = t("idrill_nav_root")
IDRILL_BACK_LABEL = t("idrill_back_label")
