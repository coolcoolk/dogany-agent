        ...
        if _effective_interim_mode() == "fold":
            if req.fold_msg_id is not None:
                ...
                if fold:
                    ...
                    # D6 / DGN-878: interim WAS captured but nothing survived
                    # composition. Three causes are indistinguishable here --
                    # whitespace-only capture, fold budget exhausted by a
                    # large final, or every paragraph subtracted as
                    # final-answer overlap -- so enumerate, never single one
                    # out. Log distinctly from the ordinary no-interim turn
                    # so silent-empty causes stay traceable.
                    logger.info(
                        "DGN-876 fold dropped for user %s: %d captured "
                        "interim block(s) composed empty (whitespace-only, "
                        "over budget, or fully subtracted as final-answer "
                        "overlap)",
                        user_id,
