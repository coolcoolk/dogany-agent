...
# Pattern that matches any model-written [라이브] or [결정대기] block in the
# response body.  The bridge strips these before appending the sidecar footer
# so the hook is the sole author regardless of what the LLM wrote.
...
def _consume_footer_sidecar(content: str) -> str:
    """Read the footer sidecar, strip model-written footer blocks from content,
    append the canonical footer once, then clear the sidecar.
    ...
    - Sidecar footer is empty string (noise-suppression turn): strip any
      model-written footer blocks and return without appending anything.
    - Sidecar footer is non-empty: strip model-written blocks, append the
      canonical footer once at the end.
    ...
    try:
        ...
        # Strip all model-written footer blocks regardless of whether the
        # hook has anything to append.
...
class SdkBridge:
    ...
    async def _finalize_result(
        ...
        if (
            ...
            # and keep the clean final so the two never overlap.
            if self._final_fully_in_interim(content, req.interim_texts):
                await self._fold_delete(req)
            else:
                # DGN-777: partial overlap -- trim the overlapping paragraphs
                # OUT OF THE FOLD render source (fold_buf) so the finalize
                # swap below shows only non-duplicated progress. A fold that
                # empties after the trim is deleted outright (no empty
                # bubble). The answer body is never touched.
        ...
        # Strip any model-written [라이브]/[결정대기] blocks from the content
        # first (the hook is the sole author), then append the canonical footer
        # once.  Empty sidecar (noise-suppression path) -> no footer appended.
        # Fail-silent: sidecar missing / corrupt -> content unchanged.
        ...
        if _effective_interim_mode() == "fold":
            if req.fold_msg_id is not None:
                ...
                # final-vs-interim overlap handling as the grown-bubble path
                # above. DGN-777 final-sacred: the answer body is never
                # trimmed -- overlapping paragraphs are subtracted from the
                # interim capture BEFORE the fold is composed, so a duplicated
                # opening shows once (in the body), never twice.
                if self._final_fully_in_interim(content, req.interim_texts):
                    ...
                    interim_trimmed = self._subtract_paras(
                        req.interim_texts, content
                    )
                    fold = compose_interim_fold(interim_trimmed, content)
                    if fold:
                        content = fold + INTERIM_FOLD_SEPARATOR + content
                    elif req.interim_texts:
                        # D6 / DGN-777: interim WAS captured but nothing
                        # survived composition (whitespace, budget-drop, or
                        # every paragraph subtracted as final-answer overlap)
                        # -- log distinctly from the ordinary no-interim turn
                        # so silent-empty causes stay traceable.
                        logger.info(
                            "DGN-682 fold dropped for user %s: %d captured "
                            "interim block(s) composed empty after overlap "
                            "subtraction",
                            user_id,
