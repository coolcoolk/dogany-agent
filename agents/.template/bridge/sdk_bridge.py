...
class SdkBridge:
    ...
    async def _maybe_mark_options(prev_message: str, content: str) -> Tuple[str, bool]:
        ...
        """
        # DGN-992: the bare-marker substring check misses the labeled form
        # ("[[OPTIONS]]" is not a substring of "[[OPTIONS: a | b]]"); the
        # line-based has_options_marker covers both so the classifier never
        # double-injects over an authored labeled marker.
        marker_present = OPTIONS_MARKER in content or has_options_marker(content)
        if not (has_numbered_list(content) and not marker_present):
    ...
    async def interrupt(self, user_id: int) -> bool:
        ...
        Task-tool subagents execute inside that tree -- so a soft interrupt
        kills the turn's in-process background subagents (2026-08-21
        incident: soft interrupt only in the log, no hard kill, subagents
        dead). The user-facing /stop reply carries STOP_BG_NOTE for this
        reason. Root fix (background work outside the session process) is
        the DGN-991 v2.0 relocation, out of scope here.
