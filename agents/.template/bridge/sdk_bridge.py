    ...
    async def _finalize_result(
        ...
        if _effective_interim_mode() == "fold":
            if req.fold_msg_id is not None:
                ...
                interim_trimmed = self._subtract_paras(req.interim_texts, content)
                fold = compose_interim_fold(interim_trimmed, content)
                if fold:
                    content = fold + INTERIM_FOLD_SEPARATOR + content
                elif req.interim_texts:
                    logger.info(
                        "DGN-876 fold dropped for user %s: %d captured interim block(s) "
                        "composed empty after final-overlap subtraction",
                        user_id,
