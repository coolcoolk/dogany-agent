...
# DGN-726: usage-exhaustion defer. When the account usage window is exhausted the
# nightly sonnet compression is guaranteed to fail (rc=1). That is a known-transient
# condition, not a code defect, so it must NOT surface to the owner (no report, no
# cron-guard alert). Instead we defer: preserve the watermark and drop a marker that
# records the next usage-reset boundary; the wrapper (consolidate-0430.sh) reads it to
# schedule a reset-aligned one-shot retry. Fail-open everywhere: any usage-lookup or
# scheduling error falls back to the normal (visible) path or the next daily run.
...
# Marker file the wrapper consumes to schedule the reset-aligned retry. Lives beside
# state.db (memory-engine dir) so it travels with a workspace move.
...
def _query_usage_state():
    ...
    out = {
        ...
        # five_hour_resets_at is not surfaced by usage-gate's cache shape; fetch it
        # from the raw usage payload if get_usage produced a live refresh that kept
        # it. Absent -> "" (caller falls back to seven_day boundary).
...
def _write_defer_marker(reset_iso):
    """DGN-726: drop the usage-defer marker the wrapper consumes to schedule the
    reset-aligned retry. Best-effort (fail-open): any write error is swallowed --
    the watermark is preserved regardless, so the next daily run re-collects the
    same span even without a scheduled retry."""
    try:
        payload = {
        ...
        tmp = CONSOLIDATE_DEFER_MARKER + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(payload, f)
        os.replace(tmp, CONSOLIDATE_DEFER_MARKER)
    except OSError as e:
...
def cmd_consolidate(args):
    ...
    if not args.dry_run:
        ...
        if _usage_exhausted(_preflight_usage):
            ...
            if _reset_iso:
                print(f"[consolidate] usage resets at {_reset_iso} -- scheduling reset-aligned retry.")
            else:
                print("[consolidate] usage reset time unknown -- will retry on next daily run.")
            _write_defer_marker(_reset_iso)
            conn.close()
            return 0  # exit 0 -> cron-guard silent; no report push.
    ...
    if convo and not candidates and compress_failed and compress_failed == len(_chunk_convo(convo)):
        ...
        if _usage_exhausted(usage):
            ...
            if reset_iso:
                print(f"[consolidate] usage resets at {reset_iso} -- scheduling reset-aligned retry.")
            else:
                print("[consolidate] usage reset time unknown -- will retry on next daily run.")
            if pending_retries:
                print(f"[consolidate] retry queue untouched ({len(pending_retries)} chunk(s) still queued for retry)")
            if not args.dry_run:
                _write_defer_marker(reset_iso)
            conn.close()
            # Return 0 so cron-guard stays silent (exit==0 = no alert). No report push.
            return 0
