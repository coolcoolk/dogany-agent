# DGN-726 (protocol upgraded by DGN-835): consolidate usage-exhaustion =
# 2-factor defer signal (exit 75 + marker with window field); cron-guard owns
# recovery routing. Fully sandboxed: temp state.db + temp memories dir; all
# LLM / transcript / embedding / push calls stubbed; usage state stubbed
# (never hits the network or usage-gate). Never touches the live engine state
# or memories/.
#
# Run: python3 -m pytest memory-engine/tests/test_dgn726_usage_defer.py -v
#
# Coverage:
#   - usage-exhausted all-fail  -> NO push, exit 75, watermark preserved, marker
#     written with window + next reset boundary (POST-HOC classifier).
#   - PRE-FLIGHT exhausted       -> compression skipped, exit 75, marker written, silent.
...
#   - DGN-835: env DOGANY_USAGE_DEFER_MARKER takes priority over the legacy path.
#   - DGN-835 B3: .consolidate-last-success stamped on success/no-new-data only.
...
def sandbox(tmp_path, monkeypatch):
    ...
    # DGN-835: env override must not leak in from the host environment.
    monkeypatch.delenv("DOGANY_USAGE_DEFER_MARKER", raising=False)
    # DGN-835 B3: last-success artifact in the sandbox too.
    monkeypatch.setattr(memory, "CONSOLIDATE_LAST_SUCCESS", str(tmp_path / ".last-success"))
...
def test_usage_defer_is_fully_silent(sandbox, monkeypatch):
    ...
    assert rc == 75, "usage-defer must exit 75 (EX_TEMPFAIL, DGN-835 2-factor signal)"
    ...
    assert m["window"] == "five_hour", "marker must name the exhausted window"
    assert m["last_success_at"] == "", "no success artifact yet -> empty field (fail-open)"
    assert not (tmp_path / ".last-success").exists(), "defer must NOT stamp last-success"


def test_defer_marker_carries_last_success(sandbox, monkeypatch):
    # DGN-835 MAJOR-C: the marker ferries the last-SUCCESS timestamp so the
    # cron-guard stale reader never touches engine paths.
    tmp_path = sandbox
    (tmp_path / ".last-success").write_text("2026-08-10T04:35:00Z\n", encoding="utf-8")
    monkeypatch.setattr(memory, "compress_with_haiku", _fail_all_compressor)
    monkeypatch.setattr(memory, "_query_usage_state", lambda: {
        "five_hour_util": 100.0, "seven_day_util": 60.0,
        "five_hour_resets_at": "2026-08-05T12:00:00Z", "seven_day_resets_at": "",
    })
    monkeypatch.setattr(memory, "_send_silent_report", lambda rep: True)
    ...
    assert m["last_success_at"] == "2026-08-10T04:35:00Z", \
        "marker must embed the artifact content (stripped)"
...
def test_preflight_skips_compression(sandbox, monkeypatch):
    ...
    assert rc == 75, "pre-flight defer must exit 75 (DGN-835)"
    ...
    assert m["window"] == "five_hour"
    assert not (tmp_path / ".last-success").exists()
...
def test_genuine_failure_visible_when_usage_unknown(sandbox, monkeypatch):
    ...
    assert not (tmp_path / ".last-success").exists(), "failure must NOT stamp last-success"


# ---------------------------------------------------------------------------
# DGN-835: env-priority marker path (cron-guard protocol)
# ---------------------------------------------------------------------------

def test_env_marker_path_takes_priority(sandbox, monkeypatch):
    tmp_path = sandbox
    env_marker = tmp_path / "usage-defer" / "com.test.label.json"
    monkeypatch.setenv("DOGANY_USAGE_DEFER_MARKER", str(env_marker))
    monkeypatch.setattr(memory, "compress_with_haiku", _fail_all_compressor)
    monkeypatch.setattr(memory, "_query_usage_state", lambda: {
        "five_hour_util": 10.0, "seven_day_util": 99.5,
        "five_hour_resets_at": "", "seven_day_resets_at": "2026-08-06T13:00:00Z",
    })
    monkeypatch.setattr(memory, "_send_silent_report", lambda rep: True)
    ...
    assert rc == 75
    assert not (tmp_path / ".defer.json").exists(), "legacy path must NOT be written when env is set"
    assert env_marker.is_file(), "env marker path must be written (parent dir auto-created)"
    m = json.loads(env_marker.read_text(encoding="utf-8"))
    assert m["window"] == "seven_day"
    assert m["reset_at"] == "2026-08-06T13:00:00Z"


def test_exhausted_window_prefers_five_hour():
    # both exhausted -> five_hour (resets sooner -> earliest automatic retry).
    assert memory._exhausted_window(
        {"five_hour_util": 100.0, "seven_day_util": 100.0}) == "five_hour"
    assert memory._exhausted_window(
        {"five_hour_util": 10.0, "seven_day_util": 99.0}) == "seven_day"
    assert memory._exhausted_window(
        {"five_hour_util": 10.0, "seven_day_util": 10.0}) == ""
    assert memory._exhausted_window(None) == ""


# ---------------------------------------------------------------------------
# DGN-835 B3: .consolidate-last-success stamped on normal completion only
# ---------------------------------------------------------------------------

def test_no_new_data_stamps_last_success(sandbox, monkeypatch):
    tmp_path = sandbox
    rc, out = run_consolidate(monkeypatch, "", None, 0)  # no convo, no retries
    assert rc == 0
    assert "No new conversations" in out
    stamp = tmp_path / ".last-success"
    assert stamp.is_file(), "no-new-data is a normal completion -> stamp"
    # ISO-8601 UTC content
    body = stamp.read_text(encoding="utf-8").strip()
    assert body.endswith("Z") and "T" in body


def test_success_run_stamps_last_success(sandbox, monkeypatch):
    tmp_path = sandbox
    # compressor yields no keepable candidates -> "No persistent facts" exit 0.
    monkeypatch.setattr(memory, "compress_with_haiku",
                        lambda text, prompt_prefix=None, model=None: [])
    monkeypatch.setattr(memory, "_query_usage_state", lambda: {
        "five_hour_util": 10.0, "seven_day_util": 10.0,
        "five_hour_resets_at": "", "seven_day_resets_at": "",
    })
    monkeypatch.setattr(memory, "_send_silent_report", lambda rep: True)
    convo = CHUNK_OK
    rc, out = run_consolidate(monkeypatch, convo, "2026-07-14T01:00:00.000000Z", 1)
    assert rc == 0
    assert (tmp_path / ".last-success").is_file()
