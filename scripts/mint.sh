...
# relmod.sh: CLI wrapper the relationship-care bundle skill and the morning
# brief (routines/bundle/morning-brief.sh) call at <instance>/database/ --
# without it the contact-gap section and the whole relationship-care skill
# silently no-op. Keep this list in lockstep with update.sh (3f) and
# publish.sh INSTALL_SHARED_FILES.
...
if [ -f "$LIFEKIT_DB" ]; then
  ...
  # DGN-674 F7: a silent skip here shipped instances whose structured lane
  # (lifekit.db) never existed -- lifekit activation then broke much later
  # with no breadcrumb (A3). sqlite3 is preinstalled on macOS; on minimal
  # Linux it is one package away. Hard fail so the gap surfaces AT MINT.
  echo "[mint][ERROR] sqlite3 not found -- cannot initialize lifekit.db (the structured lane)." >&2
  echo "  Install sqlite3 (e.g. apt-get install sqlite3 / apk add sqlite) and re-run mint." >&2
  exit 1
else
...
#        Stays WARN-only (not part of the F1 hard gate): AGENT.md/USER.md are
