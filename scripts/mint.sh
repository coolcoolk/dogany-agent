...
mint.sh -- instantiate a standalone dogany-agent
  ...
  Secrets (environment variables ONLY -- argv is visible in ps):
    ...
    DOGANY_EMAIL_PW    email app password for .env (no flag exists on purpose)
...
AGENT_PREFIX="[agent]"
AGENT_LANG="en"
# Secrets (DOGANY_BOT_TOKEN, DOGANY_EMAIL_PW) come from environment only;
# they are never stored as script-level variables to avoid any accidental echo.
BUILD_VENV=1
CORE_ONLY=0
FORCE=0
OWNER_IDS=""           # --owner-id
...
# renders). Emits the exact key names bridge/config.py reads. Reads secrets
# from environment (DOGANY_BOT_TOKEN, DOGANY_EMAIL_PW -- set by caller, never
# argv). Conditional lines: TZ only when a tz was given; BRIDGE_MODELS /
# LOCAL_WHISPER_MODEL only when set so config.py defaults apply. LOCALE is
# ALWAYS emitted. No [mint] banner lines on stdout.
env_render() {
