...
def step_natural_key_maps(ag_copy: str, warg_copy: str,
    ...
    # (verified: same name, Warg has no FK refs on event.place_id or routine_def.place_id)
    ...
    shared_tables = ['workouts', 'meals', 'metric_log', 'event', 'sub_event',
...
def _compute_warg_id_remap(warg_copy: str, maps: dict) -> dict:
    ...
    # event: Warg events offset
    ev_offset = _make_offset('event')
    ev_remap = {}
    for wid, in warg.execute('SELECT id FROM event ORDER BY id'):
        ev_remap[wid] = wid + ev_offset
    id_remap['event'] = ev_remap
...
def _owning_agent_for_row(table: str, created_by: str = None) -> str:
    ...
    if table == 'routine_def':
        if created_by in AG_CREATED_BY_VALUES:
            return 'ag'
        return 'warg'
    # event/sub_event: preserve verbatim (handled in caller)
    return 'warg'
...
class MergeEngine:
    ...
    def _step_load_warg(self):
        ...
        # --- event: Warg events ---
        ev_dst_cols = [c for c in column_names(tgt, 'event')
                       if c not in ('ag_source_id',)]
        ev_src_cols = [c for c in column_names(warg, 'event')
                       if c not in ('ag_source_id',)]
        ev_common = [c for c in ev_dst_cols if c in set(ev_src_cols)]

        for row in warg.execute(
            f'SELECT {", ".join(chr(34)+c+chr(34) for c in ev_common)} FROM event ORDER BY id'
        ):
            row_dict = dict(zip(ev_common, row))
            wid = row_dict['id']
            new_id = id_remap['event'].get(wid, wid)
            row_dict['id'] = new_id
        ...
        for row in warg.execute(
            ...
            if 'event_id' in row_dict and row_dict['event_id'] is not None:
                row_dict['event_id'] = id_remap['event'].get(
    ...
    def _step_verify(self) -> dict:
        ...
        # event
        check_count('event', table_count(ag, 'event'), table_count(warg, 'event'))
        # config
        ...
        # ---- area_id gate (workouts, meals, event, routine_def) ----
        area_row_failures = 0
        for tbl in ('workouts', 'meals', 'event', 'routine_def'):
