...
#   T12 second kit end-to-end (DGN-1003): provides_kit=fookit installs
#       service/fookit/, database/fookit.py is migrate-recognized, db-init
#       creates fookit.db, config/fookit.conf merges; no lifekit residue.
#   T13 capabilities.db_lane=false (DGN-1003): db-init/migrate capability-SKIP
#       (loud), install succeeds, no DB file created, non-DB surfaces land.
#   T14 db_lane=false + shipped database/ files = FATAL on the INSTALLER's own
#       check (skip-smuggling, lockstep with compat-lint C3).
#   T15 poisoned provides_kit (traversal / reserved) refused by the installer's
#       own validation; nothing written outside the instance root.
#   TL  kit-name contract lockstep matrix (DGN-1003 criterion 6): compat-lint
#       and pack_install run over the same manifests; any verdict or
#       resolved-name divergence fails the suite (drift alarm for the
#       intentionally duplicated validation).
#
...
# _mk_pack <dir> <pack_version> <euv> [kit]
# Creates a minimal kit pack fixture at <dir> with (kit defaults to lifekit;
# DGN-1003 generalization -- all kit-name-derived surfaces follow <kit>):
#   pack-manifest.json (kind=kit, provides_kit=<kit>, payload_root=payload)
#   payload/database/<kit>.py  (EXPECTED_USER_VERSION=<euv>)
#   payload/database/<kit>.sh  (check/dump stubs, exit 0)
#   payload/database/schema.sql  (PRAGMA user_version=<euv>)
#   payload/database/migrations/001_init.sql (if euv>=1, -- reversible: yes)
#   payload/service/<kit>/__init__.py
#   payload/skills-bundle/diet-log/SKILL.md
#   payload/routines/bundle/morning-brief.sh
#   payload/config/<kit>.conf  (KEY_A=val_a, KEY_NEW=val_new)
_mk_pack() {
  local dir="$1" ver="$2" euv="$3" kit="${4:-lifekit}"

  mkdir -p "$dir/payload/database/migrations"
  mkdir -p "$dir/payload/service/$kit"
...
MANIFEST
  ...
  # payload/database/<kit>.py with EXPECTED_USER_VERSION
  cat > "$dir/payload/database/$kit.py" <<PY
# $kit.py stub for tests
EXPECTED_USER_VERSION = $euv
PY

  # payload/database/<kit>.sh -- CLI contract surface (C6: check + dump exit 0)
  cat > "$dir/payload/database/$kit.sh" <<'SH'
#!/bin/bash
# kit CLI stub: CLI contract surface for tests (exit 0 on check/dump)
case "${1:-}" in
  check|dump) exit 0 ;;
  *) exit 1 ;;
esac
...
SQL
  ...
  # payload/service/<kit>/__init__.py
  echo "# service facade stub" > "$dir/payload/service/$kit/__init__.py"
...
SH
  ...
  # payload/config/<kit>.conf (two keys: one existing-compat, one new)
  cat > "$dir/payload/config/$kit.conf" <<CONF
KEY_A=val_a
KEY_NEW=val_new
CONF
...
cat > "$FIXTURE_CATALOG_DIR/catalog.json" <<'CAT'
{
  ...
  "packs": [
    ...
    { "id": "nodbkit", "kind": "kit", "package_dir": "unused", "status": "published" }
  ]
}
CAT

# _run_install <pack_dir> <instance_dir> [pack_id] -- runs pack_install.sh kit
# path (pack_id defaults to lifekit). Returns installer exit code; output
# captured to /dev/null unless VERBOSE=1.
_run_install() {
  local pack="$1" inst="$2" pid="${3:-lifekit}"
...
# _run_install_out <pack_dir> <instance_dir> <pack_id> -- like _run_install but
# prints combined stdout+stderr (for log-line assertions). Exit code preserved.
_run_install_out() {
  local pack="$1" inst="$2" pid="$3"
  bash "$INSTALLER" "test-slug" "$inst" \
    --pack "$pid" \
    --pack-dir "$pack" \
    --catalog "$FIXTURE_CATALOG_DIR/catalog.json" \
...
# ---------------------------------------------------------------------------
# T12: second kit end-to-end (DGN-1003 success criteria 2/3/4)
# provides_kit=fookit: service/fookit/ installs (not dropped), database/
# fookit.py is recognized by migrate (no "no versioned kit core" skip),
# db-init names the DB fookit.db, config/fookit.conf merges.
# ---------------------------------------------------------------------------
echo "T12: fookit kit install (kit-name generalization end-to-end)"

_t12_pack="$(mktemp -d)"
_t12_inst="$(mktemp -d)"
_mk_pack "$_t12_pack" "1.0.0" "1" "fookit"
_mk_instance "$_t12_inst"

_t12_out="$(_run_install_out "$_t12_pack" "$_t12_inst" "fookit")"
_t12_rc=$?
if [[ "$_t12_rc" -eq 0 ]]; then
  ok "T12: installer exit 0 for provides_kit=fookit"
  if [[ -f "$_t12_inst/service/fookit/__init__.py" ]]; then
    ok "T12: service/fookit/__init__.py installed (facade not dropped)"
  else
    bad "T12: service/fookit/__init__.py MISSING (facade silently dropped)"
  fi
  if [[ -f "$_t12_inst/database/fookit.py" ]]; then
    ok "T12: database/fookit.py installed"
  else
    bad "T12: database/fookit.py MISSING"
  fi
  if [[ -f "$_t12_inst/database/fookit.db" ]]; then
    _t12_uv="$(sqlite3 "$_t12_inst/database/fookit.db" 'PRAGMA user_version;' 2>/dev/null || echo '?')"
    if [[ "$_t12_uv" == "1" ]]; then
      ok "T12: fookit.db initialized, user_version==1"
    else
      bad "T12: fookit.db user_version=$_t12_uv (expected 1)"
    fi
  else
    bad "T12: database/fookit.db MISSING (db-init used wrong name?)"
  fi
  if [[ -e "$_t12_inst/database/lifekit.db" || -e "$_t12_inst/service/lifekit" ]]; then
    bad "T12: lifekit-named artifacts created for a fookit kit (hardcoding residue)"
  else
    ok "T12: no lifekit-named artifacts (name fully derived)"
  fi
  if printf '%s' "$_t12_out" | grep -q "no versioned kit core, skipping"; then
    bad "T12: migrate skipped as 'no versioned kit core' (fookit.py not recognized)"
  else
    ok "T12: migrate recognized database/fookit.py (no versioned-core skip)"
  fi
  if printf '%s' "$_t12_out" | grep -q "migrate: DB user_version=1 == code pin=1"; then
    ok "T12: migrate verified fookit.db against fookit.py pin (in sync)"
  else
    bad "T12: migrate in-sync line missing from install log"
  fi
  if [[ -f "$_t12_inst/config/fookit.conf" ]] && grep -q "^KEY_NEW=val_new" "$_t12_inst/config/fookit.conf"; then
    ok "T12: config/fookit.conf merged"
  else
    bad "T12: config/fookit.conf missing or not merged"
  fi
  if grep -q "DOGANY_PACKS=fookit@1.0.0" "$_t12_inst/.instance.conf" 2>/dev/null; then
    ok "T12: DOGANY_PACKS=fookit@1.0.0 recorded"
  else
    bad "T12: DOGANY_PACKS entry wrong: $(grep '^DOGANY_PACKS=' "$_t12_inst/.instance.conf" 2>/dev/null || echo absent)"
  fi
else
  bad "T12: installer exited non-zero for provides_kit=fookit (rc=$_t12_rc)"
  printf '%s\n' "$_t12_out" | tail -5
fi

rm -rf "$_t12_pack" "$_t12_inst"

# ---------------------------------------------------------------------------
# T13: capabilities.db_lane=false kit installs cleanly (DGN-1003 criterion 5)
# db-init and migrate are capability-SKIPped (loud), install succeeds, no DB
# file is created. Matches the compat-lint C3/C6 capability skip.
# ---------------------------------------------------------------------------
echo "T13: db_lane=false kit (db-init/migrate capability skip)"

_t13_pack="$(mktemp -d)"
_t13_inst="$(mktemp -d)"
_mk_pack "$_t13_pack" "1.0.0" "1" "nodbkit"
_mk_instance "$_t13_inst"
# No DB lane: drop the database payload entirely and declare the capability.
rm -rf "$_t13_pack/payload/database"
python3 - "$_t13_pack/pack-manifest.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
d["capabilities"] = {"db_lane": False}
json.dump(d, open(p, "w"), indent=2)
PY

_t13_out="$(_run_install_out "$_t13_pack" "$_t13_inst" "nodbkit")"
_t13_rc=$?
if [[ "$_t13_rc" -eq 0 ]]; then
  ok "T13: db_lane=false install exit 0 (no false-FAIL on missing DB lane)"
  if printf '%s' "$_t13_out" | grep -q "db-init: capabilities.db_lane=false -- SKIP"; then
    ok "T13: db-init capability-SKIP line present (loud, not silent)"
  else
    bad "T13: db-init capability-SKIP line missing"
  fi
  if printf '%s' "$_t13_out" | grep -q "migrate: capabilities.db_lane=false -- SKIP"; then
    ok "T13: migrate capability-SKIP line present (loud, not silent)"
  else
    bad "T13: migrate capability-SKIP line missing"
  fi
  if compgen -G "$_t13_inst/database/*.db" >/dev/null 2>&1; then
    bad "T13: a DB file was created despite db_lane=false"
  else
    ok "T13: no DB file created (no half-install)"
  fi
  if [[ -f "$_t13_inst/service/nodbkit/__init__.py" ]]; then
    ok "T13: non-DB surfaces (service facade) still installed"
  else
    bad "T13: service/nodbkit/ missing (skip leaked beyond the DB lane)"
  fi
else
  bad "T13: db_lane=false install exited non-zero (rc=$_t13_rc)"
  printf '%s\n' "$_t13_out" | tail -5
fi

rm -rf "$_t13_pack" "$_t13_inst"

# ---------------------------------------------------------------------------
# T14: db_lane=false + database/ files shipped = FATAL (skip-smuggling,
# lockstep with compat-lint C3). The installer refuses on its OWN check --
# the FATAL line is the installer's, proving it does not merely trust the
# compat-lint gate.
# ---------------------------------------------------------------------------
echo "T14: db_lane=false contradiction (database/ files shipped) FATAL"

_t14_pack="$(mktemp -d)"
_t14_inst="$(mktemp -d)"
_mk_pack "$_t14_pack" "1.0.0" "1" "nodbkit"
_mk_instance "$_t14_inst"
python3 - "$_t14_pack/pack-manifest.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
d["capabilities"] = {"db_lane": False}
json.dump(d, open(p, "w"), indent=2)
PY
# database/ payload intentionally KEPT (contradiction).

_t14_out="$(_run_install_out "$_t14_pack" "$_t14_inst" "nodbkit")"
_t14_rc=$?
if [[ "$_t14_rc" -ne 0 ]]; then
  ok "T14: contradiction refused (installer exit non-zero)"
else
  bad "T14: installer exit 0 despite db_lane=false + shipped database/ files"
fi
if printf '%s' "$_t14_out" | grep -q "skip-smuggling refused; lockstep with compat-lint C3"; then
  ok "T14: refusal is the installer's OWN check (not just the lint gate)"
else
  bad "T14: installer-side skip-smuggling FATAL line missing"
fi

rm -rf "$_t14_pack" "$_t14_inst"

# ---------------------------------------------------------------------------
# T15: poisoned provides_kit fail-closed (DGN-1003 criterion 6 / grill:
# write-outside-instance-root). Traversal and reserved names must die in the
# INSTALLER's own validation, before any filesystem write.
# ---------------------------------------------------------------------------
echo "T15: poisoned provides_kit fail-closed (traversal / reserved names)"

_t15_root="$(mktemp -d)"
_t15_pack="$_t15_root/pack"
_t15_inst="$_t15_root/inst"
mkdir -p "$_t15_pack"
_mk_pack "$_t15_pack" "1.0.0" "1" "fookit"
_mk_instance "$_t15_inst"
# Poison provides_kit with a traversal value AFTER building a valid shape.
python3 - "$_t15_pack/pack-manifest.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
d["provides_kit"] = "../../escape"
json.dump(d, open(p, "w"), indent=2)
PY

_t15_out="$(_run_install_out "$_t15_pack" "$_t15_inst" "fookit")"
_t15_rc=$?
if [[ "$_t15_rc" -ne 0 ]]; then
  ok "T15: traversal provides_kit refused (exit non-zero)"
else
  bad "T15: traversal provides_kit ACCEPTED (path injection open)"
fi
if printf '%s' "$_t15_out" | grep -q "kit: provides_kit invalid"; then
  ok "T15: refusal is the installer's own validation (before the lint gate)"
else
  bad "T15: installer-side provides_kit FATAL line missing"
fi
if compgen -G "$_t15_root/escape*" >/dev/null 2>&1 \
   || compgen -G "$(dirname "$_t15_root")/escape*" >/dev/null 2>&1; then
  bad "T15: files written OUTSIDE the instance root (escape* artifact exists)"
else
  ok "T15: nothing written outside the instance root"
fi
# Reserved name: 'database' would alias a framework surface.
python3 - "$_t15_pack/pack-manifest.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
d["provides_kit"] = "database"
json.dump(d, open(p, "w"), indent=2)
PY
if _run_install "$_t15_pack" "$_t15_inst" "fookit"; then
  bad "T15: reserved name 'database' ACCEPTED"
else
  ok "T15: reserved name 'database' refused"
fi

rm -rf "$_t15_root"

# ---------------------------------------------------------------------------
# TL: kit-name contract LOCKSTEP matrix (DGN-1003 criterion 6)
# The name derivation/validation contract lives in TWO places by design
# constraint (compat-lint.sh is DGN-1002-frozen; the installer must validate
# independently because the lint gate is skippable). This matrix runs BOTH
# scripts over the same manifests and fails if their verdicts ever diverge --
# the drift alarm that replaces code sharing.
# ---------------------------------------------------------------------------
echo "TL: compat-lint <-> pack_install kit-name lockstep matrix"

COMPAT_LINT="$SCRIPT_DIR/../pack/compat-lint.sh"
_FWV="$(tr -d '[:space:]' < "$REPO_ROOT/VERSION")"

# _tl_case <label> <provides_kit_json|ABSENT> <fs_name>
#   Builds a valid kit pack shaped for <fs_name> (files named after it), sets
#   provides_kit to the RAW JSON value given (or removes the key for ABSENT),
#   then runs compat-lint --install-side and pack_install --dry-run.
#   Asserts: verdict agreement. For accept cases, also asserts both resolved
#   the same kit name.
_tl_case() {
  local label="$1" pk_json="$2" fs_name="$3"
  local dir inst cat_f
  dir="$(mktemp -d)"; inst="$(mktemp -d)"
  _mk_pack "$dir" "1.0.0" "1" "$fs_name"
  _mk_instance "$inst"
  # Per-case catalog: row id must match the manifest id (D-D gate).
  cat_f="$dir/catalog.json"
  python3 - "$dir/pack-manifest.json" "$cat_f" "$pk_json" <<'PY'
import json, sys
mp, cp, pk_raw = sys.argv[1], sys.argv[2], sys.argv[3]
d = json.load(open(mp))
if pk_raw == "ABSENT":
    d.pop("provides_kit", None)
else:
    d["provides_kit"] = json.loads(pk_raw)
json.dump(d, open(mp, "w"), indent=2)
json.dump({"version": 1, "packs": [
    {"id": d["id"], "kind": "kit", "package_dir": "unused", "status": "published"}
]}, open(cp, "w"), indent=2)
PY
  local lint_out lint_rc inst_out inst_rc
  lint_out="$(bash "$COMPAT_LINT" --pack-dir "$dir" --framework-version "$_FWV" \
      --catalog "$cat_f" --install-side 2>&1)"; lint_rc=$?
  inst_out="$(bash "$INSTALLER" "test-slug" "$inst" --pack "$fs_name" \
      --pack-dir "$dir" --catalog "$cat_f" --no-start --no-state --dry-run 2>&1)"; inst_rc=$?
  local lint_v=accept inst_v=accept
  [[ "$lint_rc" -ne 0 ]] && lint_v=reject
  [[ "$inst_rc" -ne 0 ]] && inst_v=reject
  if [[ "$lint_v" == "$inst_v" ]]; then
    ok "TL[$label]: lockstep verdict agree ($lint_v)"
  else
    bad "TL[$label]: LOCKSTEP BROKEN -- compat-lint=$lint_v pack_install=$inst_v"
    printf '%s\n' "$inst_out" | tail -3
  fi
  if [[ "$lint_v" == "accept" && "$inst_v" == "accept" ]]; then
    local lint_name inst_name
    lint_name="$(printf '%s\n' "$lint_out" | sed -n 's/.*kit name resolved: \([^ ]*\).*/\1/p' | head -1)"
    inst_name="$(printf '%s\n' "$inst_out" | sed -n 's/.*kit: kit-name: \([^ ]*\).*/\1/p' | head -1)"
    if [[ -n "$lint_name" && "$lint_name" == "$inst_name" ]]; then
      ok "TL[$label]: resolved name identical ($lint_name)"
    else
      bad "TL[$label]: resolved name diverged (lint='$lint_name' install='$inst_name')"
    fi
  fi
  rm -rf "$dir" "$inst"
}

# Accept cases (well-formed names; pack files shaped to match).
_tl_case "valid-fookit"   '"fookit"'                              "fookit"
_tl_case "valid-1char"    '"a"'                                   "a"
_tl_case "valid-mixed"    '"kit_2-x"'                             "kit_2-x"
_tl_case "valid-32char"   '"abcdefghijklmnopqrstuvwxyz012345"'    "abcdefghijklmnopqrstuvwxyz012345"
_tl_case "absent-legacy"  'ABSENT'                                "lifekit"

# Reject cases (invalid values; fs shape irrelevant -- validation dies first).
_tl_case "empty"          '""'          "fookit"
_tl_case "uppercase"      '"Fookit"'    "fookit"
_tl_case "leading-digit"  '"1kit"'      "fookit"
_tl_case "slash"          '"foo/kit"'   "fookit"
_tl_case "space"          '"foo kit"'   "fookit"
_tl_case "dotdot"         '".."'        "fookit"
_tl_case "glob"           '"foo*"'      "fookit"
_tl_case "33char"         '"abcdefghijklmnopqrstuvwxyz0123456"' "fookit"
_tl_case "newline-embed"  '"evil\nfookit"' "fookit"
_tl_case "nonstring-bool" 'true'        "fookit"
_tl_case "nonstring-num"  '123'         "fookit"
_tl_case "nonstring-null" 'null'        "fookit"
for _rsv in bridge memory-engine scripts routines service config database mirror skills-bundle payload; do
  _tl_case "reserved-$_rsv" "\"$_rsv\"" "fookit"
done

# Unparseable manifest (DGN-1004 lockstep): compat-lint fail-closes; the
# installer must reject too (its manifest reads die loudly), never fall
# through to a default-name install.
_tlu_dir="$(mktemp -d)"; _tlu_inst="$(mktemp -d)"
_mk_pack "$_tlu_dir" "1.0.0" "1" "fookit"
_mk_instance "$_tlu_inst"
printf '{ "id": "fookit", "kind": "kit", BROKEN\n' > "$_tlu_dir/pack-manifest.json"
_tlu_lint_rc=0
bash "$COMPAT_LINT" --pack-dir "$_tlu_dir" --framework-version "$_FWV" \
  --catalog "$FIXTURE_CATALOG_DIR/catalog.json" --install-side >/dev/null 2>&1 || _tlu_lint_rc=$?
_tlu_inst_rc=0
bash "$INSTALLER" "test-slug" "$_tlu_inst" --pack "fookit" --pack-dir "$_tlu_dir" \
  --catalog "$FIXTURE_CATALOG_DIR/catalog.json" --no-start --no-state --dry-run >/dev/null 2>&1 || _tlu_inst_rc=$?
if [[ "$_tlu_lint_rc" -ne 0 && "$_tlu_inst_rc" -ne 0 ]]; then
  ok "TL[unparseable-manifest]: lockstep verdict agree (reject)"
else
  bad "TL[unparseable-manifest]: LOCKSTEP BROKEN -- lint rc=$_tlu_lint_rc install rc=$_tlu_inst_rc (both must reject)"
fi
rm -rf "$_tlu_dir" "$_tlu_inst"
