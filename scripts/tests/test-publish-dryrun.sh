...
# --- unit counts -------------------------------------------------------------
# DGN-803 LS-5: the 8 lifekit bundle-skill units (incl. spending-log, the only
# private/poc unit) left the tree with the lifekit extraction -> 22 - 8 = 14
# units, all public+official.
grep -q "product.yaml units found: 14" "$LOG"; check "14 units read (post lifekit extraction)" $?
grep -q "public+official units selected: 14" "$LOG"; check "14 public+official selected" $?

# --- extracted lifekit content must not be exported --------------------------
# spending-log (and every other lifekit bundle skill) now lives in the lifekit
# pack repo; its privacy handling moved to the pack gates (compat-lint C5).
# The export list must carry no extracted lifekit path. (Repo-ROOT paths only:
# scripts/tests/fixtures/ contains lifekit-shaped test stubs that legitimately
# export with the framework unit.)
sed -n '/## Export list/,/## F9-BLOCKED/p' "$REPORT" | grep -q 'skills-bundle/spending-log'
check "spending-log NOT in export list (extracted to the lifekit pack)" $([ $? -ne 0 ] && echo 0 || echo 1)
sed -n '/## Export list/,/## F9-BLOCKED/p' "$REPORT" | grep -qE '^- (database/(lifekit|schema)|service/lifekit/)'
check "no extracted lifekit root path in export list" $([ $? -ne 0 ] && echo 0 || echo 1)
...
grep -q 'files:.*repo-meta=16' "$LOG" 2>/dev/null || grep -q 'repo-meta injected     : 16' "$LOG"
check "repo-meta count = 16 (6 root + 8 README images + 2 install-shared)" $?
...
# rules/RULES.md (template symlink target) + rules/USER.example.md by name;
# neither is unit-owned, so the strict allowlist drops them and the minted
# instance breaks (no RULES.md). These must be injected (enumerated, NOT a
# rules/ glob). DGN-803 LS-5: the database/* entries left the list with the
# lifekit extraction (kit content ships via pack_install, not mint/publish).
for f in rules/RULES.md rules/USER.example.md; do
  printf '%s\n' "$META_SECTION" | grep -qxF -- "- $f"
  check "install-shared injected: $f" $?
done
# backdoor guard: injection must NOT pull any database/ path anymore (the
# whole subtree is kit territory post-extraction; only restore-data.sh remains
# in-tree and no installer path injects it).
printf '%s\n' "$META_SECTION" | grep -qE '^- database/'
check "install-shared did NOT smuggle database/ paths (no backdoor)" $([ $? -ne 0 ] && echo 0 || echo 1)
