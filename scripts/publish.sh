...
SWEEP="${DOGANY_SWEEP_SCRIPT:-$HOME/dogany/dev-crew/agents/metal/scripts/secret-sweep.sh}"
...
# by name, no more. NOT a glob over rules/ or database/ (that would ship the
# migrations/ + tests/ dev surface); the enumeration keeps default-deny intact.
# It rides the SAME inject_meta belt as REPO_META (must be git-tracked, must
# survive F9 / third-party), so injection can never smuggle a blocked path.
# The list mirrors mint.sh 1c (shared-root bundle) + the template symlink
# target; if mint.sh's bundle list changes, this list must track it.
...
database/lifekit.sh
database/README.md
database/remind_select.py
database/routine_roller.py
database/routine_projection.py
database/relmod.py
database/relmod.sh
'
