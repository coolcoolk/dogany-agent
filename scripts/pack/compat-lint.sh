...
# ---------------------------------------------------------------------------
# CHECK 4b: payload hygiene -- backup/archive residue
# FIX-2: C5 token-based gates miss token-free packaging artifacts.
# Block regardless of token content: *.bak, *.bak.*, *.bak-*, *~, *.orig,
# *.swp; any path component named _archive.
# ---------------------------------------------------------------------------
_info "C4b -- payload hygiene (backup/archive residue)"
...
if [[ -d "$PAYLOAD_DIR" ]]; then
  while IFS= read -r fpath; do
    ...
    # Path component: _archive anywhere in the relative path
    if [[ -z "$violation" ]]; then
      case "/$rel/" in
        */_archive/*)
...
if [[ "$C4B_FAIL" -eq 0 ]]; then
  _pass "C4b: payload hygiene clean (no backup/archive residue)"
fi
