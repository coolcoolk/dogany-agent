...
#   C-RELNOTE  release-note authorship (DGN-1147 A, publish-side only): the
#       pack ROOT release notes (CHANGELOG.md / RELEASES.md / RELEASE-NOTES.md
#       / releases/*.md) must state the reason in the author's own sentence,
#       not hand it off to a quoted owner utterance. This is the first slot
#       that judges repo-ROOT PROSE: C4/C4b are payload-scoped and C5 walks
#       $PACK_DIR but judges file names. Predicate + measured false-positive
#       calibration live in lib/relnote_authorship.sh.
...
#   - KIT_NAME derives from the manifest 'provides_kit' field. Key ABSENT on
#     kind=kit => C1 FAIL + abort (DGN-1143: the old legacy default "lifekit"
#     was REMOVED -- measured population of manifests relying on it is ZERO
#     [every published kit tag since pack/lifekit/v1.0.0 declares the field],
#     and after the dec-145 kit rename a silent default would resolve to a
#     kit that no longer exists). Key PRESENT => value must match
#     ^[a-z][a-z0-9_-]{0,31}$ and must not be a framework-reserved name;
#     anything else (empty, traversal, glob chars, non-string) => C1 FAIL +
#     abort (fail closed -- a poisoned name must never drive the C3/C4/C6
#     path rules).
...
#     manifests (removed by DGN-1143 -- see above), so kind=pack gets NO kit
#     identity (KIT_NAME empty; C3/C6
...
# framework-reserved name list), shared with pack_install.sh's independent
...
# DGN-1045 follow-up: payload-seeded predicate, shared with pack_publish.sh's
# finalize G-F1 gate (the two used to answer "is this payload seeded" with two
# different rules -- see the lib file header).
PAYLOAD_SEEDED_LIB="$SCRIPT_DIR/lib/payload_seeded.sh"
[[ -f "$PAYLOAD_SEEDED_LIB" && -r "$PAYLOAD_SEEDED_LIB" ]] \
  || { echo "ERROR: payload-seeded library missing ($PAYLOAD_SEEDED_LIB) -- fail-closed" >&2; exit 1; }
# shellcheck source=lib/payload_seeded.sh
. "$PAYLOAD_SEEDED_LIB"
...
# Semver comparator single source (DGN-1031 residual debt 1): the grammar +
# satisfaction algorithm lives in lib/semver_range.py. The two functions
# below are thin wrappers around its CLI -- the exit-code and stderr
# contracts are identical to the old inline heredocs (equivalence-verified,
# scripts/tests/test-semver-range.sh). Fail-closed by construction: a
# missing module makes python3 exit 2 ("can't open file") and a broken one
# exit nonzero, which both call sites already treat as FAIL lanes -- module
# absence can never read as "check passed".
_SEMVER_LIB="$SCRIPT_DIR/lib/semver_range.py"

# _semver_satisfies <version> <range>
# Supports: >=X.Y.Z <A.B.C (space-separated ANDs)
# Returns 0 if satisfied, 1 if not, 2 on parse/grammar error.
# Only implements the subset used by dogany pack manifests.
_semver_satisfies() {
  python3 "$_SEMVER_LIB" satisfies "$1" "$2"
}

# _is_semver_range <string>
# Returns 0 if the string is a valid semver range (basic token set), 1 if not.
_is_semver_range() {
  python3 "$_SEMVER_LIB" is-range "$1"
}
...
# Regex core + reserved list are DGN-1018 RR2 shared data (see
# lib/pack_token_grammar.sh) -- the anchors here stay LOCAL (bash form).
_KIT_TOKEN_RE='^'"${PACK_TOKEN_CORE}"'$'
_RESERVED_TOKENS=("${PACK_RESERVED_TOKENS[@]}")
...
# C6 database/<kit>.sh):
#   - 'provides_kit' key ABSENT on kind=kit -> C1 FAIL + ABORT (DGN-1143).
#     The pre-DGN-1002 legacy default "lifekit" is REMOVED: the population
#     relying on it was measured at ZERO (all published kit tags declare the
#     field), and after the dec-145 rename (lifekit -> kit_life) the default
#     would silently point every path rule at a nonexistent kit.
...
# DGN-1143: the default itself is now GONE -- kind=kit MUST declare
# provides_kit (absence = C1 FAIL + ABORT, same fail-closed direction as an
# invalid value: an unresolvable kit identity must never drive C3/C4/C6).
KIT_NAME=""
...
if [[ "$PACK_CLASS" == "pack" ]]; then
  ...
  if [[ "$_pk_present" -eq 0 ]]; then
    _fail "C1: kind=kit manifest missing provides_kit -- the legacy 'lifekit' default was removed (DGN-1143: zero published manifests relied on it, and after the dec-145 kit rename it would silently resolve to a nonexistent kit). Declare provides_kit explicitly."
    echo "[compat-lint] ABORT: kit identity unresolvable, remaining checks skipped (no kit name must ever drive path allowlists by default)" >&2
    exit 1
  fi
  ...
  _info "C1: kit name resolved: $KIT_NAME (provides_kit declared)"
fi
...
elif d['service_namespace'] != d['service_namespace'].rstrip('\n'):
    # Trailing-newline trapdoor: this probe's value is captured through \$( ),
    # which STRIPS trailing newlines, so 'bns\n' would reach [[ =~ ]] as 'bns'
    # and pass a grammar the raw value does not satisfy. The pack_install twin
    # validates the raw JSON string with a \Z anchor and refuses it -- report
    # the damage here so both sides stay verdict-identical (DGN-1018 5-B class).
    print('unsafe: %r' % (d['service_namespace'],))
else:
...
if [[ "$_ns_probe" != "absent" ]]; then
  ...
  elif [[ "$_ns_probe" == unsafe:* ]]; then
    _fail "C1: service_namespace invalid: ${_ns_probe#unsafe: } -- must match ^[a-z][a-z0-9_-]{0,31}\$ and not be a framework-reserved name"
  else
...
if rk['kit'] != rk['kit'].rstrip('\n'):
    # Same trailing-newline trapdoor as the _ns_probe above: \$( ) capture
    # would strip it and hand [[ =~ ]] a value the manifest does not contain.
    print('kitunsafe: %r' % (rk['kit'],)); sys.exit(0)
...
if [[ "$_rk_probe" == "absent" ]]; then
  ...
  case "$_rk_probe" in
    ...
    kitnonstring:*)
      ...
      ;;
    kitunsafe:*)
      _fail "C-KITDEP: requires_kit.kit invalid: ${_rk_probe#kitunsafe: } -- must match ^[a-z][a-z0-9_-]{0,31}\$ and not be a framework-reserved name"
      ;;
    rangenonstring:*)
...
# The rule itself lives in lib/payload_seeded.sh -- shared with the publisher's
# finalize G-F1 gate so the two sides cannot answer it differently.
_payload_seeded() {
...
#                "winner": "<pack-id>"   (OPTIONAL, DGN-1143)} ]
# "winner" declares the agreed canonical shipper for a skill name that is
# shipped by MORE THAN ONE pack (cross-pack collision, DGN-1143): the
# collision gate (lib/skill_collision_check.sh, run by pack_publish gate (e)
# and pack_install) passes a collision only when EVERY shipping pack declares
# the SAME winner. Here C7 validates only the local form (value grammar);
# cross-pack agreement needs the pack universe, which is not this script's
# single-pack interface (C-KITDEP precedent).
# Fail-closed rules:
#   - sharing_mode must be exactly "share" or "own"
#   - winner, when present, must be a non-empty pack-id-shaped string
#     (^[a-z][a-z0-9_-]{0,31}$)
...
for e in d.get("skills") or []:
    ...
    if "winner" in e:
        import re as _re
        w = e.get("winner")
        if not (isinstance(w, str) and _re.match(r'^[a-z][a-z0-9_-]{0,31}$', w)):
            print("skill '%s' has invalid winner=%r -- must be a pack-id-shaped string ^[a-z][a-z0-9_-]{0,31}$ (DGN-1143)" % (n, w))
...
# eliminate drift vs pack_publish.sh. DGN-1049: pack_publish.sh still carries
# the narrower pre-fix EXCLUDE_SUFFIXES=( ".db" ".sqlite" ".sqlite3" ) case-
# sensitive exact-suffix list -- same drift risk this TODO already flagged,
# now also a coverage gap (see the gate(a) case-pattern below). Flagged, not
# fixed here: out of this ticket's scope (compat-lint.sh only).
...
#
# DGN-1049 rider (grill finding, verified): the plain-suffix match below
# ("$base" == *"$sfx", case-sensitive, exact suffix list) is the ONLY thing
# in this script that blocks a shipped SQLite artifact outside
# payload/database/ (C4's payload allowlist is path-based only -- it happily
# passes payload/service/<ns>/data.db for kind=pack, see C4 comments above).
# Two verified bypasses of the ORIGINAL suffix list closed here:
#   1. case: "DATA.DB" / "Data.Db" -- bash [[ == ]] is case-sensitive, so an
#      uppercase/mixed-case extension slipped through untouched (confirmed:
#      a fixture with payload/service/foo/DATA.DB passed C5 clean).
#   2. WAL-mode sidecars shipped WITHOUT the main file: "*.db-wal"/"*.db-shm"
#      (and the rollback-journal "*.db-journal") were not in the suffix list
#      at all -- a pack shipping only those (skipping the plain .db) passed
#      clean (confirmed empirically). A live WAL-mode SQLite install can run
#      off the sidecars once a checkpoint lands them into place, so this
#      isn't just cosmetic residue.
# Fix: match on the LOWERCASED basename (portable tr, this script targets
# bash 3.2 / macOS default -- no ${var,,}) against a glob set that folds
# .db/.sqlite/.sqlite3 in with their -wal/-shm/-journal sidecars, plus a
# bare "*.sqlite*" catch-all per the DGN-1049 rider ("consider *.sqlite*").
# This is NOT a new check -- gate(a) already blocked the base "ship a .db
# file" case pre-fix (verified: FAIL C5 gate(a) on a plain
# payload/service/<ns>/data.db fixture); this closes the two case/sidecar
# gaps found while proving that, so a redundant path-based C4 rule for the
# base case was not added.
gate_a_fail=""
while IFS= read -r f; do
  ...
  case "$base_lc" in
    *.db|*.db-wal|*.db-shm|*.db-journal|*.sqlite*)
      gate_a_fail+="  real-data file: ${f#"$PACK_DIR/"}"$'\n'
      ;;
  esac
...
# ---------------------------------------------------------------------------
# CHECK C-RELNOTE: release-note authorship (named slot, DGN-1147 A)
#
# THE RULE (packs/CONTRACT.md §6-1c): 릴리스 노트는 바뀐 것과 그 이유를
# 작성자 자기 문장으로 쓴다. The gate's mechanical floor under that rule: a
# release note must not hand the reason off to a QUOTED OWNER UTTERANCE.
#
# SCOPE -- the repo ROOT, which no existing slot reached. C4/C4b are
# $PAYLOAD_DIR-scoped; C5's file walk covers $PACK_DIR but judges file NAMES,
# not prose; pack_publish gate (s) runs the estate-scrub judge over REF_DIR
# and its own header names the hole -- "Known boundary: seal artifacts at the
# package root (CHANGELOG.md, pack-manifest.json) are outside REF_DIR and not
# scanned by this gate" (pack_publish.sh:715-717). The pack CHANGELOG is the
# file a stranger reads, and
# nothing was reading it. Measured 2026-08-28: health-trainer's root
# CHANGELOG.md:126 shipped a cited owner utterance, tracked and published.
#
# PREDICATE: scripts/pack/lib/relnote_authorship.sh -- see that file's header
# for the two rejected predicates, the measured false-positive rates (103 hits
# for "any quoted Korean", ~50% FP for "attribution + any quote"), and why the
# adopted one keys on a quoted UTTERANCE (a sentence) rather than a quoted TERM
# (a noun phrase).
#
# SIDE: publish-side only, by the D-D precedent (a slot may be side-scoped when
# the other side cannot act on the verdict).
#   - publish is where the defect can still be PREVENTED: gate (d) runs this
#     linter against $PACKAGE_DIR *after* STEP 4 has written CHANGELOG.md
#     (pack_publish.sh:667 then :812), so an offending seal artifact is refused
#     before it is ever published.
#   - install-side would refuse an ALREADY-PUBLISHED pack for a defect its
#     installer cannot fix, and would newly brick updates from historical tags
#     whose CHANGELOG still carries the pre-disposal text (the DGN-1147
#     evidence line was live in the working tree on 2026-08-28 and disposed
#     mid-day -- historical tags were NOT measured and may still carry it).
#     Widening to install-side is an owner call with a tag-population survey
#     attached, not a silent default.
#
# SEVERITY: FAIL-CLOSED, no WARN lane -- and that is affordable because it was
# COUNTED FIRST. Live population 2026-08-28, all three published pack roots
# (lifekit / health-trainer / dev): 0 hits. Fail-closed here breaks nothing
# that exists. (Had the count been high, this would have landed as WARN with
# the count recorded; it was zero, so it lands as the gate CONTRACT §6-1a asks
# publish-side gates to be.)
#
# INSTRUMENT LIVENESS: the predicate is multibyte and goes BLIND under
# LC_ALL=C (measured: 1 hit -> 0 hits on the evidence line). A silent 0 is the
# failure mode this estate keeps hitting, so the locale is not assumed: the
# library picks the first locale in which the pattern matches its positive
# canary AND rejects its negative canary. No working locale => FAIL, never a
# clean scan. "0을 봤다" and "볼 수 없었다" cannot print the same thing here.
# ---------------------------------------------------------------------------
_info "C-RELNOTE -- release-note authorship (root release notes, DGN-1147)"

if [[ "$INSTALL_SIDE" -eq 1 ]]; then
  _skip "C-RELNOTE: install-side -- authorship is a publish-time defect the installer cannot fix (publish-side gate (d) is the enforcement point)"
else
  RELNOTE_AUTHORSHIP_LIB="$SCRIPT_DIR/lib/relnote_authorship.sh"
  if [[ ! -f "$RELNOTE_AUTHORSHIP_LIB" || ! -r "$RELNOTE_AUTHORSHIP_LIB" ]]; then
    _fail "C-RELNOTE: predicate library missing ($RELNOTE_AUTHORSHIP_LIB) -- fail-closed"
  else
    # shellcheck source=lib/relnote_authorship.sh
    . "$RELNOTE_AUTHORSHIP_LIB"

    _relnote_loc=""
    if ! _relnote_loc="$(relnote_authorship_locale)"; then
      _fail "C-RELNOTE: predicate is BLIND in every candidate locale (tried: $RELNOTE_LOCALE_CANDIDATES) -- refusing to report a clean scan from a dead instrument"
    else
      _relnote_surface="$(relnote_authorship_surface "$PACK_DIR" | tr '\n' ' ')"
      if [[ -z "${_relnote_surface// /}" ]]; then
        _skip "C-RELNOTE: pack root ships no release note (looked for: $RELNOTE_ROOT_FILES, $RELNOTE_ROOT_DIRS/*.md) -- nothing to judge"
      else
        _relnote_rows="$(relnote_authorship_scan "$PACK_DIR" "$_relnote_loc")"
        _relnote_hits="$(printf '%s\n' "$_relnote_rows" | grep -c '^HIT|' || true)"
        _relnote_allow="$(printf '%s\n' "$_relnote_rows" | grep -c '^ALLOW|' || true)"

        # A waiver is never silent: every waived line is printed, always.
        if [[ "${_relnote_allow:-0}" -gt 0 ]]; then
          echo "[compat-lint] C-RELNOTE: line waivers in effect (estate-scrub:allow):"
          printf '%s\n' "$_relnote_rows" | grep '^ALLOW|' \
            | awk -F'|' '{ out=$4; for (i=5;i<=NF;i++) out=out "|" $i;
                           printf "    ALLOWED %s:%s: %s\n", $2, $3, substr(out,1,200) }'
        fi

        if [[ "${_relnote_hits:-0}" -gt 0 ]]; then
          echo "[compat-lint] FAIL: C-RELNOTE: release note cites an owner utterance instead of the author's own sentence:" >&2
          printf '%s\n' "$_relnote_rows" | grep '^HIT|' \
            | awk -F'|' '{ out=$4; for (i=5;i<=NF;i++) out=out "|" $i;
                           printf "    %s:%s: %s\n", $2, $3, substr(out,1,200) }' >&2
          echo "    RULE: 릴리스 노트는 바뀐 것과 그 이유를 작성자 자기 문장으로 쓴다 (packs/CONTRACT.md §6-1c)." >&2
          echo "    이 파일은 남이 읽는다. 이유를 네 문장으로 다시 써라 -- 인용을 지우는 게 아니라, 이유를 소유해라." >&2
          _fail "C-RELNOTE: ${_relnote_hits} cited owner utterance(s) in root release note(s)"
        else
          _pass "C-RELNOTE: release-note authorship clean (scanned: ${_relnote_surface%% }; locale='${_relnote_loc:-inherit}', canary-verified; ${_relnote_allow:-0} waiver(s))"
        fi
      fi
    fi
  fi
...
if [[ "$DD_FAIL" -eq 1 ]]; then
  ...
  if [[ ! -f "$KIT_SH" ]]; then
    ...
    # DGN-1036: the smoke must not contaminate the payload it lints.
    # Executing the kit CLI compiles imported sibling python modules into
    # payload/database/__pycache__/*.pyc -- which the NEXT run's C4b then
    # reports as packaging residue (the linter contaminating its own
    # subject, false FAIL on re-run, dirty pack working trees).
    # PYTHONDONTWRITEBYTECODE=1 blocks bytecode-cache creation at the
    # source: the env var propagates through the <kit>.sh shell wrapper to
    # the python child (verified against lifekit.sh, which exec's python3
    # without env sanitization). The smoke still executes for real -- only
    # the cache side effect is suppressed. Post-hoc cleanup was rejected
    # (ticket): a missed cleanup leaves silent contamination and the
    # "which files are mine" discrimination logic is a new defect surface.
    # Run check verb
    if ! PYTHONDONTWRITEBYTECODE=1 "$KIT_SH" check >/dev/null 2>&1; then
      _fail "C6: $KIT_NAME.sh check returned non-zero exit code"
      C6_FAIL=1
    fi
    # Run dump verb
    if ! PYTHONDONTWRITEBYTECODE=1 "$KIT_SH" dump >/dev/null 2>&1; then
