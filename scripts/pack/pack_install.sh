...
# ---------------------------------------------------------------------------
# KIT-CLASS INSTALL PATH (DGN-803 LS-4)
# Dispatches early when kind=kit; handles the kit payload layout (no
# reference_slug / categories contract -- uses payload_root directly).
...
if [[ "$PACK_KIND" == "kit" ]]; then
  ...
  # ---- preflight ----
  [[ -n "$KIT_PROVIDES" ]] || _fail "kit manifest missing 'provides_kit' field"
  ...
  if [[ -f "$_COMPAT_LINT" ]]; then
    ...
    if [[ -z "$_FW_VERSION" ]]; then
      _fail "kit: compat-lint: cannot determine framework VERSION"
    fi
    _log "kit: compat-lint: install-side gate (framework=$_FW_VERSION)"
    _lint_out=""
    if ! _lint_out="$(bash "$_COMPAT_LINT" \
      ...
      _fail "kit: compat-lint install-side gate FAILED -- install aborted"
    fi
    while IFS= read -r _ll; do _log "  $_ll"; done <<< "$_lint_out"
    _log "kit: compat-lint PASS"
  else
    _log "kit: compat-lint WARN -- compat-lint.sh not found at $_COMPAT_LINT (gate skipped)"
  fi

  # ---- reverse-drift guard: kit_core (DGN-803 LS-4) ----
  # If the instance already has a lifekit.py with a HIGHER pin than the payload,
  ...
  KIT_CORE_SKIP=0
  PAYLOAD_LIFEKIT_PY="$KIT_PAYLOAD_DIR/database/lifekit.py"
  INST_LIFEKIT_PY="$ROOT/database/lifekit.py"
  if [[ -f "$PAYLOAD_LIFEKIT_PY" && -f "$INST_LIFEKIT_PY" ]]; then
    _payload_pin="$(_kit_extract_ver_lifekit_py "$PAYLOAD_LIFEKIT_PY" 2>/dev/null || true)"
    _inst_pin="$(_kit_extract_ver_lifekit_py "$INST_LIFEKIT_PY" 2>/dev/null || true)"
    if [[ "$_payload_pin" =~ ^[0-9]+$ && "$_inst_pin" =~ ^[0-9]+$ ]]; then
      if [[ "$_inst_pin" -gt "$_payload_pin" ]]; then
        KIT_CORE_SKIP=1
        printf '%s\n' "============================================================"
        _log "kit: REVERSE-DRIFT GUARD: instance lifekit.py pin=$_inst_pin > payload pin=$_payload_pin -- kit_core copy SKIPPED (DGN-803 LS-4)"
  ...
  elif [[ -f "$PAYLOAD_LIFEKIT_PY" && ! -f "$INST_LIFEKIT_PY" ]]; then
    _log "kit: reverse-drift guard: instance has no lifekit.py -- fresh install, PROCEED"
  fi

  if [[ "$DRY" -eq 1 ]]; then
    _log "dry-run: kit install plan for $PACK_ID -> $ROOT"
    ...
    echo "[pack_install] DRY-RUN OK (kit) -- no writes"
    exit 0
  fi
  ...
  # ---- structured lane init: schema.sql -> lifekit.db (DGN-803 LS-5) ----
  ...
  # never existed and lifekit broke much later with no breadcrumb).
  _KIT_SCHEMA="$ROOT/database/schema.sql"
  _KIT_DB="$ROOT/database/lifekit.db"
  if [[ -f "$_KIT_DB" ]]; then
    _log "kit: db-init: lifekit.db exists -> keep (idempotent)"
  elif [[ ! -f "$_KIT_SCHEMA" ]]; then
    _log "kit: db-init: no schema.sql at instance database/ -- skipping (payload shipped none)"
  elif ! command -v sqlite3 >/dev/null 2>&1; then
    _fail "kit: db-init: sqlite3 not found -- cannot initialize lifekit.db (DGN-674 F7 hard fail)"
  else
    # Atomic init (DGN-783 B2): load the schema into a tmp DB, then mv into
    # place. A mid-load failure previously left a partial lifekit.db that the
    ...
    if sqlite3 "$_KIT_DB_TMP" < "$_KIT_SCHEMA"; then
      ...
      _log "kit: db-init: initialized lifekit.db from schema.sql (empty structured lane)"
    else
      rm -f "$_KIT_DB_TMP"
      _fail "kit: db-init: schema.sql failed to load -- partial DB discarded, lifekit.db NOT created"
    fi
  fi

  # ---- kit-migrate: forward-only schema migrations (DGN-783 B1) ----
  # Single-owner seam: the kit install step owns lifekit.db migrations.
  ...
  _KIT_INST_PY="$ROOT/database/lifekit.py"
  if [[ ! -f "$_KIT_INST_PY" ]]; then
    _log "kit: migrate: no instance database/lifekit.py -- no versioned kit core, skipping"
  elif [[ ! -f "$_KIT_DB" ]]; then
    _log "kit: migrate: no lifekit.db -- nothing to migrate (db-init owns creation)"
  else
    command -v sqlite3 >/dev/null 2>&1 \
      || _fail "kit: migrate: sqlite3 not found -- cannot verify/advance lifekit.db schema (fail-closed)"
    _target_pin="$(_kit_extract_ver_lifekit_py "$_KIT_INST_PY" 2>/dev/null || true)"
    if [[ ! "$_target_pin" =~ ^[0-9]+$ ]]; then
      if [[ "$KIT_CORE_SKIP" -eq 1 ]]; then
        ...
        _log "kit: migrate: instance lifekit.py pin unparseable and kit_core was SKIPPED -- migrate skipped (state unchanged)"
      else
        _fail "kit: migrate: cannot parse EXPECTED_USER_VERSION from freshly installed lifekit.py -- DB/code sync unverifiable"
      fi
    else
      ...
      if [[ "$_cur_ver" -lt "$_target_pin" ]]; then
        ...
        for _mig in "$_KIT_MIG_DIR"/[0-9][0-9][0-9]_*.sql; do
          ...
          _mbak="$ROOT/database/lifekit.db.v${_mv_now}.bak-$_mts"
          sqlite3 "$_KIT_DB" ".backup '$_mbak'" \
            || _fail "kit: migrate: failed to back up lifekit.db before migration $_mnnn"
          _log "  migrate: backup -> $_mbak"
          sqlite3 "$_KIT_DB" < "$_mig" \
  ...
  # ---- service_facade: payload/service/lifekit/* -> instance/service/lifekit/ ----
  if [[ -d "$KIT_PAYLOAD_DIR/service/lifekit" ]]; then
    _log "kit: step service_facade: service/lifekit/ copy"
    mkdir -p "$ROOT/service/lifekit"
    while IFS= read -r _src; do
      [[ -e "$_src" ]] || continue
      [[ "$(basename "$_src")" == ".gitkeep" ]] && continue
      _rel="${_src#"$KIT_PAYLOAD_DIR/service/lifekit/"}"
      _dst="$ROOT/service/lifekit/$_rel"
      mkdir -p "$(dirname "$_dst")"
      cp -f "$_src" "$_dst"
      _log "  service_facade: service/lifekit/$_rel"
    done < <(find "$KIT_PAYLOAD_DIR/service/lifekit" -type f)
    _log "kit: service_facade done"
  else
    _log "kit: service_facade: no payload/service/lifekit/ -- skipping"
  fi
  ...
  if [[ -d "$KIT_PAYLOAD_DIR/mirror" ]]; then
    ...
    if [[ "$KIT_MIRROR_SKIP" -eq 0 ]]; then
      ...
      printf '# lifekit pack owns mirror/ (DGN-855)\n' > "$ROOT/mirror/.pack-owned"
  ...
  if [[ -d "$KIT_PAYLOAD_DIR/skills-bundle" ]]; then
    _log "kit: step skills-bundle: .claude/skills-bundle/ install"
    ...
    done < <(find "$KIT_PAYLOAD_DIR/skills-bundle" -mindepth 1 -maxdepth 1 -type d)
    _log "kit: skills-bundle done"
  else
    _log "kit: skills-bundle: no payload/skills-bundle/ -- skipping"
  fi

  # ---- routines/bundle: payload/routines/bundle/* -> instance/routines/bundle/ ----
  if [[ -d "$KIT_PAYLOAD_DIR/routines/bundle" ]]; then
    _log "kit: step routines-bundle: routines/bundle/ install"
    ...
    _log "kit: routines-bundle done"
  else
    _log "kit: routines-bundle: no payload/routines/bundle/ -- skipping"
  fi

  # ---- config merge-key: payload/config/{lifekit.conf,i18n*} ----
  # Merge-key: only adds keys absent from the instance config (never clobbers).
  if [[ -d "$KIT_PAYLOAD_DIR/config" ]]; then
    _log "kit: step config-merge: config/ merge-key"
    # lifekit.conf -> config/lifekit.conf
    if [[ -f "$KIT_PAYLOAD_DIR/config/lifekit.conf" ]]; then
      _merge_out="$(_kit_conf_merge "$ROOT/config/lifekit.conf" "$KIT_PAYLOAD_DIR/config/lifekit.conf")"
      _log "  config-merge lifekit.conf: $_merge_out"
    fi
    ...
    _log "kit: config-merge done"
  else
    _log "kit: config-merge: no payload/config/ -- skipping"
  fi
  ...
  if [[ -f "$_DEPS_SH" ]]; then
    _log "kit: step deps-provision (DGN-850)"
    ...
    _log "kit: deps-provision WARN -- pack_deps_provision.sh not found at $_DEPS_SH (skipped; consumers fall back)"
  fi
  ...
  # unit.generic.
  python3 - "$ROOT/.instance.conf" "$MANIFEST" "${KIT_PROVIDES:-$PACK_ID}" <<'PYEOF'
import json, sys, pathlib
...
PYEOF
  ...
  _log "=== kit-install DONE pack=$PACK_ID slug=$SLUG provides_kit=$KIT_PROVIDES ==="
  echo "[pack_install] DONE (kit) -- see $LOG_FILE"
  exit 0
fi

# ---------------------------------------------------------------------------
# END OF KIT-CLASS PATH -- agent/module installs continue below.
# ---------------------------------------------------------------------------
