...
_rk_verdict="$(python3 - "$MANIFEST" "$ROOT/.instance.conf" <<'PYEOF'
...
kit = rk.get("kit")
rng = rk.get("range")
if not isinstance(kit, str) or not re.match(r'^[a-z][a-z0-9_-]{0,31}$', kit):
    out("BLOCK:requires_kit.kit invalid: %r -- must be a string matching "
        "^[a-z][a-z0-9_-]{0,31}$ (provides_kit grammar reuse)" % (kit,))
for rsv in ("bridge", "memory-engine", "scripts", "routines", "service",
            "config", "database", "mirror", "skills-bundle", "payload"):
    if kit == rsv:
...
if [[ "$PACK_KIND" == "kit" ]]; then
  ...
  # legacy default (key absent -> "lifekit"), same value regex, same
...
PYEOF
  ...
  _log "kit: kit-name: $KIT_NAME (provides_kit $( [[ "$_pk_present" -eq 1 ]] && echo declared || echo "absent -> legacy default" )) db_lane=$KIT_DB_LANE"
  ...
  # ---- service_facade: payload/service/<kit>/* -> instance/service/<kit>/ ----
  # DGN-1003: the facade directory derives from KIT_NAME (compat-lint C4
  # allows exactly service/<kit>/ in the payload -- lockstep).
  if [[ -d "$KIT_PAYLOAD_DIR/service/$KIT_NAME" ]]; then
    _log "kit: step service_facade: service/$KIT_NAME/ copy"
    mkdir -p "$ROOT/service/$KIT_NAME"
    while IFS= read -r _src; do
      [[ -e "$_src" ]] || continue
      [[ "$(basename "$_src")" == ".gitkeep" ]] && continue
      _rel="${_src#"$KIT_PAYLOAD_DIR/service/$KIT_NAME/"}"
      _dst="$ROOT/service/$KIT_NAME/$_rel"
      mkdir -p "$(dirname "$_dst")"
      cp -f "$_src" "$_dst"
      _log "  service_facade: service/$KIT_NAME/$_rel"
    done < <(find "$KIT_PAYLOAD_DIR/service/$KIT_NAME" -type f)
    _log "kit: service_facade done"
  else
    _log "kit: service_facade: no payload/service/$KIT_NAME/ -- skipping"
  fi
  ...
  if [[ -d "$KIT_PAYLOAD_DIR/config" ]]; then
    ...
    # <kit>.conf -> config/<kit>.conf
    if [[ -f "$KIT_PAYLOAD_DIR/config/$KIT_NAME.conf" ]]; then
      _merge_out="$(_kit_conf_merge "$ROOT/config/$KIT_NAME.conf" "$KIT_PAYLOAD_DIR/config/$KIT_NAME.conf")"
      _log "  config-merge $KIT_NAME.conf: $_merge_out"
    fi
