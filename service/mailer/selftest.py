...
class FakeRun:
    ...
    def __call__(self, cmd, capture_output=False, text=False, **kw):
        # cmd[0] is an ABSOLUTE path since the DGN-963 PATH rescue landed
        # (_gws_bin() resolves the binary instead of trusting PATH), so match on
        # the basename. Comparing the literal "gws" made this fake reject the
        # real call shape and the whole selftest died on an AssertionError.
        argv = [os.path.basename(cmd[0])] + list(cmd[1:])
        # auth status
        if argv[:3] == ["gws", "auth", "status"]:
        ...
        # gmail send
        if argv[:5] == ["gws", "gmail", "users", "messages", "send"]:
        ...
        raise AssertionError("unexpected gws call: %s" % (argv,))
