...
def _gws_bin() -> str:
    ...
    "dead". It was not dead -- gws lives there and nothing else does. Here the
    damage is a MISDIAGNOSIS: _gws_authed_with_send() catches FileNotFoundError
    and returns False, i.e. "not connected", so a user whose Google account is
    perfectly authed gets routed back through onboarding to fix a problem they
    do not have. PATH first, then the known install locations. Override with
    GWS_BIN.
    """
...
def _gws_authed_with_send():
    ...
    """
    try:
        proc = subprocess.run([_gws_bin(), "auth", "status"],
