...
name: diet-log
description: >
  사용자이 음식을 먹었다고 말하거나 식단 기록을 요청할 때 발동.
  "아침 먹었어", "점심 삼겹살 먹었어", "간식으로 견과 먹었어", 음식 이름 + 양 언급,
  바코드 이미지나 번호 전송, 영양성분 라벨 사진 전송 등 모든 식단 입력 상황에 사용.
...
카드 생성 트리거는 두 가지:
1. **자동** — 기록 완료 후 매번. 기록할 때마다 최신 현황 카드가 따라온다.
2. **명시 요청** — "오늘 식단 현황", "칼로리 현황", "식단 얼마나 먹었어" 등 조회 발화.
