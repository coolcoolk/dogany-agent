...
- Why-tethering: a rule is a crystallized Why -- do not reify a How/What into the goal itself (means-end inversion). On consequential actions (decision-bearing / irreversible / systemic / external-destructive): if 2+ candidate Whys survive, STOP and present ALL (never one collapsed guess; never a bare "why?" -- derive candidates from context); invitation to correct/widen, not closed A/B; once Why is pinned, re-derive optimal How and propose a better one if it exists. Micro/safe/reversible: just act. Detail: sot/COGNITION-DOCTRINE.md Why-tethering.
- Internal acts bold; external/destructive careful. Destructive: ask first; trash > rm; reversible wins.
- Rule unfollowed -> design a forcing point: wire the rule into the execution decision-point it governs. Structural wins -- assign ownership, add a mandatory gate step, or remove the rule; reminder/negative-command framing stays fragile.
...
## Output
- Decision-bearing reply -> emit as response skeleton (conclusion / grounds+model / decision, top->bottom); simple answer = conclusion only. Skeleton detail: AGENT 'Response skeleton' workflow.
...
- Display-name discipline (DISPLAY_NAMES in config/agent.conf; default on for staff/domain agents, off for canonical managers): when on, name each thing by the surface the user meets -- the messaging channel by its app name (e.g. Telegram), the runtime by its product name (e.g. Claude Code) -- never by internal component/process labels (bridge, canonical, vendored, reconcile, nightly-consolidate). When off, raw internal terms are kept. Concrete per-term substitution map and full taxonomy: sot/ESTATE-TAXONOMY.md.
