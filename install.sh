...
#   - Secrets travel to mint.sh via env vars (DOGANY_BOT_TOKEN / DOGANY_EMAIL_PW),
...
# ---------------------------------------------------------------------------
# Step 7 + 8: mint the instance (mint.sh writes the .env; we pass values)
# ---------------------------------------------------------------------------
...
# secrets via the DOGANY_BOT_TOKEN / DOGANY_EMAIL_PW environment variables.
...
step_mint_and_env() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    msg "다음 인자로 호출될 예정: DOGANY_BOT_TOKEN=<token> DOGANY_EMAIL_PW=<hidden> mint.sh --root '$target' --name '$AGENT_NAME' --lang '$DOGANY_LANG' --owner-id '$OWNER_ID' --tz '$DOGANY_TZ' --email '$EMAIL_ADDRESS' --email-cc '$EMAIL_CC' --whisper '$LOCAL_WHISPER_MODEL' --models '$BRIDGE_MODELS' $(mint_dep_flag)" \
        "Would call: DOGANY_BOT_TOKEN=<token> DOGANY_EMAIL_PW=<hidden> mint.sh --root '$target' --name '$AGENT_NAME' --lang '$DOGANY_LANG' --owner-id '$OWNER_ID' --tz '$DOGANY_TZ' --email '$EMAIL_ADDRESS' --email-cc '$EMAIL_CC' --whisper '$LOCAL_WHISPER_MODEL' --models '$BRIDGE_MODELS' $(mint_dep_flag)"
    msg "작성될 .env: $env_file" "Would write .env: $env_file"
    msg "--- .env 미리보기 ---" "--- .env preview ---"
    # Masked preview: the REAL secrets must never reach stdout. Token is shown
    # via mask_token; the app password shows *** when set, blank when unset.
    # Secrets ride env vars into mint.sh --print-env, never argv.
    local pw_mask=""
    if [ -n "$EMAIL_APP_PASSWORD" ]; then pw_mask="***"; fi
    DOGANY_BOT_TOKEN="$(mask_token "$BOT_TOKEN")" DOGANY_EMAIL_PW="$pw_mask" \
    ...
    ( umask 077; \
      DOGANY_BOT_TOKEN="$BOT_TOKEN" DOGANY_EMAIL_PW="$EMAIL_APP_PASSWORD" \
        bash "$MINT_SH" --print-env --lang "$DOGANY_LANG" \
        --owner-id "$OWNER_ID" --tz "$DOGANY_TZ" \
        --email "$EMAIL_ADDRESS" --email-cc "$EMAIL_CC" \
  ...
  # 6/7a) mint the instance (reuses scripts/mint.sh). Secrets travel via env
  # (DOGANY_BOT_TOKEN / DOGANY_EMAIL_PW), never argv. All value flags are
