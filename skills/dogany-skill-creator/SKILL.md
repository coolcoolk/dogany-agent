...
## body-conventions (rule text authoring)
- examples in rule/step text = schematic placeholders ONLY. show the SHAPE of
  the rule, not a real case. use tokens like `<USER_NAME>`, `<PROJECT_X>`,
  `<DATE>`, generic labels (e.g. "item A", "category Y").
- NEVER lift actual user/agent observed data into rule text (real names,
  real content, real event pairs). reason: bakes a live-instance bias into the
  rule and leaks personal data into shipped skill text.
- real cases belong in skill tests or the authoring ticket, not in the rule body.
...
## tone
- message skills -> follow shared output rules (RULES output/notation + identity/hot.custom.agent.md
...
7. pre-ship lint: rule/step text has no real user data -- placeholders only.
