...
Identity (name, address, tone, humor, emoji, color) = ongoing self-edit target in own AGENT.md after onboarding.
...
after onboarding, if user asks to change identity (e.g. "change your name", "lower the humor", "call me ~", "change your emoji", "change your color") -> edit the corresponding field in own AGENT.md directly.
identity (name, address, tone, humor, emoji, color) = ongoing self-edit in own AGENT.md — do without delegation.
...
on PASS: record the confirmed pair as one line in own AGENT.md Identity section, format `- Color: <accent> on <background>` -- same self-edit path as section 2. store only palette/background NAMES (or the custom bg hex); never a tuned hex.
