...
## hard rules
- CLASS GATE: read `DOGANY_AGENT_CLASS` from `<agent-root>/.instance.conf`
  (missing = main). If domain: the lifekit bundle mounts on the main agent
  only -- do not activate here; point the user to their main agent. The gate
  applies to NEW activation only: items already active (`LIFEKIT=on`) are never
  deactivated by class; deactivation happens only when the user asks.
...
- onboarding not finished (ONBOARDING_PENDING marker in identity/hot.custom.agent.md) -> do NOT
...
2. read current lifekit.conf state. present items ONE at a time, short
   description resolved for `bundle.<id>` in this order:
      (1) service/lifekit/i18n/<AGENT_LANG>.json  (lifekit pack overlay),
      (2) config/i18n/<AGENT_LANG>.json           (framework i18n, inherited),
      (3) the skill's own SKILL.md, one line       (fallback).
...
5. on FIRST activation (LIFEKIT transitions to `on`): extend the Role section
   of identity/hot.custom.agent.md with a staff-agent orchestration bullet -- coordinating staff
   agents / domain lanes on top of the general role. Idempotent: skip if
   already present. On deactivation do NOT auto-remove it; trim only when the
   user asks.
