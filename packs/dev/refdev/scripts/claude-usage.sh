#!/usr/bin/env bash
# claude-usage.sh -- Claude Code usage-window check (dev pack, generalized).
# Default: live rate-limit only (short). With --full: also the cache report.
# NOTE: Live section gets credentials from ~/.claude/.credentials.json or the
# macOS Keychain. When neither yields a token (API-key household, no OAuth),
# the script exits NONZERO with a one-line message so callers can skip the
# usage gate explicitly instead of blocking on it.
#
# Usage: claude-usage.sh [--full]   (default: live-only)
# Exit codes: 0 ok / 1 usage cannot be resolved (or no cache data with --full)
...
if [[ -n "$_live_err" ]]; then
  # Polite one-line degradation: callers treat nonzero exit as "usage gate
  # does not apply here" and proceed with an explicit skip (no blocking).
  _access_token=""
  echo "Claude usage could not be resolved (${_live_err}) -- usage gate does not apply in this environment."
  exit 1
fi
...
def bar(pct, width=20):
    # short unicode block bar -- narrow enough that mobile <pre> rendering
    # does not wrap the line (wrapping breaks bar/line alignment).
    try:
...
# per-model / scoped limits -- no severity tag, shown as extra rows.
...
# Default is live-only (keeps the output short). The detailed cache snapshot
# report is opt-in via --full.
if [[ "$SHOW_FULL" != "1" ]]; then
  exit 0
fi
