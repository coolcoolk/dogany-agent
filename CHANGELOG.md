...
Patch. The 2.0 layout migration now finishes in a single update run, and says so.

The migration deliberately ran in two stages: the first rewired the onboarding hook
and settings, the second moved the identity and rule files. That split protected a
live agent from a re-onboarding window, but nothing told the owner a second run was
needed -- the notice went to stdout, and the update restarts the bridge, so the
session holding that line disappeared. An instance could sit half-migrated
indefinitely while reporting a clean update.

**The split turned out to be an ordering problem, not a structural one.** The only
thing the second stage waited on was a single settings value, rewritten later in the
same run. Moving that one write ahead of the file move removes the second run
entirely -- no continuation machinery, no extra restart.

**And the outcome is now recorded where a restart cannot erase it.** Whether the
migration finished, was postponed, or gave up, the agent tells the owner once, in
its own words. A migration that automates its own completion and then stays silent
has moved the defect, not fixed it.

Instances already sitting half-migrated converge on their next ordinary update.

## [2.0.2] - 2026-08-29

Patch. Back-ports framework fixes that existed only in a live instance.

A live instance carried seven framework files with local-only fixes that were never
carried back to canonical. The v2.0.1 update attempt surfaced them: the content-drift
guard refused to overwrite 121 lines of live-only content and wrote nothing. The guard
was right -- one of those fixes was the DGN-963 remedy for Google Workspace CLI
resolution, and overwriting it would have reintroduced that fault.

**A connected Google account is no longer misreported as disconnected.** The mailer and
mirror adapter now resolve the `gws` binary instead of trusting `PATH` alone, falling
back to the known install locations and honouring a `GWS_BIN` override. Four mirror and
browser-auth routines carry the same rescue. Previously a `PATH` change could make an
authed account read as unauthenticated, routing the user back through onboarding to fix
a problem they did not have.

Two files were deliberately NOT back-ported: their canonical versions are ahead of the
live copies, and overwriting would have regressed them.

## [2.0.1] - 2026-08-29

Documentation patch. No code changes.

The v2.0.0 release note carried estate-internal instance names in its cut-record
comment, which the public-export prose gate correctly refused. The canonical v2.0.0
tag is valid and stays; the public mirror ascends to this tag instead. The wording
was rewritten generically rather than baselined past the gate.

## [2.0.0] - 2026-08-29

The agent becomes a platform. A feature bundle -- a kit, a skill set -- is now an
independent product with its own version line, installed and updated apart from the
framework that hosts it. That single change is what makes this a major.

**Packs ship on their own.** The install contract, the payload surface contract, pack
coordinate resolution, and the installer's reference-vs-copy rule all land together.
Skill ownership is machine-enforced: when two packs deliver the same skill, a gate
catches it. Before, install order silently picked the winner. The delivery-source list
is derived from the installer's own anchors rather than hand-maintained, so a new
surface cannot be missed by forgetting to declare it.

**Updates arrive on a channel.** dev, staging, stable -- with a promotion judge
(`promote.sh --check`) between them, a dev-tag cut path, and a hotfix lane. Previously
there was one lane straight to stable.

**Asynchronous work ends explicitly.** Delegated and deferred work now terminates in a
recorded completion or failure; silence is not a terminal state. The ledger and its
sweep are driven from two independent legs that do not watch each other.

**The agent's own document layout moved** (destructive; the update migrates it).
Persona and owner files move under `identity/`, working discipline under `rules/`,
and the vendor contract under `vendors/`, behind a new loader hub. Anything an
instance wrote itself is preserved by naming convention rather than by an allowlist.
Migration is state-driven -- interrupt it and re-running converges -- and the reverse
procedure stands on its own.

**Public boundary cleaned.** Development and release machinery is out of the public
mirror, hardcoded owner paths are gone, and the publish keep-list holes are closed.

**Declared vs actual is audited daily.** Ledger, catalog, and manifest are compared
against what is actually installed, every day. Three separate defects had been growing
from that one blind spot.

46 tickets. Known remaining items are listed in `releases/v2.0.0.md`.

## [1.43.0] - 2026-08-24

The quiet-failure purge, carrying the deployment ring's first real cut. Two
threads meet here. The first was prepared under the 1.42.0 number: the
three-stage deployment ring takes actual shape -- until now the ring was
three stages on paper and one straight-to-stable lane in practice. A
dev-channel tag can now be cut, and a promotion judge examines the four
conditions (canary consumed / surfaces exercised / bridge alive / owner
approval) each with printed evidence; owner approval is never machine-judged.
The unified update loop stands with it: framework and packs update as one
plan, a crew-shared database receives each change exactly once, and a
mid-flight failure can no longer resurrect as a half-applied state. (1.42.0
itself was consumed in the field from a dev-channel cut and never received a
stable tag; its content ships here and the number rolls forward.)

The second thread is this cut's own anchor: gates that could never open,
checks that always passed, and deliveries that silently dropped content are
each made honest -- prove what you checked, fail loud, or be retired. The
private-repo push gate is the headline. A live workspace's pre-push hook ran
the public-boundary sweep against a private owner-backup repo whose very
content is the owner's tickets and paths, so the gate structurally never
opened: an unpushed backlog of hundreds of commits, reported for weeks as
"awaiting approval", was in fact a jammed gate nobody investigated -- an
alarm that always fires is background noise. The fix adds zero new gate
machinery: the existing owner-backup exemption marker now actually reduces
the sweep, the reduction is printed on every push (a clean full sweep and a
clean reduced sweep can never look alike), the two hook variants that had
silently diverged (fail-open root / fail-closed template) collapse into one
byte-identical script whose missing-sweep policy is decided at runtime from
the repo's own content, and the sweep script itself finally rides the
instance update channel instead of staying frozen at mint time. Test
fixtures now self-declare by form (a uniform TEST-ONLY- prefix) instead of
accumulating path allowlist entries that would also pass a future real
credential.

The same silence is hunted down the publish chain. The export allowlist (an
enumerated KEEP list since the public-boundary cleanup) had twice in twelve
hours silently dropped a file a kept script depends on -- publish succeeds,
every gate is green, and the breakage surfaces later on the consumer's
machine. The export is now checked as a closed set: a dependency-closure
gate statically scans every kept script and blocks the publish when it
references a repo file outside the kept set, the runtime lib directory is
kept whole by glob so per-file omission is structurally impossible, and a
reverse audit blocks any file in that directory no kept script references --
so a dev-only module cannot ride the glob out into the public mirror either.
Scan and reference counts print on every run: zero findings and did-not-run
never look the same.

On the bridge, three silences fall. The fourth [[OPTIONS]] delivery path --
the domain fast-path push -- was never wired to the canonical marker
recognizer, so a fast-path body carrying both a fenced table and an options
marker landed with the table delivered and zero buttons built; it now
computes the same recognizer the other three paths already use. A
session-inbox file that fails UTF-8 decoding used to be retried every twenty
seconds forever, blocking every file sorted behind it; it is now quarantined
once, loudly. And /authsync is retired outright: its unconditional
file-to-keychain overwrite re-injected superseded refresh tokens after CLI
runtime rotations -- the root cause of the estate-wide daily auth deaths.
The only credential writer is now the CLI itself; token-sync.sh becomes a
read-only inspector whose "stores diverged" verdict is reported as the
designed steady state rather than an error, and a typed /authsync gets a
retirement notice instead of reaching the model. The locale trap that
poisoned that inbox in the first place is fixed at the template: push and
self-update scripts probe for a real UTF-8 locale (en_US.UTF-8, then
C.UTF-8) before truncating anything, truncation is character-based, and a
host with no UTF-8 locale at all warns loudly instead of byte-slicing a
Korean character in half.

The loudest silence was background work vanishing. A subagent dispatched
from a conversation lives inside the session process, and the remote
interrupt the bridge sends -- issued by an explicit stop, but also
automatically whenever the owner merely speaks while a turn is running --
does not simply cancel the turn: it unconditionally sweeps the task registry
and kills every running agent in the session. No reachable signal ends a
turn and spares them. The scope field the terminal's own escape key uses is
absent from the remote protocol entirely, and injecting one changes nothing
(measured). Fourteen subagent deaths in one week on a sibling instance trace
to this path, nine of them triggered by nothing more than a mid-turn
message. The answer is not another guard but relocation: a detached
dispatcher spawns long work as a worker in its own session, outside the
registry the sweep walks. Measured against both killing axes -- interrupt
and disconnect -- the in-session subagent dies to the first, a background
shell survives the first and dies to the second, and the detached worker
survives both and runs to completion. It ships in the template with the two
dependencies it needs, and a first-run ledger gap that would have killed the
very first spawn on a fresh instance is bootstrapped shut. The stop wording
itself is deliberately left as-is: it currently tells the truth about what
dies, and it changes only when the mechanism does.

Pack machinery rounds it out. Behavior packs (kind=pack) now actually take
the contract install path -- the kit pipeline with kit-exclusive steps
class-gated off and the service surface keyed by the pack's own namespace --
and the dev pack becomes its first real adopter, retiring the legacy-grace
no-op that had been reading as a green lint verdict while checking nothing.
Proving that closed two SQLite trap-doors in the lint gate: uppercase
extensions and WAL sidecars shipped without their main file both passed
clean before. The public installer menu had gone silently empty -- it
filtered on a status vocabulary the pack contract had retired, excluding
every published pack; the vocabulary is aligned, kit rows are excluded by
class rather than by spelling, and a builder-side pack can declare itself
menu-hidden while staying installable by id (hidden is not forbidden). The
health-trainer pack flips to published with its counter-signature pair. The
devkit ticket-hygiene scanner gets the same honesty treatment: a naming
convention it could not express made it index zero tickets and exit green,
and an empty ticket id fell into dedup and silently dropped tickets from the
scan -- a glob knob replaces the prefix assumption, zero-indexed is a loud
failure, and every dropped file is reported by name. The instance
ticket-prefix convention is engraved into the worklog template so sibling
instances stop colliding on one another's number sequences.

*Correction (2026-08-27, DGN-945 §5): "devkit ticket-hygiene scanner" above
was a misnomer at release time -- the scanner ships in the dev behavior pack
(kind=pack, id `dev`), and "devkit" is reserved for the crew-shared kit
product, which does not yet exist. The released wording is kept as history.*

## [1.41.0] - 2026-08-22

Packs learn dependencies, and a silently-dying button path is repaired. A
pack can now declare the kit it requires, and both halves of the install
machinery enforce the declaration: the lint gate validates its form, and the
installer refuses to proceed when the requirement is not met on the target
instance -- fail-closed on both sides, with the two deliberately-independent
implementations held identical by a cross-test. Non-kit behavior packs get an
explicit payload surface contract of their own: what such a pack may carry is
now an allowlist, kit-exclusive cargo (a database lane, a mirror adapter) is
rejected instead of quietly accepted, and a pack impersonating a kit fails
the gate outright. The installed-packs ledger write becomes atomic, so two
installs racing each other can no longer corrupt the record. The headline
rider is a button repair: a labeled options marker could die silently --
three hand-rolled marker checks never learned the labeled form, so the
buttons and the very warning meant to catch their absence were skipped
together, which is what took down button rendering on a live agent. Marker
recognition now routes through one canonical recognizer, the warning fires
even when the gate is off, and a structural guard keeps hand-rolled checks
from coming back. Rounds out with a catalog coordinate fix pointing the
health-trainer pack at its real home, and a test back-land for the in-flight
debounce work that had shipped without its suite -- four standing failures
clear, with no behavior change.

## [1.40.0] - 2026-08-22

The pack gate stops assuming there is only one kit. Both halves of the
install machinery -- the lint gate and the installer -- carried the lifekit
name as a hardcoded constant, so a second kit could never structurally pass
inspection or install. The kit name now derives from the pack's own manifest
declaration, a kit may declare it ships no database lane and have the DB
checks skipped honestly instead of smuggled past, and a corrupted manifest
now fails the gate closed instead of slipping through the legacy-pack grace
path as a false green. A kit skill can also be declared crew-shared: one
canonical copy serves every instance in a crew via symlink, guarded so a
framework update can never engrave one instance's placeholder values into
the shared body. Three riders round it out: an owner `/restart` command with
an immediate acknowledgement (the restart path finally has a slash entry
instead of a button buried inside `/authsync`), a canonical-repo trunk guard
that blocks plain hand-edits on main while staying inert in live instances,
and a containment guard on the knowledge-snapshot mirror that refuses to run
when the destination is ahead of the source -- closing the defect that would
have silently destroyed a live knowledge warehouse.

## [1.39.4] - 2026-08-21

Interaction dead-ends. Three surfaces knew the next step and gave no way to
take it. An options list asked as a list could collapse into plain text, so
the buttons never appeared and the answer had to be typed. `/authsync` knew a
restart was required to pick up fresh credentials, yet ended at "a restart is
recommended" with no path to one -- it now attaches a one-tap restart button
on the successful-sync branch only (an already-matching, non-macOS, or failed
run stays button-free), and the wording is corrected from "recommended" to
"must". And stopping a task silently took the background work spawned inside
that turn with it: measurement pinned the loss at the FIRST soft stop, not the
second, so the copy now states the consequence immediately, tells you how to
resume, and describes what a second stop actually does. That last one is
interim -- the real fix is moving dispatched work out of the turn's lifetime,
and the notice is scheduled for review when that lands. Rounds out with a
vendored-tree test carry repair that removes two long-standing suite failures.

## [1.39.3] - 2026-08-21

Interaction-surface determinism. Buttons and pins had stopped being
predictable -- whether they appeared depended on which delivery path or
which owner produced them. Model-turn, fast-path, and proactive-push sends
now share one artifact-render tail, so a set logged via the fast path shows
the same keyboard the model-turn path shows, and pushes gain multi-button
rows plus BotFather-style drilldown (no longer capped at one button). The
pinned dashboard surface moves to section-level ownership so a framework
board refresh no longer wipes a domain-owned pin. The streaming draft bubble
demarks markdown live instead of showing raw `**` until the turn finishes.
The pin itself gains formatting: it had always been sent without a
parse_mode, so a code block or bold inside a pinned card rendered
literally -- it now goes out as HTML with a plain-text fallback, so an owner
can put a code-block table in a pin without risking the pin vanishing on a
parse error. Rounds out with four pre-2.0 stability riders: native-CLI-first
PATH resolution, a watchdog auto-recovery marker self-heal, an /authsync
path-resolution fix, and a deterministic plist pick in install.sh.

Anchor: DGN-968 (DGN-966, DGN-967, DGN-969, DGN-974).

**Bridge**
- Shared artifact-render contract: model-turn / fast-path / proactive-push
  now render keyboards through one path (`bridge/artifacts.py` +
  `_send_content_artifacts`), closing a fast-path gap where the
  `[[IDRILL:...]]` marker was silently dropped instead of rendered.
  `push.sh` gains multi-button rows, grids, and drilldown in a single send
  (`--button` contract unchanged). Rider: N-step arm confirmation `{i}`
  tokens now render from the full capture list instead of a fixed first
  index. New user-facing notice `external_file_omitted_noninteractive`
  (ko/en, dec-094 UX gate cleared) for non-interactive rails withholding a
  file outside PROJECT_ROOT. (DGN-966)
- Streaming draft bubble demarks markdown live (no more raw `**`/`__`/
  backtick exposure mid-stream); final render is byte-identical to before.
  (DGN-969)
- /authsync skill-path resolution: workspace-first (`PROJECT_ROOT/.claude/
  skills`) with HOME fallback, matching `/skills`'s existing dual lookup --
  fixes a standing false "script not found" on standard instance layouts.
  (DGN-929)

- Pinned message is now sent with formatting: the pin path had no
  parse_mode, so code blocks / bold / inline code rendered literally and a
  domain card could not carry a code-block table. It now runs through the
  same converter combination the chat path uses and sends as HTML. The
  fallback is the point: if conversion fails or Telegram rejects the parse
  (matched narrowly on `can't parse entities`), the same send is retried as
  plain text -- a `<` or `&` in a ticket title can no longer stop the pin
  from updating. Unrelated errors (e.g. "message is not modified") are not
  swallowed by the fallback. (DGN-974)

**Framework**
- Pin surface (`dashboard.md`) moves from single-owner to section
  ownership: an empty framework board write no longer deletes a
  domain-owned pin (e.g. a workout exercise card). (DGN-967)

**Infra**
- start.sh PATH standardized so the self-updating native CLI install
  (`$HOME/.local/bin`) resolves ahead of Homebrew/`/usr/local/bin`,
  closing the DGN-961 stale-CLI recurrence path. (DGN-963)
- watchdog service-marker self-heal: backfills a never-written recovery
  marker on the healthy-heartbeat path, and resolves ambiguous plist
  candidates deterministically instead of silently picking the wrong one.
  (DGN-964)
- install.sh `install_launchd` picks its plist deterministically instead of
  taking the first glob match: identity-based Label match first, sorted
  fallback when nothing matches, and a loud no-op (never a silent guess)
  when there are no candidates. `*.watchdog.plist` is excluded explicitly
  rather than relying on alphabetical luck. Same fix pattern as DGN-964.
  Only reachable on directories carrying legacy plist leftovers; fresh
  mints were unaffected. (DGN-965)

## [1.39.2] - 2026-08-20

Patch: residual v1.39.x live-regression repair and render quality. Fixes a
shutdown teardown race that froze fold bubbles as raw markdown on restart,
promotes markdown ATX headings to a glyph plus bold, reaps orphan /btw fork
processes and logs empty-response causes, repairs Warg render seams (Korean
display-name byte-cut and countdown/timer ordering), and blocks
degenerate-repetition feedback in the session recap. Update-notice section
glyphs default to the left-triangle to match the update-available label.
(Rolls up the v1.39.1 fixes, which were committed but never tagged/shipped.)

### Fixed
- Shutdown fold-flush teardown race: fold bubbles no longer freeze as raw markdown on restart (DGN-946).
- Warg render seams: Korean display-name torn-read hold + countdown turn_active deferral (DGN-950).
- /btw fork: orphan CLI leak reaped on timeout + empty-response cause logging (DGN-953).
- Session recap: degenerate-repetition (flood) feedback collapse (DGN-951).

### Changed
- Markdown ATX headings render as glyph + bold instead of raw marks (DGN-954).
- Update-notice section glyph default now the left-triangle (unify with the update-available label).

## [1.39.1] - 2026-08-20

Live-regression repair for the v1.39.0 deterministic interaction layer. The
v1.39.0 topology streamed interim narration and the terminal answer into the
same handler drafts, so the decision-turn finalize operated on interim+final
glue: OPTIONS buttons broke and raw HTML tags leaked into replies. The
streaming handler now seals each interim bubble at the inline terminal
boundary, so the terminal answer opens a fresh draft and finalizes cleanly.
In fold mode, an over-budget compose silently dropped the whole folded
narration; a budget-drop rescue now re-emits the surviving narration as its
own bubble above the answer, and the three fold-absence causes (budget rescue
/ lossless echo / teardown race) log separably. Update-notice rendering is
restored (DGN-718 blockquote layout) and the streamed-finalize raw-tag leak
is fixed. Rider: idrill arm files gain an optional followup_cmd -- a
post-record argv fired only after the terminal command exits 0, under the
same security invariants (list-form, no shell, absolute argv[0], 15s
timeout), with stdout posted as a new message. Hardening: fold-rescue send
failure logs ERROR (never a false success), background budget-drop logs as
loss, and argv[0] absolute-path is enforced in code, not just contract.

DGN-947 (inline seal + fold rescue, anchor), DGN-846 (update-notice finalize),
DGN-939 item2 (followup_cmd).

## [1.39.0] - 2026-08-20

결정론 인터랙션 층. 로깅/드릴다운 인터랙션을 모델 내레이션 메시지 스트림에서
브릿지 결정론 콜백으로 옮긴다. 인라인 키보드 버튼을 누르면 그 메시지가 제자리
갱신되며 다음 단계를 요구한다(새 메시지 0 / 모델 턴 0 / 딜레이 0, 봇파더식 뒤로
네비). 세트 로깅 2단계 드릴다운을 N단계로 일반화하고, 최종 선택 시 기록 + 표
갱신까지 무모델턴 결정론으로 처리한다. 진행 표시는 에이전트 클래스로 분기 --
개발 에이전트는 실시간 스트리밍 후 완료 시 접힌 기록으로 정리(+최종답 별도 버블),
도메인 에이전트는 실시간만(미설정 기본 = 도메인 실시간). 심층 부가정보는 확장형
인용에 접어 담고, 사진 / 문서 캡션 맨 앞의 /queue도 진행 중인 턴을 끊지 않고 다음
턴으로 합친다.

DGN-939 (인플레이스 콜백 엔진 + arm 계약 + 접이식 인용), DGN-930 (진행표시 분기),
DGN-941 (캡션 /queue 합치기).


## [1.38.3] - 2026-08-19

Class-wise notification policy (notify_policy) with two mechanisms, so
in-progress noise stays silent while real signals ring. Each bridge send
belongs to a notification class; the class decides whether the Telegram push
is silent or loud. Mechanism A covers edited-surface bubbles (draft / fold /
countdown / dashboard): the notification is decided once at the first send and
every later in-place edit is notification-free. Mechanism B covers one-shot
messages (Telegram-default loud), where a send site opts into silence by
tagging a class -- currently the [[OPTIONS]] button appendix, so the buttons
never bury the loud answer body. Locked no-config defaults: draft=loud,
fold=silent, countdown=loud (first tick = set-start signal), dashboard=silent,
options_prompt=silent. NOTIFY_POLICY env ("class=silent" / "class=loud" list)
overrides per class; unset leaves the locked defaults, so nothing changes
unless you opt in. (DGN-932)

## [1.38.2] - 2026-08-18

Usage gate gains two opt-in switches (defaults unchanged). USAGE_GATE is a
master on/off kill switch for the whole gate. USAGE_GATE_7D scopes the weekly
(7d) block: full (default) blocks every session; auto-only blocks only
autonomous/cron sessions and waives 7d for the owner-driven live session (5h
window + burn-rate hint still apply). Session kind is detected via PROJECT_ROOT
(bridge-set, live only), with a DOGANY_SESSION override and a fail-safe to
"auto" so autonomous fan-out is never wrongly waived. Autonomous protection and
the DGN-835 relief/ledger/exempt layers are all preserved. (DGN-925)

## [1.38.1] - 2026-08-18

idrill primitive made fully shape-agnostic: step buttons, validation, and
message copy are declared by the arm file (step_buttons / step_validate /
step_text) instead of hardcoded; the initial keyboard renders via a new
[[IDRILL:<arm_id>]] output marker (reusing the [[OPTIONS]] marker path). The
bridge is now domain-neutral for the drilldown primitive -- no use-case-specific
values baked in. Pre-activation (inert). (DGN-924)

## [1.38.0] - 2026-08-17

In-place drilldown capture primitive (idrill) + output-discipline reconcile +
unified conf parser. The bridge gains a stepped-capture callback family: each
tap swaps the same message to the next-step keyboard, and the final tap fires
the command declared in a one-shot arm file, replacing the message in place
with a confirmation -- zero model turns, zero new messages. A domain-neutral
cross-kit process-boundary primitive (pack writes the arm file; bridge
executes the declared argv, no shell); first application: workout set logging.
Riders: decision-reply-frame skill / RULES output-discipline reconcile, and a
single conf parser adopted by five routines.
(DGN-918, DGN-921, DGN-916)

**Bridge**
- idrill in-place drilldown capture: a button tap swaps the inline keyboard to
  the next step; the final tap consumes a one-shot arm file
  (traversal-guarded) and executes the DECLARED argv with positional value
  substitution (argv exec, no shell -- contract v2), then renders the
  confirmation in place. Expired/duplicate taps and command failures are
  fail-soft. Domain-neutral (the bridge knows no domain fields); first
  application is workout set logging. Generic i18n (en/ko) strings + dedicated
  test suite (71) included. (DGN-918)

**Output discipline**
- decision-reply-frame skill: recommendation marker restricted to body list
  lines (never on button labels), Korean trigger phrases added, RULES.md
  cross-reference aligned to the English template section name. (DGN-921)

**Routines**
- Single conf parser routines/lib/conf_reader.py: quote/whitespace
  normalization for config/agent.conf unified in one place; adopted by
  onboarding-check, output-gate-stop, session-recap, status-footer,
  usage-gate via guarded import (fail-open contract preserved). (DGN-916)

## [1.37.0] - 2026-08-17

Output discipline + bridge interaction quality. Interrupt-default inverted to
debounce-then-interrupt, /queue and /stop semantics locked, output-gate canaries
deployed, countdown end gains a tappable affordance, slash command menu
single-sourced, /btw output routed through the shared HTML formatter, and four
integration fixes bundled.
(DGN-911, DGN-913, DGN-914, DGN-915, DGN-919, DGN-920, DGN-922)

**Bridge interaction UX**
- In-flight default inverted: a new message arriving while the agent is running
  now waits 5 s (debounce) before interrupting -- nudging instead of
  immediately cancelling the running turn. /queue enqueues a message explicitly
  with no interrupt; /stop discards the buffer and cancels. (DGN-911)
- Countdown timer end emits a tappable completion affordance, eliminating the
  dead-air gap after a timer finishes. (DGN-915)
- Slash command menu single-sourced from COMMAND_MENU_SPEC: the /-popup menu and
  /help are always in sync (order, copy, i18n). Owner-locked menu copy. (DGN-919)
- /btw fork output routed through the shared HTML formatter -- **bold** now
  renders instead of leaking raw markers. (DGN-920)

**Output-gate discipline**
- BACKREF detector (canary, detect-only): flags a final answer that defers its
  substance to the collapsed progress log rather than stating it directly. (DGN-913)
- Multi-question prefilter + hit-turn raw canary logging; Phase-2 harvest
  answer-omission sweep (data foundation for future enforcement). (DGN-914)

**Fixes**
- Ghost turn after /stop eliminated; countdown done-button survives the 20-min
  stale gate; long /btw responses split and delivered in full; /btw fork no
  longer starves the main conversation. (DGN-922)

## [1.36.1] - 2026-08-16

알림·재시작 통지의 HTML 태그 깨짐 수정 + 락스텝 재발 방지 게이트 (DGN-906/891/907/905).
- 재시작·알림 통지에서 글자로 새던 HTML 태그(`<blockquote>` 등)가 정상 렌더로 복구된다.
  push 통로가 부르던 정리 함수가 vendored bridge에서 누락됐던 회귀를 복구. (DGN-906)
- 텔레그램 HTML 태그가 전송 분할 경계에서 깨지던 문제 수정 -- 열린 태그 자동 닫기·재개,
  태그 제거 폴백 (formatting/bot/streaming). (DGN-891)
- 락스텝 재발 방지: 업데이트 완료 시 알림 전송 계약을 자동 점검하고, 만료된 보존 핀을
  자동 무효화(계약 통과 시에만 재착지)하는 게이트 추가. (DGN-907)
- vendored bridge 핀을 OSS main(87ac8ca)으로 정합 -- 드리프트 0. (DGN-905)

## [1.36.0] - 2026-08-16

Why-tethering estate rule + authsync command + compat-lint hardening + /btw fork isolation (DGN-900/759/868/902).
- Why-tethering estate rule (DGN-900, anchor): RULES Principles gains an explicit means-end
  inversion guard. When 2+ candidate Whys survive on a consequential action, the agent must
  STOP and present ALL candidates -- never collapse to one guess, never issue a bare "why?".
  Once the Why is pinned the agent re-derives the optimal How and proposes a better one if
  it exists. Micro/safe/reversible actions are exempt (just act). This rule is now part of
  the canonical baseline and propagates to all minted instances via self-update.
- Bridge authsync command (DGN-759): `/authsync` slash command re-syncs Telegram
  credentials after a re-login without requiring a full restart. Keeps the bridge live
  through credential rotation.
- compat-lint bak-residue hardening (DGN-868): C4b bak-residue fixture corrected so the
  check actually fires (false-negative eliminated). `test-secret-sweep.sh` added as a
  regression harness for secret-sweep paths. Internal test surface only; no user-visible
  behavior change.
- /btw slash command (DGN-902): `/btw` opens a session-fork branch for aside notes without
  polluting the main conversation context. Isolation is per-fork.
- Release-closer step 7 (B1): release-close subagent now performs a read-only rider
  open-ticket scan and reports shipped-but-open candidates in the close report (no write
  outside the 3-place edit scope).

## [1.35.1] - 2026-08-15

프레임워크 하드닝 번들 -- 기억 정리 정확도 + 카드 렌더 안정화 + 백그라운드 작업 안전장치 (DGN-884/885/887).
- 기억 정리 정확도(DGN-884): 야간 기억 정리가 항상 최신 색인 기준으로 중복을 걸러냄. 같은 사실이
  두 번 저장되던 경로를 엔진 차원에서 봉합(consolidate 전 인덱스 갱신을 엔진에 내재화 -- 이전엔
  래퍼 경로에만 있어 다른 호출 경로에서 stale 풀 대비 dedup이 새어나감). dry-run 무기록 계약 보존.
- 카드 렌더 안정화(DGN-885): 렌더 환경(matplotlib)이 빠져 있어도 자동 보장 스크립트
  (setup-render-venv.sh)로 카드가 깨지지 않음. 공용 렌더 venv 단일 경로로 통일.
- 내부 안전장치(DGN-887): 백그라운드 작업이 운영 브랜치를 되돌리는 사고를 원천 차단 -- 쓰기 계열
  서브에이전트에 "라이브 레포 브랜치 전환/HEAD 이동 금지, 작업은 격리 공간에서만" 계약 삽입.

## [1.35.0] - 2026-08-15

watchdog 라벨-증발 자동복구 백스톱 -- launchd에서 서비스가 통째로 사라져도 스스로 재등록 (DGN-888, anchor).
- bridge/watchdog.sh: 서비스 라벨이 launchd에 미등록인 상황(bootout 후 bootstrap 실패 등)을
  기존엔 "skipping"으로 방치했으나, 마커 계약 기반 복구 분기 추가 -- 등록 주체가 선언한 plist
  경로/라벨 마커를 검증(라벨 완전일치 + placeholder 거부)한 뒤 launchctl enable -> bootstrap으로
  재등록. rate-limit 게이트 뒤에 배치 + 전 시도를 원장 기록(hot-loop 차단).
- bridge/watchdog_setup.sh: 등록 시점에 .service_plist 마커(plist 절대경로 + 라벨) 기록.
- 효과: 2026-08-15 라벨 증발로 인한 18분 다운 같은 사고를 약 4~6분 내 자동 복구. 마커
  부재(fresh/manual 설치)는 기존대로 무영향.
- 알림 i18n: 복구 성공/반복실패 통지를 ko/en 양쪽 제공(shell locale 분기, Python 비의존).
- 운영 주의: 브릿지를 의도적으로 완전 정지할 때는 watchdog unload 또는 .service_plist 마커
  제거를 포함해야 함(안 그러면 백스톱이 재기동시킴).

## [1.34.2] - 2026-08-15

sdk_bridge 역드리프트 클래스 소멸 -- 라이브가 preserve로 얼려둔 마지막 델타를 캐논에 흡수 (DGN-886, anchor).
- bridge/sdk_bridge.py: `user_has_streamed_output`에 DGN-699 D4 체크 4줄 carry-back.
  자란 fold 버블을 스트리밍된 부분 출력으로 인정 -> fold-only 죽은 턴이 turn-death
  세이프넷에서 "메시지 미처리"로 오표시되지 않고 부드러운 partial-output 변형으로 처리.
  DGN-721 재벤더(da4be60)가 timeout 경로 D4만 이식하고 이 reader를 누락했던 것을 보정.
- 효과: 이 델타가 라이브 sdk_bridge의 유일한 canonical-미보유 잔재였음 -> 흡수 후 캐논이
  라이브의 순수 상위집합 -> 라이브가 sdk_bridge를 `.dogany-preserve`에서 제거하고 self-update로
  소비 가능(역드리프트 재발 차단). UPSTREAM.md Vendor-rev 엔트리 추가(canonical-only, OSS pin 불변).

## [1.34.1] - 2026-08-15

옵션 버튼 라벨 규율 붕괴 -- 라벨 잘림 근절 (DGN-881, anchor).
- bridge/options.py: 버튼 라벨의 구분자 파싱층(sep-strip / 폭 절단 / 콜론·점 특례 등)을
  통째로 제거. 라벨이 짧으면 그대로 표시, 31 가중폭 초과 시에만 번호 손잡이로 후퇴하고
  전체 설명은 본문 목록에 유지. 음절 중간 절단 / 짝 없는 괄호 잔존 등 "잘림" 클래스 소멸.
- i18n: 번호 손잡이(ko "N번" / en "No.N")와 추천 표시(ko "(추천)" / en "(rec)")를
  로컬라이즈 토큰으로 승격. 추천 표시는 라벨이 아니라 본문에 위치.
- 출력 계약(bridge.md / telegram.md): 얇은 라벨 계약으로 갱신.
- 진행 기록(접힌 인용): compose 경로에서 footer 접합이 마지막 문단 보존을 무력화하던
  버그 수정 + 빈 fold 로그 원인 정정 (DGN-877, DGN-878). (DGN-876 전문 보존은 v1.32.0 선반영)

## [1.34.0] - 2026-08-14

공개 미러 = 캐논을 따라가는 단일 미러 경로 (DGN-880, anchor).
- scripts/publish.sh: 발행을 단일 "mirror-forward" 경로로 통일. 공개보다 앞선 stable
  태그를 오름차순으로 하나씩 발행(가장 위 버전으로 점프하지 않음). 각 태그마다
  로컬==원격 SHA + origin/main 조상 + 트리 VERSION 3중 검증, 태그 콘텐츠만 tag
  worktree에서 취하고 큐레이션 기계장치(allowlist / visibility manifest / secret-sweep /
  .sweepignore)는 현재 main 고정 -- 옛 규칙으로 새는 fail-open 차단. 스텝마다
  secret-sweep 0-hit 강제. 스킵 판정 = 발행-subject AND 원격 태그 존재(태그 유실 창 방어).
- scripts/lib/mirror-forward.sh (신규): 열거 / 검증 / 스킵 헬퍼.
- scripts/lib/cadence-gate.sh: C1 가감속 게이트를 WARN/계측 전용으로 강등 -- 발행자가 늘
  한 칸씩 오르므로 큰 점프가 구조적으로 불가, 하드 BLOCK 불필요(brick 방지는 구조로 달성).
- execute 모드에서 secret-sweep 스크립트 경로 env 치환 차단(공개 시크릿 게이트 무력화 방지).

옵션 버튼 오버플로 -- 잘림으로 의미가 쪼개지지 않게 (DGN-879, anchor).
- agents/.template/bridge/options.py: _shorten_button_label에서 음절 중간 생짜 절단(hard-cap
  raw-cut)을 제거. 라벨이 안전폭(31 weighted) 초과 시 버튼을 순수 "N번" 손잡이로 후퇴시키고,
  strip_consumed_options가 그 항목의 전체 라벨을 본문에 무조건 재서술한다(의미는 언제나 본문에
  보존). _BUTTON_LABEL_HARD_CAP 상수와 글자단위 절단 루프 삭제. sep-strip/704c 가드/790 콜론
  처리는 그대로 유지.
- agents/.template/bridge/tests: test_dgn879_overflow_handle.py 회귀 추가 +
  test_dgn704_label_shorten.py를 overflow->handle 신계약으로 갱신.

telegram.md / bridge.md 프레임워크 refresh 전파 (DGN-875, rider).
- update.sh: telegram.md / bridge.md를 framework-owned verbatim refresh(섹션 3k3/3k4)로
  매니페스트에 추가 -- canonical 출력계약 개선이 self-update로 라이브에 자동 전파된다. 사용자
  수정본은 dated 백업 후 갱신하며, AGENT.md/USER.md identity 가드에는 영향 없음.

## [1.32.0] - 2026-08-14

fold(인용접기) 전문 보존 -- 진행 나레이션이 최종답보다 풍부할 때 fold를 통째 삭제하지 않고
중복 문단만 트림해 나머지 진행 전문을 접힌 인용으로 남긴다 (DGN-876).
- agents/.template/bridge/sdk_bridge.py: _finalize_result의 fold 처리에서 최종답이 진행기록에
  전부 포함되면 fold 버블을 통째 삭제하던 fast-path(2곳: grown-bubble / compose-fallback)를
  제거하고, 기존 _subtract_paras(DGN-777) 단일 경로로 통합. 이제 fold는 중복을 뺐을 때 남는
  것이 없을 때(interim == 최종, 순수 에코)만 삭제되고, 진행기록이 최종답의 상위집합이면 초과
  진행 문단이 접힌 인용으로 보존된다. 미사용이 된 _final_fully_in_interim 메서드 삭제(31줄).
- agents/.template/bridge/tests/test_dgn699_growing_fold.py: DGN-876 회귀 6종 추가(full-dup
  삭제 / superset 보존 x2 / 부분중복 유지 / 알림 0 회귀). 실코드 그릴(Fable) GO, 신규 실패 0.
