...
def _run_reconcile_locked(state_conn, src_conn, cal_ids, tl_id, repair,
    ...
    checked = 0
    sot_rows = {}
    for r in src_conn.execute("SELECT * FROM timeblock").fetchall():
        ev = dict(r)
        if not A.in_mirror_scope(ev):
            continue
        if scope_ulids is not None and ev["ulid"] not in scope_ulids:
            continue
        # DGN-579 bundle: enrich BEFORE projection compare -- the surface
        # holds the place-resolved location + persons with-line, so the SoT
        # side must project from the same enriched dict (F1).
        ev = A.enrich_event(src_conn, ev)
        sot_rows[ev["ulid"]] = ev
        checked += 1
