...
def route_surface(event: dict) -> str:
    ...
    DGN-579 D-C3 (owner 2026-07-26): presence mirrors as a calendar all-day
    banner (reverses the B5/E3 exclusion)."""
...
def in_mirror_scope(event: dict) -> bool:
    ...
    mirror scope -- it renders as a non-blocking all-day banner on the
    travel calendar. Presence rows flow through the same recurrence /
...
# presence all-day banner (owner D-C3 2026-07-26).
#
...
#   - the presence banner span/title is derived by one helper pair used by
#     projection AND body build.
# ---------------------------------------------------------------------------
...
PRESENCE_BANNER_FMT = mirror_i18n.t(
    "mirror.presence_banner", u"\uc5ec\ud589: {place}")
...
def _presence_banner_dates(event):
    """Presence span -> all-day banner dates (start_date, end_date_excl),
    local to the row's display tz. end_at is NOT NULL by presence CHECK.
    A timed end mid-day still covers that local date -> exclusive end is
    the NEXT date; an exact-midnight end is already exclusive (m15)."""
    tz = ZoneInfo(event.get("display_tz") or DISPLAY_TZ_NAME)
    s_local = datetime.strptime(
        event["start_at"], "%Y-%m-%dT%H:%M:%SZ").replace(
        tzinfo=timezone.utc).astimezone(tz)
    e_local = datetime.strptime(
        event["end_at"], "%Y-%m-%dT%H:%M:%SZ").replace(
        tzinfo=timezone.utc).astimezone(tz)
    s_date = s_local.date()
    if (e_local.hour, e_local.minute, e_local.second) == (0, 0, 0):
        e_date = e_local.date()
    else:
        e_date = e_local.date() + timedelta(days=1)
    if e_date <= s_date:
        e_date = s_date + timedelta(days=1)
    return s_date.isoformat(), e_date.isoformat()


def _presence_banner_title(event):
    """Banner title 'Trip: <place>' (i18n mirror.presence_banner). Place =
    resolved place name (enrich_event fills location from the place FK;
    presence requires place_id by CHECK); falls back to the row title."""
    place = event.get("location") or event.get("title") or ""
    return PRESENCE_BANNER_FMT.format(place=place)
...
def calendar_projection_from_event(event: dict) -> dict:
    ...
    it; bare dicts default to ''); presence projects as an all-day banner
    (D-C3) -- banner title, banner date span, always transparent."""
    gcal_status, color_id = event_to_cal_status(event)
    tz_name = event.get("display_tz") or DISPLAY_TZ_NAME
    if event.get("kind") == "presence":
        s_date, e_date = _presence_banner_dates(event)
        start_at, end_at = gcal_datetime_to_utc_instants(
            {"date": s_date}, {"date": e_date}, tz_name)
        return {
            "title": _presence_banner_title(event),
            "note": event.get("note") or "",
            ...
            "color_id": color_id,
            "transparency": "transparent",   # non-blocking by contract
            "persons": event.get("persons") or "",
        }
...
def _build_cal_body(event, surface_id):
    ...
    is_presence = event.get("kind") == "presence"
    body = {
        "id": surface_id,
        # DGN-579 D-C3: presence surfaces as an all-day banner title.
        "summary": (_presence_banner_title(event) if is_presence
                    else (event.get("title") or "")),
    ...
    if is_presence:
        # Banner span: all-day dates regardless of the stored schedule_kind
        # (a timed presence still covers its local dates, D-C3).
        s_date, e_date = _presence_banner_dates(event)
        body["start"] = {"date": s_date}
        body["end"] = {"date": e_date}
        return body
...
def sync_event(event, cal_ids, tl_id, state_conn, src_conn=None):
    ...
    DGN-579 D-C3: presence rows sync too (travel-calendar all-day banner);
    the former skipped_presence early-return is retired.
...
def _apply_calendar_3way(item, state_conn, src_conn):
    ...
    if row["kind"] == "presence":
        # DGN-579 D-C3: the banner is render-only. Surface edits never enter
        # the SoT (presence_add/presence_move own presence writes; a generic
        # bypass apply would break the one-presence-at-a-time guard).
        # Deletion is handled by the pull's cancelled lane; any other edit
        # is rejected + the canonical banner is re-echoed via the outbox.
        mirror_log(state_conn, "presence_inbound_rejected", ulid,
                   "banner edit dropped; canonical re-push enqueued")
