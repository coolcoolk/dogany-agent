...
The product is built out of one containment relation -- smaller units nest
inside larger ones: a **skill** sits inside a **module**, a module inside a
**pack**, and a pack inside a **kit**. Reading from the smallest unit outward:

<p align="center"><img src="docs/img/structure-tree-en.png" width="700" alt="Structure as a tree with a definitions table. SKILL (smallest unit -- performs one single action; e.g. a reminder skill pings you once at 9pm to take your meds) sits inside MODULE (completes one ability from skill(s) + data and writes to the kit's data contract; e.g. diet-log takes '200g chicken breast', computes calories, saves to LifeKit), which sits inside PACK (bundles modules + knowledge + a speaking rubric to shape a staff agent; e.g. a health-trainer pack builds a workout-coach staff agent that plans routines and checks in), which sits inside KIT (the data contract for a whole domain, whose modules and packs share one schema; e.g. LifeKit is the data contract for all of daily life -- meals, workouts, schedule). Skill in module in pack in kit. Off-nest: a core module skips the kit and attaches straight to the main agent; installing a pack creates a staff agent under the main agent; the main agent equips the kit"></p>

- **Skill** -- the smallest unit: it performs one single action, carried by a
  single `SKILL.md`. For example, a reminder skill just pings you once at 9pm
  to take your meds. Skills are the atoms everything larger is assembled from.
- **Module** -- one or more skills plus their data, bundled to complete a
  single ability, writing into a kit's data contract. A skill lives inside a
  module. For example, the diet-log module takes "200g chicken breast",
  computes the calories, and saves the result to LifeKit. A module is either
  kit-bound (writes into a specific kit, like diet-log) or a core module
  (kit-independent -- see the exception below).
- **Pack** -- modules plus knowledge plus a speaking-style rubric, bundled to
  shape a staff (sub-)agent. Modules live inside a pack. A pack carries no
  persona -- voice and tone are decided later at onboarding. For example, a
  health-trainer pack builds a workout-coach staff agent that plans your
  routines and checks in on you.
- **Kit** -- the largest unit: the data contract for a whole domain, so that
  every module and pack in that domain shares one schema. Packs and modules
  live inside a kit. Which kit you equip is what defines the agent's domain.
  For example, **LifeKit** is the data contract for all of daily life -- meals,
  workouts, appointments, and the like. (Other domain kits -- for
  development/PM, or business -- are planned.)

Two things sit outside the nesting. A **core module** skips the kit entirely
and attaches straight to the main agent (for example, reminders or memory
search, which any agent can use). And installing a pack creates a **staff
agent** that sits under the main agent. The main agent itself -- your one and
only body, holding the identity and the memory, with its persona decided per
instance at onboarding -- is what equips the kit.
...
- Hot inject: `USER.md` and `PROFILE.md` are loaded into context on every turn
...
(`PROFILE.md`), your profile (`USER.md`), a reusable procedure (`SKILL.md`,
which may persist domain records through the service layer into
`database.sql` -- the kit lane, e.g. LifeKit), or everything else into
`memories/`.
...
<p align="center"><img src="docs/img/routing-en.png" width="560" alt="Memory routing: user chat is classified into PROFILE.md, USER.md, SKILL.md (service -> database.sql, the kit lane, e.g. LifeKit), or memories/ topic files; manual routing is agent-driven, the AUTO lane is engine-driven and expanded in the next diagram"></p>
...
- **`agents/main/`** -- the repo-internal scaffold mint target (dev-mode /
  ...
  (loader) imports `RULES.md` (immutable operating rules), `PROFILE.md` (the
...
- **`skills/`** -- framework skills shared across agents (`dogany-cron-register`,
  ...
  `.claude/skills/`. Lifekit domain skills (`diet-log`, `workout-log`,
  `appointment-log`, `relationship`, `relationship-care`, `task-update`,
  `spending-log`, `dogany-routine`) are NOT part of this repo anymore: lifekit
  is an INDEPENDENT PACK (its own repo, `dogany-lifekit`) installed post-mint
  via `scripts/pack/pack_install.sh`, which places them DORMANT under
  `.claude/skills-bundle/`; the `dogany-lifekit-setup` skill then activates
  them via an instance-local symlink `.claude/skills/<id> -> ../skills-bundle/<id>`.
  They are never pre-placed as live dirs under `.claude/skills/` (that would
  make the bundle un-gateable). Non-lifekit opt-in skills ship dormant in the
  template `skills-bundle/` with their own activation paths: `agent-browser`
  (activated during install via the browser automation opt-in step) and
  `youtube-digest` (activated post-mint by asking the agent to enable YouTube
  summaries).
- **`database/`** -- the instance's structured-data mount point. The lifekit
  structured lane (`lifekit.py` / `lifekit.sh` / `schema.sql`: a local SQLite
  "life OS" -- meals, workouts, people, appointments) ships with the lifekit
  pack and lands here at kit activation; this repo keeps only the engine-side
  restore tooling (`restore-data.sh`). No `*.db` data is ever shipped.
- **`service/`** -- stable SDK surfaces that skills import instead of raw data
  layers. The `service.lifekit` facade moved to the lifekit pack.
...
- A minted instance IS its own `PROJECT_ROOT`: it carries real copies (never
  symlinks) of the rules, framework skills, and service surfaces.
...
- Kit content installed by `pack_install.sh` (e.g. the lifekit pack) keeps the
  same instance layout: pack code resolves its DB and peers relative to its
  own installed location, never a parent tree.
...
First run triggers onboarding: `PROFILE.md` is a blank skeleton, and the
...
- The optional `lifekit` lane is an independent pack (repo `dogany-lifekit`);
  its structured lane (`lifekit.db`) is initialized from the pack's
  `schema.sql` at kit activation (`pack_install.sh`), empty -- no user data
  seeded. A mint without the kit ships no lifekit files at all.
