...
# placeholders, .env, lifekit-absence (DGN-803 LS-5), venv imports, bridge selfcheck,
...
# ---------------------------------------------------------------------------
# S4: lifekit extraction contract (DGN-803 LS-5)
# ---------------------------------------------------------------------------
# Lifekit left the framework: a fresh (non-lifekit) mint must complete with
# ZERO lifekit files and NO lifekit.db -- that absence is the NORMAL state,
# never an error. The old schema/user_version lockstep asserts moved with the
# content to the pack gate (compat-lint C3) and the kit-install tests
# (scripts/tests/test-pack-install-kit.sh).
stage S4 "non-lifekit mint ships no lifekit surface (extraction contract)"
LIFEKIT_DB="$INST/database/lifekit.db"
for _lk in database/lifekit.db database/schema.sql database/lifekit.py \
           database/lifekit.sh database/remind_select.py \
           database/routine_roller.py database/routine_projection.py \
           database/relmod.py database/relmod.sh database/migrations \
           service/lifekit/__init__.py config/lifekit.conf; do
  [ -e "$INST/$_lk" ] && fail "extracted lifekit artifact present on fresh mint: $_lk"
done
ok "no lifekit artifacts on a fresh mint (db/schema/core/facade/conf/migrations all absent)"
# Tolerance leg (grill G7): the cron consumers of extracted files must be
# silent no-ops, not error spam.
( cd "$INST" && bash routines/remind.sh --dry >/dev/null 2>"$WORK/remind.err" )
[ "$?" = "0" ] || fail "remind.sh non-zero without remind_select.py (G7 tolerance broken)"
[ -s "$WORK/remind.err" ] && fail "remind.sh wrote stderr on the no-op path: $(cat "$WORK/remind.err")"
ok "remind.sh = silent exit-0 no-op without remind_select.py"
( cd "$INST" && bash routines/routine-roller.sh >/dev/null 2>"$WORK/roller.err" )
[ "$?" = "0" ] || fail "routine-roller.sh non-zero without routine_roller.py (tolerance broken)"
ok "routine-roller.sh = silent exit-0 no-op without routine_roller.py"
...
# Leg 2: onboarding done -> KIT tristate offer pipeline (DGN-466 OQ-B).
# A bare mint.sh instance ships NO KIT= line (the install.sh dgn227 anchor is
# the seeder): the hook reads that as decided-kitless (fail-safe) -> no kit
# or lifekit offer fires. Then walk the tristate: KIT=pending -> kit-select
# fires; KIT=lifekit + LIFEKIT=pending -> lifekit walkthrough fires (class
...
PY
# DGN-803 LS-5: mint no longer scaffolds config/lifekit.conf (it ships with
# the lifekit pack; install.sh seeds the LIFEKIT= key itself). Bare mint =
# no lifekit.conf at all; the walkthrough leg below seeds it explicitly,
# simulating the install.sh anchor.
assert "bare mint ships no config/lifekit.conf (LS-5 extraction)" \
  bash -c "! test -e '$INST/config/lifekit.conf'"
assert "bare mint ships no KIT= line (FATAL-1: anchor must be able to seed)" \
  bash -c "! grep -q '^KIT=' '$INST/config/agent.conf'"
H2A="$(hook_run)" || fail "onboarding hook crashed on kit-absent leg"
printf '%s' "$H2A" | python3 -c '
import json, sys
...
if raw:
    d = json.loads(raw)
    ctx = d["hookSpecificOutput"]["additionalContext"]
assert "kit selection" not in ctx.lower(), "kit-select fired without KIT=pending"
assert "[lifekit pending]" not in ctx, "lifekit offer fired with KIT absent"
' || fail "KIT-absent fail-safe broken (offer fired without a KIT decision)"
ok "KIT absent (bare mint) -> decided-kitless fail-safe, no kit offer"
printf 'KIT=pending\n' >> "$INST/config/agent.conf"
H2B="$(hook_run)" || fail "onboarding hook crashed on kit-select leg"
printf '%s' "$H2B" | python3 -c '
import json, sys
d = json.load(sys.stdin)
ctx = d["hookSpecificOutput"]["additionalContext"]
assert "[kit selection pending]" in ctx, "no kit-select offer in ctx"
' || fail "kit-select ctx did not fire on KIT=pending"
ok "kit-select offer ctx fires on KIT=pending (tristate)"
sed -i.bak 's/^KIT=pending$/KIT=lifekit/' "$INST/config/agent.conf" \
  && rm -f "$INST/config/agent.conf.bak"
# Seed the lifekit lifecycle key the way the install.sh anchor does (the
# template scaffold left with the pack -- LS-5): the walkthrough gate reads
# config/lifekit.conf LIFEKIT=pending.
printf 'LIFEKIT=pending\n' >> "$INST/config/lifekit.conf"
H2="$(hook_run)" || fail "onboarding hook crashed on lifekit leg"
printf '%s' "$H2" | python3 -c '
import json, sys
d = json.load(sys.stdin)
ctx = d["hookSpecificOutput"]["additionalContext"]
assert "[lifekit pending]" in ctx, "no lifekit walkthrough offer in ctx"
' || fail "lifekit walkthrough ctx did not fire (KIT=lifekit + LIFEKIT=pending)"
ok "lifekit walkthrough ctx fires after kit choice (KIT=lifekit, bare-mint class defaults main)"
...
# ---------------------------------------------------------------------------
stage S10 "re-mint --force keeps identity/config/.env"
# config/lifekit.conf stays in the keep list: S7 seeded it as an INSTANCE
# file (the install.sh anchor path) and a re-mint must never clobber it even
# though the template no longer ships a scaffold (LS-5). database/lifekit.db
# left the list -- a non-lifekit mint has none (S4 asserts absence).
KEEP_FILES="AGENT.md USER.md config/agent.conf config/lifekit.conf config/secret-patterns.conf .telegram_bot/.env"
BEFORE=""
...
# LS-5: no lifekit.db on a non-lifekit mint -- update.sh must not create one.
[ -e "$LIFEKIT_DB" ] && fail "update.sh created lifekit.db on a non-lifekit instance"
ok "lifekit.db still absent after update (non-lifekit instance)"
