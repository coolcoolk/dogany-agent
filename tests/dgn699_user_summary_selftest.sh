...
# DGN-822: html_esc removed from the mirrored build -- self-update.sh now
# passes notice bodies RAW; push.sh's bridge sanitizer escapes entities.

# Helper: simulate the DGN-785/DGN-829 three-section notice body build for a
# given releasefile + lang. Returns the blockquote body text (without the
# header line and without the outer <blockquote> tags) so tests can grep it.
# DGN-829: label format is "<glyph> <text>"; uses same DOGANY_SECTION_GLYPHS
# parsing logic as self-update.sh (space-split, 3-token positional).
build_notice_body() {  # <releasefile> <lang>
  _bnb_file="$1"
  _bnb_lang="$2"
  # DGN-829: parse DOGANY_SECTION_GLYPHS (same logic as self-update.sh)
  _bnb_sg_raw="${DOGANY_SECTION_GLYPHS:-}"
  _bnb_sg1="" _bnb_sg2="" _bnb_sg3=""
  if [ -n "$_bnb_sg_raw" ]; then
    _bnb_parsed="$(printf '%s\n' "$_bnb_sg_raw" | tr -s '[:space:]' '\n' | grep -v '^$' | head -3)"
    _bnb_sg1="$(printf '%s\n' "$_bnb_parsed" | sed -n '1p')"
    _bnb_sg2="$(printf '%s\n' "$_bnb_parsed" | sed -n '2p')"
    _bnb_sg3="$(printf '%s\n' "$_bnb_parsed" | sed -n '3p')"
  fi
  [ -z "$_bnb_sg1" ] && _bnb_sg1="✅"
  [ -z "$_bnb_sg2" ] && _bnb_sg2="📌"
  [ -z "$_bnb_sg3" ] && _bnb_sg3="📋"
  if [ "$_bnb_lang" = "ko" ]; then
    _bnb_ls="${_bnb_sg1} 업데이트 요약"
    _bnb_ld="${_bnb_sg2} 업데이트 상세"
    _bnb_lt="${_bnb_sg3} 새로 해볼 수 있는 것"
  else
    _bnb_ls="${_bnb_sg1} At a glance"
    _bnb_ld="${_bnb_sg2} What changed"
    _bnb_lt="${_bnb_sg3} Try this"
  fi
...
# ===========================================================================
CURRENT="T9"; say "T9: all three glyph labels present when all keys populated (DGN-829)"
OUT="$(build_notice_body "$THREE_KEYS" ko)"
# DGN-829: labels now "✅ 업데이트 요약" / "📌 업데이트 상세" / "📋 새로 해볼 수 있는 것"
if printf '%s' "$OUT" | grep -q '업데이트 요약'; then
  ok "ko summary label present (glyph + text)"
else
  bad "ko summary label missing: [$OUT]"
fi
if printf '%s' "$OUT" | grep -q '업데이트 상세'; then
  ok "ko detail label present (glyph + text)"
else
  bad "ko detail label missing: [$OUT]"
fi
if printf '%s' "$OUT" | grep -q '새로 해볼 수 있는 것'; then
  ok "ko try label present (glyph + text)"
else
...
# DGN-829: check text portion of the label (glyph-agnostic)
if printf '%s' "$OUT" | grep -q '새로 해볼 수 있는 것'; then
  bad "try label should be absent when ko_try key missing: [$OUT]"
else
  ok "ko_try absent -> third section omitted"
fi
if printf '%s' "$OUT" | grep -q '업데이트 요약' \
   && printf '%s' "$OUT" | grep -q '업데이트 상세'; then
