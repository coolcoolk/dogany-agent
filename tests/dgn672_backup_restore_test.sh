...
make_sandbox() {
  ...
  cp "$REPO_ROOT/database/lifekit.py" "$REPO_ROOT/database/lifekit.sh" \
     "$REPO_ROOT/database/schema.sql" "$REPO_ROOT/database/restore-data.sh" \
     "$sb/database/"
...
grep -q '^PRAGMA user_version=11;' "$SB/database/lifekit.sql"
...
[[ "$(db_ver "$SB/database/lifekit.db")" == "11" ]]
...
BAK="$(ls -t "$SB/database"/lifekit.db.v11.bak-* 2>/dev/null | head -1)"
...
BAK2="$(ls -t "$SB/database"/lifekit.db.v11.bak-* 2>/dev/null | head -1)"
...
"$SB/database/restore-data.sh" --to latest --target-version 11 >/dev/null 2>&1
check "T6 --target-version 11 proceeds" $?
[[ "$(db_ver "$SB/database/lifekit.db")" == "11" ]]
check "T6 asserted version landed" $?

# (b) real catch-up apply: snapshot at v11, instance moved to EXPECTED=12
SB="$(make_sandbox 1)"
sed -i '' -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 12/' \
  "$SB/database/lifekit.py" 2>/dev/null \
  || sed -i -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 12/' \
       "$SB/database/lifekit.py"
cat > "$SB/database/migrations/012_catchup_fixture.sql" <<'EOF'
BEGIN;
CREATE TABLE catchup_t (id INTEGER PRIMARY KEY);
PRAGMA user_version = 12;
COMMIT;
EOF
"$SB/routines/backup-data.sh" >/dev/null 2>&1        # snapshot carries 11
"$SQLITE" "$SB/database/lifekit.db" < "$SB/database/migrations/012_catchup_fixture.sql"
"$SB/database/lifekit.sh" task-add "t6b post-snapshot task" 2026-08-01 >/dev/null
"$SB/database/restore-data.sh" --to latest >/dev/null 2>&1
check "T6 catch-up restore succeeds (11 -> 12)" $?
[[ "$(db_ver "$SB/database/lifekit.db")" == "12" ]]
...
SB="$(make_sandbox 1)"
sed -i '' -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 13/' \
  "$SB/database/lifekit.py" 2>/dev/null \
  || sed -i -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 13/' \
       "$SB/database/lifekit.py"
cat > "$SB/database/migrations/011_skew_a.sql" <<'EOF'
BEGIN;
CREATE TABLE skew_a (id INTEGER PRIMARY KEY);
PRAGMA user_version = 12;
COMMIT;
EOF
cat > "$SB/database/migrations/012_skew_b.sql" <<'EOF'
BEGIN;
CREATE TABLE skew_b (id INTEGER PRIMARY KEY);
PRAGMA user_version = 13;
COMMIT;
EOF
# live lineage: 011 applied (v12), snapshot taken at v12
"$SQLITE" "$SB/database/lifekit.db" < "$SB/database/migrations/011_skew_a.sql"
"$SQLITE" "$SB/database/lifekit.db" < "$SB/database/migrations/012_skew_b.sql"
git -C "$SB" rm -q --cached . -r >/dev/null 2>&1; git -C "$SB" add -A; git -C "$SB" commit -qm wip >/dev/null 2>&1
# craft the mid-version candidate: dump of a v12 DB (skew: 012 sets 13, NNN=12
# is not > cur=12 -> silently skipped by an NNN-compare -> achieved gate STOP)
"$SQLITE" "$SB/database/lifekit.db" "PRAGMA user_version = 12; DROP TABLE skew_b;" 2>/dev/null
"$SB/routines/backup-data.sh" >/dev/null 2>&1        # snapshot carries 12
"$SQLITE" "$SB/database/lifekit.db" < "$SB/database/migrations/012_skew_b.sql"  # live back to 13
quiesce "$SB/database/lifekit.db"
LIVE_MD5="$(file_md5 "$SB/database/lifekit.db")"
OUT="$("$SB/database/restore-data.sh" --to latest 2>&1)"
RC=$?
...
BAKF="$(ls -t "$SB/database"/lifekit.db.v13.bak-* 2>/dev/null | head -1)"
if [[ -z "$BAKF" ]]; then
  "$SQLITE" "$SB/database/lifekit.db" ".backup '$SB/database/lifekit.db.v13.bak-manual'"
  BAKF="$SB/database/lifekit.db.v13.bak-manual"
fi
# make an OLDER binary snapshot (v12) to prove no-catchup does not migrate it
"$SQLITE" "$SB/database/lifekit.db" "PRAGMA user_version = 12;"
"$SQLITE" "$SB/database/lifekit.db" ".backup '$SB/database/lifekit.db.v12.bak-fixture'"
"$SQLITE" "$SB/database/lifekit.db" "PRAGMA user_version = 13;"
"$SB/database/restore-data.sh" --to "$SB/database/lifekit.db.v12.bak-fixture" --no-catchup >/dev/null 2>&1
...
sed -i '' -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 13/' \
  ...
  || sed -i -e 's/^EXPECTED_USER_VERSION = 11/EXPECTED_USER_VERSION = 13/' \
       "$SB/database/lifekit.py"
cat > "$SB/database/migrations/012_gap_only.sql" <<'EOF'
BEGIN;
...
RC=$?
[[ "$RC" -eq 1 ]] && echo "$OUT" | grep -q "STOP"
check "T6 anomaly gap (12 != EXPECTED 13): STOP" $?
[[ "$(file_md5 "$SB/database/lifekit.db")" == "$LIVE_MD5" ]]
