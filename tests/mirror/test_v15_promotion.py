...
def _import_adapter(root):
    ...
    for name in ("adapter", "sdk_bridge", "notify", "http_direct",
                 "mirror_i18n", "mirror_colors", "reconcile"):
...
def _import_notify(root):
    for name in ("notify", "mirror_i18n", "mirror_colors"):
        sys.modules.pop(name, None)
    sys.path.insert(0, os.path.join(root, "mirror"))
...
def test_drift_guard():
    ...
    # (d) DGN-654 scalar floor form (MIN_USER_VERSION = N) parses, and it
    # WINS over a legacy tuple left in the same file.
    floor_pin = os.path.join(d, "floor_pin.py")
    with open(floor_pin, "w") as fh:
        fh.write("MIN_USER_VERSION = 11  # forward-tolerant floor\n")
        fh.write("ALLOWED_USER_VERSIONS = (7, 8)  # stale legacy line\n")
    r = _run_guard_snippet('extract_ver_sdk_bridge_py "%s"' % floor_pin)
    _check("(d) MIN_USER_VERSION scalar parsed (11), wins over legacy",
           r.stdout.strip() == "11", (r.returncode, r.stdout, r.stderr))
    # ... guard engages across forms: instance legacy (8,9) vs fw floor 11
    # -> instance NOT ahead -> PROCEED.
    r = _run_guard_snippet(
        'if drift_guard_file "mirror/sdk_bridge.py" "%s" "%s" '
        '"sdk_bridge_py"; then echo PROCEED; else echo SKIP; fi'
        % (floor_pin, ahead_pin))
    _check("(d) mixed-form compare (fw floor 11 vs instance max 9): PROCEED",
           r.stdout.strip().endswith("PROCEED"),
           (r.returncode, r.stdout, r.stderr))

    # Regression pin: canonical mirror/sdk_bridge.py's live version gate is
    # parseable by the extractor (the shipped guard can engage). DGN-654:
    # the gate is the forward-tolerant floor MIN_USER_VERSION = 11.
    r = _run_guard_snippet('extract_ver_sdk_bridge_py "%s"'
                           % os.path.join(SRC_MIRROR, "sdk_bridge.py"))
    _check("canonical sdk_bridge.py version gate parses (floor=11)",
           r.stdout.strip() == "11", (r.returncode, r.stdout, r.stderr))
