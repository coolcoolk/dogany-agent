...
mint_stub() { # mint_stub <root> <slug>
  ...
  # FATAL-1 fix (DGN-466): copy the REAL template agent.conf so the stub
  # mirrors what mint.sh actually ships (mint scaffolds agent.conf verbatim
  # from the template, only rewriting AGENT_LANG). The old stub wrote just
  # AGENT_LANG=ko -- no other lines -- which HID FATAL-1: the shipped
  # template's KIT= state (present vs absent) is exactly what decides whether
  # the dgn227 KIT anchor's write-if-absent can seed by class. Keep
  # AGENT_LANG=ko for the ko-path assertions by rewriting that one line.
  cp "$tpl/config/agent.conf" "$root/config/agent.conf"
  sed -i.bak 's/^AGENT_LANG=.*/AGENT_LANG=ko/' "$root/config/agent.conf" \
    && rm -f "$root/config/agent.conf.bak"
...
assert "template ships no KIT= value line (FATAL-1 / OQ-B)" \
  bash -c "! grep -q '^KIT=' '$SANDBOX/agents/.template/config/agent.conf'"
assert "KIT=pending anchored (main, no DOGANY_KIT preset -- OQ-B tristate)" \
  grep -qx "KIT=pending" "$R1_ROOT/config/agent.conf"
...
# BK-1 rider (DGN-783): DOGANY_KIT whitelist -- bogus value must fall back to pending.
# A typo like "lifkit" must NOT be written; it would freeze the instance kitless (OQ-B).
H1W="$(mktemp -d /tmp/dgn227-r1w.XXXXXX)"
R1W_ROOT="$H1W/.dogany/main"
mint_stub "$R1W_ROOT" "dogany"
run_flow "$H1W" '
  DOGANY_KIT=boguskit
  step_agent_class
  dgn227_postmint "'"$R1W_ROOT"'"
  write_lite_marker "'"$R1W_ROOT"'"
' || bad "R1w (bogus kit) flow exited non-zero (see $H1W/flow.log)"
assert "DOGANY_KIT=bogus -> KIT=pending fallback, not written (BK-1 whitelist)" \
  grep -qx "KIT=pending" "$R1W_ROOT/config/agent.conf"
...
assert "KIT=none anchored (domain -- kitless by construction, never pending)" \
  grep -qx "KIT=none" "$R2_ROOT/config/agent.conf"
assert "LIFEKIT=off" \
...
run_flow "$H2" '
  ...
  check_single_agent "'"$R4_MAIN"'" || { echo "single-agent check refused"; exit 10; }
...
# DGN-803 LS-5: bundle daily-retro.sh left the template with the lifekit pack;
# assert against the pack payload copy (sibling checkout) when the template
# copy is gone. Neither present -> explicit skip (pack checkout absent).
DAILY_RETRO_SH="$SANDBOX/agents/.template/routines/bundle/daily-retro.sh"
[ -f "$DAILY_RETRO_SH" ] || \
  DAILY_RETRO_SH="${DOGANY_LIFEKIT_PAYLOAD:-$SANDBOX/../dogany-lifekit/payload}/routines/bundle/daily-retro.sh"
if [ -f "$DAILY_RETRO_SH" ]; then
  assert "14i: daily-retro.sh no longer hardcodes the '워그 건강 리포트 미도착' literal" \
    bash -c "! grep -q '워그 건강 리포트 미도착' '$DAILY_RETRO_SH'"
  assert "14i: daily-retro.sh parameterizes the health-peer name (config-driven)" \
    grep -q 'RETRO_HEALTH_PEER_NAME' "$DAILY_RETRO_SH"
else
  say "  skip: 14i daily-retro.sh not found (lifekit pack checkout absent -- DGN-803 LS-5)"
fi
...
run_flow "$H15" '
  ...
  check_single_agent "'"$R15_DOM"'" || { echo refused; exit 11; }
