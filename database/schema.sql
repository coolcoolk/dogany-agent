...
CREATE TABLE IF NOT EXISTS event (
    ...
    -- DGN-274 travel blocks (migration 008, folded here for fresh DBs).
    -- sqlite ALTER cannot add CHECK -- SDK enforces value sets (006/008
    -- precedent), so no belt CHECKs here either (parity with migrated DBs).
    block_class    TEXT,                          -- 'travel' | NULL
    derived_from   TEXT,                          -- anchor event ulid
    derived_role   TEXT,                          -- 'before' | 'after'
    derived_pinned INTEGER NOT NULL DEFAULT 0,    -- user pin
    derived_delta  TEXT,                          -- JSON pin snapshot
