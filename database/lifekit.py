...
# ── day-close (DGN-669 aggregate finality) ─────────────────
# closed(D) := now_local >= max(last_entry_at(D) + QUIET_HOURS,
#                               D+1 GRACE_TIME local floor)
# Derived at read time from created_at (schema default localtime) -- no stored
# close marker; a late backfill insert re-opens the day automatically.
# Constants (not config keys) by design: get_config reads the JSON
# _CONFIG_STORE, so a config key is not free here; promote only if a second
# instance needs different values (DGN-669 grill MINOR4).
DAY_CLOSE_QUIET_HOURS = 6
DAY_CLOSE_GRACE_TIME = datetime.time(12, 0)   # next-day noon floor (local)
...
# ── 집계 ───────────────────────────────────────────────────
def _day_last_entry_at(date, conn):
    """MAX(created_at) over meals+workouts rows with date=D; None if no rows.

    created_at is fixed-width 'YYYY-MM-DD HH:MM:SS' localtime text (schema
    default datetime('now','localtime')), so lexicographic MAX is
    chronologically sound.

    Known limitation (DGN-669 MAJOR1, spun-out ticket): only inserts and
    date-move-ins are detected. meal_del / workout_del and meal_upd
    date-move-out mutate a day's numbers WITHOUT advancing MAX(created_at)
    (a delete can even move it backward), so those mutations to an
    already-closed day are undetected -- the day stays closed with changed
    numbers. Full coverage needs a touch-log/tombstone (separate ticket);
    migration 012 (updated_at) would cover updates only."""
    row = conn.execute(
        "SELECT MAX(created_at) FROM ("
        "SELECT created_at FROM meals WHERE date=? "
        "UNION ALL SELECT created_at FROM workouts WHERE date=?);",
        (date, date)).fetchone()
    return row[0] if row and row[0] else None


def _day_close_time(date, last_entry_at):
    """Local datetime at which day D closes (DGN-669 hybrid rule):
    max(last_entry_at + DAY_CLOSE_QUIET_HOURS, D+1 DAY_CLOSE_GRACE_TIME).
    No entries -> the noon floor alone governs."""
    floor = datetime.datetime.combine(
        datetime.date.fromisoformat(date) + datetime.timedelta(days=1),
        DAY_CLOSE_GRACE_TIME)
    if not last_entry_at:
        return floor
    quiet = (datetime.datetime.strptime(last_entry_at, '%Y-%m-%d %H:%M:%S')
             + datetime.timedelta(hours=DAY_CLOSE_QUIET_HOURS))
    return max(floor, quiet)


def day_closed(date, conn=None, now=None):
    """DGN-669: True if day D is closed (aggregate final). now is injectable
    for tests; defaults to datetime.now() (localtime -- created_at and the
    KST date column are both local, so the comparison is consistent).
    Today/future dates are always open (the D+1 floor is not yet reached)."""
    own = conn is None
    if own:
        conn = get_conn()
    try:
        last = _day_last_entry_at(date, conn)
    finally:
        if own:
            conn.close()
    if now is None:
        now = datetime.datetime.now()
    return now >= _day_close_time(date, last)


def agg_day(date, conn=None, now=None):
    """그날 섭취/매크로/소모/밸런스 dict (v_daily_energy 기반).

    DGN-669: additive finality keys as_of / last_entry_at / closed appended;
    all pre-existing keys unchanged (consumers parse ^key=). now is an
    internal-only injectable kwarg for tests."""
    own = conn is None
    if own:
        conn = get_conn()
    try:
        ...
        return {
            ...
            'last_entry_at': last,
            'closed': now >= _day_close_time(date, last),
        }
    finally:
        if own:
            conn.close()
...
def agg_week(monday, conn=None, now=None):
    """그 주(월~일) 집계 + 직전주 대비 dict. monday=월요일 YYYY-MM-DD.

    DGN-669: additive top-level 'as_of' + per-day 'closed' flag on daily rows
    (per-day only, no top-level closed). now injectable for tests."""
    own = conn is None
    if own:
        conn = get_conn()
    try:
        ...
        for i in range(7):
            ...
            daily.append({'date': d, 'dow': dow, 'intake': ik or 0,
                          ...
                          'closed': now >= _day_close_time(
                              d, _day_last_entry_at(d, conn))})
...
def spend_add(date, amount, name, category=None, method=None, scope='personal',
              note=None, is_fixed=None, source='chat', kind='expense',
              conn=None):
    """지출/수입 한 건 기록. 새 행 id 반환.

    is_fixed 미지정(None) 시 spend_category.is_fixed_default 상속.
    카테고리 미등록이면 auto-register (+ stderr 경고). scope는 verb 계층에서 검증.
    kind='expense'|'income' (DGN-564; verb 계층에서 검증). 수입은 amount 양수."""
    own = conn is None
    if own:
        conn = get_conn()
    try:
        ...
        cur = conn.execute(
            ...
            "(date, amount, name, category_id, method, is_fixed, scope, source, "
            "note, kind) "
            "VALUES (?,?,?,?,?,?,?,?,?,?);",
            (date, int(amount), name, cid, _txt(method), int(is_fixed),
             scope, source, _txt(note), kind))
        conn.commit()
        return cur.lastrowid
    finally:
        if own:
...
_SPEND_UPD_COLS = {
    ...
    'scope': _txt, 'source': _txt, 'kind': _txt,
    'category': None,  # handled specially
}
...
# DGN-564: kind value set; no DB CHECK (source precedent), SDK enforces here.
_SPEND_UPD_VALID_KIND = {'expense', 'income'}
...
def spend_upd(sid, fields, conn=None):
    ...
    try:
        ...
        for col, raw in fields.items():
            ...
            if col == 'kind' and raw not in _SPEND_UPD_VALID_KIND:
                raise ValueError(f"kind 값 오류: {raw} (expense/income)")
...
def spend_week(monday, scope_filter='personal', include_all=False, conn=None):
    ...
    try:
        ...
        # DGN-564: expense aggregates mean "spend" -> filter kind='expense'.
        # Income rides separate subtotal + net (income - expense) lines.
        # Current week totals (expense)
        row = conn.execute(
            f"SELECT COALESCE(SUM(e.amount),0), COUNT(*) "
            f"FROM spend_entry e WHERE e.date >= ? AND e.date < ? "
            f"AND e.kind='expense' {scope_clause};",
            (monday, nxt)).fetchone()
        total, count = row

        # Previous week totals (expense)
        ...
        # Category breakdown (current week, expense).
        ...
        cat_rows = conn.execute(
            ...
            f"WHERE e.date >= ? AND e.date < ? AND e.kind='expense' "
            f"{scope_clause} "
            f"GROUP BY e.category_id ORDER BY SUM(e.amount) DESC;",
            (monday, nxt)).fetchall()

        # Income subtotal (current week)
        inc_row = conn.execute(
            f"SELECT COALESCE(SUM(e.amount),0), COUNT(*) "
            f"FROM spend_entry e WHERE e.date >= ? AND e.date < ? "
            f"AND e.kind='income' {scope_clause};",
            (monday, nxt)).fetchone()
        income_total, income_count = inc_row
        ...
        return {
            ...
            'income_total': income_total, 'income_count': income_count,
...
def spend_month(ym, scope_filter='personal', include_all=False, conn=None):
    ...
    try:
        ...
        # Category breakdown (all categories, ordered by amount desc; expense).
        # m-3 (grill fix): NULL category_id renders as '(미분류)', not '기타'.
        cat_rows = conn.execute(
            f"SELECT COALESCE(c.name,'(미분류)'), COALESCE(SUM(e.amount),0), COUNT(*) "
            f"FROM spend_entry e "
        ...
        # Method breakdown (spend-month only, grill r1 m4; expense)
...
# ══════════════════════════════════════════════════════════════════════════
# DGN-579 B1: event_day -- THE day-view query (D2 determinism). Membership
# is a WHERE clause, never agent discretion: every DATED event touching the
# local date shows, INCLUDING kind='presence'. This is the primitive's
# visibility contract -- the forcing point that keeps reclassified presence
# rows reachable. Rich briefing rendering (cap 5/day, +N more) is 스컬-owned
# and NOT built here; this returns the raw partitioned rows.
# ══════════════════════════════════════════════════════════════════════════

def event_day(date, tz_name="Asia/Seoul", conn=None):
    """Return every dated event that touches local `date`, partitioned:
      {'band':     [presence rows]  -- background band (kind='presence'),
       'anchored': [anchored rows]  -- bars (movement rows marked block_class),
       'due':      [due rows]}      -- compact markers.
    floating (untimed backlog) is NEVER in the day view (backlog surface).

    Row tuple = (id, ulid, kind, title, start_local, end_local, schedule_kind,
    time_mode, block_class, place_name, status). Local columns rendered per
    the M-3 rule. Membership: settled_outcome != 'abandoned' AND the event's
    [start, eff_end) intersects the local day window [ws, we) -- the same
    half-open instant math the occupancy predicate uses (uniform, exact)."""
    ws, we = all_day_instants(date, tz_name=tz_name)
    own = conn is None
    if own:
        conn = event_conn()
    try:
        rows = conn.execute(
            "SELECT e.id, e.ulid, e.kind, e.title, e.start_at, e.end_at, "
            "e.schedule_kind, e.time_mode, e.block_class, e.display_tz, "
            "e.status, p.name FROM event e "
            "LEFT JOIN place p ON p.id = e.place_id "
            "WHERE (e.settled_outcome IS NULL "
            "       OR e.settled_outcome <> 'abandoned') "
            "AND e.start_at IS NOT NULL "
            "AND e.start_at < ? AND " + EFF_END_SQL + " > ? "
            "ORDER BY e.start_at;", (we, ws)).fetchall()
        out = {"band": [], "anchored": [], "due": []}
        for (eid, ulid, kind, title, sa, ea, sk, tm, bc, tz, st,
             pname) in rows:
            item = (eid, ulid, kind, title,
                    _render_local_col(sa, sk, tz), _render_local_col(ea, sk, tz),
                    sk, tm, bc, pname, st)
            if kind == "presence":
                out["band"].append(item)
            elif tm == "anchored":
                out["anchored"].append(item)
            elif tm == "due":
                out["due"].append(item)
            # any other shape (should not occur for a dated row) is dropped
            # from the day view by construction -- floating has no start.
        return out
    finally:
        if own:
            conn.close()
...
EXPECTED_USER_VERSION = 11  # 011_income_kind (DGN-564)
...
def resolve_slot_exclusive(kind, schedule_kind, task_kind=None, day_block=False,
    ...
    if block_class == "travel":
        # DGN-579 D-C1 (owner-locked): movement is derived background
        # context -- soft slot, renders as a band, NEVER blocks a write.
        # Overlap surfaces via the MOVEMENT_OVERLAP response contract
        # (B2b) instead of the slot guard. Flipped from 1 (DGN-274 4.2(a)).
        return 0
...
def _recompute_and_store(conn, eid, now=None):
    ...
    DGN-579 (spec A3): presence SKIPS derive_status -- it never expires or
    auto-completes; settled -> stored outcome verbatim, else 'open'. Liveness
    (span vs now) is a query-time concern, not a status."""
    row = conn.execute(
        "SELECT schedule_kind, start_at, end_at, open_ended, settled_at, "
        "settled_outcome, completion_rule, kind FROM event WHERE id=?;",
        (eid,)).fetchone()
    sk, sa, ea, oe, set_at, set_out, rule, kind = row
    if kind == "presence":
        st = set_out if set_at is not None else "open"
...
# ── DGN-579 presence predicate (one-presence-at-a-time, grill B6) ──────────
# Presence rows are slot_exclusive=0 by CHECK, so they are structurally
# OUTSIDE the slot predicate above. Their own invariant -- at most one live
# presence covering any instant -- gets its own scoped NOT-EXISTS, used by
# BOTH write paths (presence insert guard + presence_move CAS guard).
def _presence_overlap_not_exists(exclude_id=False):
    """WHERE NOT EXISTS(...) probe against live presence rows.
    Params (in order): cand_start, cand_eff_end [, exclude_id] -- same
    convention as _overlap_not_exists."""
    sql = ("NOT EXISTS (SELECT 1 FROM event e WHERE e.kind = 'presence' "
...
def _presence_overlapping(conn, cand_start, cand_eff_end, exclude_id=None):
    """Live presence rows intersecting [cand_start, cand_eff_end).
    Returns [(id, ulid, title, start_at, end_at, recurrence_id, block_class,
    version)] start-ordered. Read-side companion of the NOT-EXISTS guard
    (supersession classification + conflict listings)."""
    sql = ("SELECT e.id, e.ulid, e.title, e.start_at, e.end_at, "
           "e.recurrence_id, e.block_class, e.version FROM event e "
           "WHERE e.kind = 'presence' AND e.settled_at IS NULL "
           "AND ? < " + EFF_END_SQL + " AND e.start_at < ?")
    params = [cand_start, cand_eff_end]
    if exclude_id is not None:
        sql += " AND e.id != ?"
        params.append(exclude_id)
    return conn.execute(sql + " ORDER BY e.start_at;", params).fetchall()


# ── DGN-579 time_mode facet (single derivation locus, spec A1) ─────────────
# This function mirrors the migration 010 backfill CASE byte-for-byte so
# live writes and migrated rows agree. Order matters: exclusive rows (incl.
# hand-made day-blocks and pre-flip travel rows) are anchored ABOVE the
# all_day->due rule (grill D-6a).
TIME_MODES = ("anchored", "due", "floating")


def resolve_time_mode(kind, schedule_kind, slot_exclusive=0,
                      block_class=None, recurrence_id=None):
    """Deterministic time_mode default (spec A1 / migration 010 CASE mirror).
    presence -> None (presence carries no time_mode by CHECK)."""
    if kind == "presence":
        return None
    if schedule_kind == "untimed":
        return "floating"
    if slot_exclusive == 1:
        return "anchored"
    if kind == "appointment":
    ...
    if recurrence_id is not None:
        return "anchored"
    if kind == "task" and schedule_kind == "timed":
        return "anchored"
    if kind == "task" and schedule_kind == "all_day":
        return "due"
    return "due"


def validate_time_mode(time_mode, schedule_kind, start_at):
    """App-side mirror of the A1 time_mode CHECKs (loud, pre-INSERT).
    Applies to ACTIONABLE rows only (presence time_mode is None by contract).
    Raises ValueError on violation; returns time_mode."""
    if time_mode not in TIME_MODES:
        raise ValueError(
            "time_mode must be one of %s, got %r"
            % ("|".join(TIME_MODES), time_mode))
    if time_mode == "floating" and schedule_kind != "untimed":
        raise ValueError(
            "time_mode 'floating' requires schedule_kind 'untimed', got %r"
            % schedule_kind)
    if schedule_kind == "untimed" and time_mode != "floating":
        raise ValueError(
            "schedule_kind 'untimed' requires time_mode 'floating', got %r"
            % time_mode)
    if time_mode in ("anchored", "due") and start_at is None:
        raise ValueError(
            "time_mode %r requires a start instant" % time_mode)
    return time_mode
...
def event_add(conn, kind, title, schedule_kind, start_at=None, end_at=None,
    ...
    # DGN-579 presence gate (spec A3): presence is an event row with its own
    # invariants -- every violation is loud HERE and backed by table CHECKs.
    if kind == "presence":
        if time_mode is not None:
            raise ValueError(
                "event_add: presence carries no time_mode (got %r)"
                % (time_mode,))
        if place_id is None:
            raise ValueError("event_add: presence requires place_id")
        if schedule_kind not in ("timed", "all_day"):
            raise ValueError(
                "event_add: presence schedule_kind must be timed|all_day, "
                "got %r" % schedule_kind)
        if start_at is None or end_at is None or open_ended:
            raise ValueError(
                "event_add: presence span requires bounded start_at AND "
                "end_at (open-ended presence forbidden in v1 -- extend "
                "end_at instead)")
        # A4/App.A-17: presence never alerts; stamp silent unless overridden.
        if notify_policy is None and notify_lead_min is None:
    ...
    if kind == "presence":
        slot_exclusive = 0            # CHECK-pinned; never the slot predicate
    else:
        slot_exclusive = resolve_slot_exclusive(
            kind, schedule_kind, task_kind, day_block,
            exclusive_override=exclusive_override)
        # DGN-579: time_mode default derived deterministically when not
        # given (single locus resolve_time_mode -- migration CASE mirror).
        if time_mode is None:
            time_mode = resolve_time_mode(kind, schedule_kind, slot_exclusive,
                                          block_class=None,
                                          recurrence_id=recurrence_id)
        validate_time_mode(time_mode, schedule_kind, start_at)
    now = now_utc()
    ulid = new_ulid()
    ...
    cols = ("ulid, kind, title, note, area_id, schedule_kind, start_at, end_at, "
            ...
            "recurrence_id, rec_date, is_routine, time_mode, place_id, "
    ...
    base_vals = [ulid, kind, title, note, area_id, schedule_kind, start_at, end_at,
                 display_tz, open_ended, slot_exclusive, completion_rule,
                 owning_agent, created_by, notion_id,
                 recurrence_id, rec_date, 1 if is_routine else 0,
                 time_mode, place_id, now, now]
    ...
    try:
        # DGN-579 (grill B6): presence inserts through the presence-scoped
        # atomic guard (one-presence-at-a-time), NEVER the slot guard.
        if kind == "presence":
            cee = _cand_eff_end(start_at, end_at, open_ended)
            sql = ("INSERT INTO event (" + cols + ") SELECT " +
                   ",".join("?" for _ in base_vals) + " WHERE " +
                   _presence_overlap_not_exists(exclude_id=False))
        ...
        elif is_blocker(slot_exclusive, start_at):
            cee = _cand_eff_end(start_at, end_at, open_ended)
            sql = ("INSERT INTO event (" + cols + ") SELECT " +
                   ",".join("?" for _ in base_vals) + " WHERE " +
...
def _cas_slot_update(conn, eid, version, set_clause, set_params,
    ...
    overlap_sql (DGN-579): optional replacement NOT-EXISTS guard with the SAME
    param convention (cand_start, cand_eff_end, exclude_id). presence_move
    passes the presence-scoped predicate here; default = the slot predicate.
    """
    where_overlap = (" AND " + (overlap_sql or _overlap_not_exists(exclude_id=True))
...
def event_move(conn, eid, version, new_start, new_end, open_ended=0):
    ...
    try:
        row = conn.execute(
            "SELECT slot_exclusive, schedule_kind, kind FROM event WHERE id=?;",
            (eid,)).fetchone()
        if row is None:
            conn.commit()
            return MutationResult.NOT_FOUND
        slot_ex, sk, kind = row
        # DGN-579 (grill B6, EDIT path): a generic move on a presence row
        # would bypass the one-presence-at-a-time guard (presence is slot 0).
        # Loud refusal; the presence-scoped CAS verb is presence_move.
        if kind == "presence":
            raise ValueError(
                "event_move refuses kind='presence' rows -- use "
                "presence_move (presence-scoped overlap guard)")
...
def presence_move(conn, eid, version, new_start, new_end):
    """DGN-579 (grill B6 EDIT-path fix): move a presence span under version
    CAS + the presence-scoped one-at-a-time guard (self-excluded). Same
    MutationResult contract as event_move (APPLIED / CAS_FAIL /
    OVERLAP_REJECT / NOT_FOUND). Both endpoints REQUIRED (bounded end is
    CHECK-pinned; open-ended presence forbidden)."""
    if new_start is None or new_end is None:
        raise ValueError(
            "presence_move: presence span requires bounded start AND end")
    canonical(new_start)
    canonical(new_end)
    ...
    try:
        row = conn.execute(
            "SELECT kind, schedule_kind FROM event WHERE id=?;",
            (eid,)).fetchone()
        if row is None:
            conn.commit()
            return MutationResult.NOT_FOUND
        ...
        if kind != "presence":
            raise ValueError(
                "presence_move refuses kind=%r rows -- use event_move" % kind)
        validate_all_day_instants(sk, new_start, new_end)
        cee = _cand_eff_end(new_start, new_end, 0)
        res = _cas_slot_update(
            conn, eid, version,
            "start_at=?, end_at=?",
            [new_start, new_end],
            new_start, cee, True,
            overlap_sql=_presence_overlap_not_exists(exclude_id=True))
        conn.commit()
        return res
    except Exception:
        conn.rollback()
...
def event_transition_schedule_kind(conn, eid, version, new_schedule_kind,
    ...
    try:
        row = conn.execute(
            "SELECT kind, block_class, recurrence_id FROM event WHERE id=?;",
            (eid,)).fetchone()
        if row is None:
            conn.commit()
            return MutationResult.NOT_FOUND
        kind, block_class, rec_id = row
        # DGN-579: presence has exactly two schedule kinds and its own move
        # verb; schedule-kind transitions on presence are refused loudly.
        if kind == "presence":
            raise ValueError(
                "event_transition_schedule_kind refuses kind='presence' "
                "rows -- use presence_move")
        ...
        # DGN-579: time_mode follows the TARGET shape (same derivation locus
        # as event_add / migration 010 -- keeps the A1 coherence CHECKs true).
        new_time_mode = resolve_time_mode(kind, new_schedule_kind,
                                          new_exclusive, block_class, rec_id)
...
def event_promote_exclusive(conn, eid, version):
    ...
    try:
        row = conn.execute(
            "SELECT schedule_kind, start_at, end_at, open_ended, "
            "slot_exclusive, kind, block_class, recurrence_id "
            "FROM event WHERE id=?;", (eid,)).fetchone()
        if row is None:
            conn.commit()
            return MutationResult.NOT_FOUND
        sk, sa, ea, oe, cur_ex, kind, block_class, rec_id = row
        # DGN-579: presence is slot 0 by CHECK; promotion is meaningless.
        if kind == "presence":
            raise ValueError(
                "event_promote_exclusive refuses kind='presence' rows "
                "(presence is slot_exclusive=0 by CHECK)")
        # DGN-579: exclusive rows are anchored (resolve_time_mode slot branch).
        new_time_mode = resolve_time_mode(kind, sk, 1, block_class, rec_id)
...
def force_settle(conn, eid, version, settled_by, settled_at_ts=None):
    ...
    DGN-579 (spec A3): presence cannot be 'done' -- being somewhere is not a
    completion. Verb-level refusal here; the settled_outcome CHECK backs it
    at the schema layer. Supersession = cancel (abandoned)."""
    row = conn.execute("SELECT kind FROM event WHERE id=?;", (eid,)).fetchone()
    if row is not None and row[0] == "presence":
        raise ValueError(
            "force_settle refuses outcome 'done' on kind='presence' "
            "(presence settles only 'abandoned' -- supersession/cancel)")
    return _settle(conn, eid, version, "done", settled_by, settled_at_ts)


# ══════════════════════════════════════════════════════════════════════════
# DGN-579 unified event model: place + presence verbs (spec A2/A3).
# Mirror enqueue (owner D-C3 2026-07-26, reverses E1): SDK-layer verbs still
# never enqueue; the PRODUCER layer (presence CLI verbs + roller) enqueues
# post-commit so presence surfaces as a GCal all-day banner.
# ══════════════════════════════════════════════════════════════════════════

PRESENCE_SUPERSEDED_BY = "presence_supersession"


def place_upsert(conn, name, aliases=None):
    """DGN-579 A2 write path (generalization of the Ag travel_place verbs):
    exact name match -> alias match -> INSERT. The ONLY insert path for
    place rows; no confirm turn (D6 1-turn capture); no fuzzy merge in v1
    (App. A item 11). Returns (place_id, canonical_name)."""
    if not name or not str(name).strip():
        raise ValueError("place_upsert: name required")
    name = str(name).strip()
    row = conn.execute("SELECT id, name FROM place WHERE name=?;",
                       (name,)).fetchone()
    if row:
        return row[0], row[1]
    for pid, pname, al in conn.execute(
            "SELECT id, name, aliases FROM place "
            "WHERE aliases IS NOT NULL;").fetchall():
        try:
            arr = json.loads(al)
        except (ValueError, TypeError):
            continue
    ...
    try:
        conn.execute(
            "INSERT INTO place (ulid, name, aliases, created_at, updated_at) "
            "VALUES (?,?,?,?,?);",
            (new_ulid(), name,
             json.dumps(list(aliases), ensure_ascii=False) if aliases else None,
             ts, ts))
        pid = conn.execute("SELECT id FROM place WHERE name=?;",
                           (name,)).fetchone()[0]
        conn.commit()
        return pid, name
    except Exception:
...
def place_set_base(conn, place_id):
    """DGN-579 A2 base relocation: old base row -> 0, new row -> 1, ONE txn.
    No history rewrite (App. A item 18). idx_place_base (partial unique)
    mechanically guarantees the single base. SDK enforces is_base in (0,1)
    (008 'SDK enforces' precedent -- ALTER cannot add CHECK)."""
    row = conn.execute("SELECT id FROM place WHERE id=?;",
                       (int(place_id),)).fetchone()
    if row is None:
        raise ValueError("place_set_base: no place id %r" % place_id)
    ts = now_utc()
    conn.execute("BEGIN IMMEDIATE;")
    try:
        conn.execute(
            "UPDATE place SET is_base=0, updated_at=? WHERE is_base=1;",
            (ts,))
        conn.execute(
            "UPDATE place SET is_base=1, updated_at=? WHERE id=?;",
            (ts, int(place_id)))
        conn.commit()
    except Exception:
        conn.rollback()
...
def _presence_provenance_split(rows):
    """Split _presence_overlapping rows into (routine_generated,
    user_authored) per the derived provenance facet: routine_generated :=
    recurrence_id IS NOT NULL OR block_class IS NOT NULL."""
    routine, user = [], []
    for r in rows:
        (routine if (r[5] is not None or r[6] is not None) else user).append(r)
    return routine, user


def presence_add(conn, title, place_id, start_at, end_at,
                 schedule_kind="timed", owning_agent=None, created_by=None,
                 note=None, display_tz="Asia/Seoul"):
    """DGN-579 A3: register a USER-AUTHORED presence span (roller-emitted
    presence goes through event_add directly in materialize_def).

    Overlap policy (dossier 2.2/4.3, all inside this verb):
      - existing routine_generated presence -> superseded: settled
        'abandoned' (by=presence_supersession) + its live UNPINNED derived
        rows settled in the same txn; then insert.
      - existing user_authored presence -> PROMPT: no insert; the blocker
        rows come back as a structured conflict listing.
    Returns a dict:
      {'status': 'inserted', 'eid': int, 'ulid': str,
       'superseded': [ulid, ...]}                        on success
      {'status': 'conflict', 'rows': [presence rows]}    on equal-provenance
    Never enqueues to the mirror outbox itself -- the CLI/roller producer
    layer enqueues post-commit (D-C3)."""
    if start_at is None or end_at is None:
        raise ValueError(
            "presence_add: presence span requires bounded start AND end")
    if owning_agent is None or created_by is None:
        raise ValueError("owning_agent and created_by are required")
    canonical(start_at)
    canonical(end_at)
    ...
    if schedule_kind not in ("timed", "all_day"):
        raise ValueError(
            "presence_add: schedule_kind must be timed|all_day, got %r"
            % schedule_kind)
    if place_id is None:
        raise ValueError("presence_add: place_id required")
    cee = _cand_eff_end(start_at, end_at, 0)
    # D4 fix: supersession-settle AND the guarded INSERT run in ONE
    # BEGIN IMMEDIATE. If the insert guard loses (a concurrent live presence
    # appeared post-probe), the whole txn rolls back -- supersession never
    # commits without the replacement presence landing. No two-txn window.
    overlaps = _presence_overlapping(conn, start_at, cee)
    routine_rows, user_rows = _presence_provenance_split(overlaps)
    if user_rows:
        return {"status": "conflict", "rows": user_rows}
    now = now_utc()
    ulid = new_ulid()
    superseded = []
    conn.execute("BEGIN IMMEDIATE;")
    try:
        for (bid, bulid, _t, _sa, _ea, _rid, _bc, bver) in routine_rows:
            cur = conn.execute(
                "UPDATE event SET settled_at=?, settled_by=?, "
                "settled_outcome='abandoned', version=version+1, "
                "updated_at=? WHERE id=? AND version=? "
                "AND settled_at IS NULL;",
                (now, PRESENCE_SUPERSEDED_BY, now, bid, bver))
            if cur.rowcount != 1:
                continue        # CAS moved under us -> insert guard re-checks
            _recompute_and_store(conn, bid, now=now)
            superseded.append(bulid)
            # live UNPINNED derived rows ride the supersession (A3).
            for (did,) in conn.execute(
                    "SELECT id FROM event WHERE derived_from=? "
                    "AND settled_at IS NULL AND derived_pinned=0;",
                    (bulid,)).fetchall():
                conn.execute(
                    "UPDATE event SET settled_at=?, settled_by=?, "
                    "settled_outcome='abandoned', version=version+1, "
                    "updated_at=? WHERE id=? AND settled_at IS NULL;",
                    (now, PRESENCE_SUPERSEDED_BY, now, did))
                _recompute_and_store(conn, did, now=now)
        # atomic presence INSERT under the presence-scoped guard (the
        # just-superseded rows now carry settled_at, so they drop out of
        # the guard predicate -- only a NON-superseded live presence blocks).
        cols = ("ulid, kind, title, note, schedule_kind, start_at, end_at, "
                "display_tz, open_ended, slot_exclusive, completion_rule, "
                "owning_agent, created_by, place_id, notify_policy, "
                "time_mode, created_at, updated_at")
        vals = [ulid, "presence", title, note, schedule_kind, start_at,
                end_at, display_tz, 0, 0, "manual", owning_agent, created_by,
                place_id, "silent", None, now, now]
        sql = ("INSERT INTO event (" + cols + ") SELECT " +
               ",".join("?" for _ in vals) + " WHERE " +
               _presence_overlap_not_exists(exclude_id=False))
        ins = conn.execute(sql, vals + [start_at, cee])
        if ins.rowcount != 1:
            conn.rollback()   # insert guard lost -> supersession rolled back too
            return {"status": "conflict",
                    "rows": _presence_overlapping(conn, start_at, cee)}
        ...
        conn.rollback()
        raise
    return {"status": "inserted", "eid": eid, "ulid": ulid,
            "superseded": superseded}


def presence_at(conn, at_ts):
    """DGN-579 A3: the live presence row covering at_ts (at most one by the
    guarded invariant) ELSE the base place row. Returns
    ('presence', place_id, event_row_id) or ('base', place_id, None) or
    (None, None, None) when no base is seeded."""
    canonical(at_ts)
    row = conn.execute(
        "SELECT e.id, e.place_id FROM event e "
        "WHERE e.kind='presence' AND e.settled_at IS NULL "
        "AND e.start_at <= ? AND ? < " + EFF_END_SQL + " LIMIT 1;",
        (at_ts, at_ts)).fetchone()
    if row:
        return "presence", row[1], row[0]
    base = conn.execute(
        "SELECT id FROM place WHERE is_base=1;").fetchone()
    if base:
...
def unsettle(conn, ulid, by):
    ...
    eid, version = found
    row = conn.execute(
        "SELECT settled_outcome, kind FROM event WHERE id=?;", (eid,)).fetchone()
    if row is None:
        return "not_found", None
    outcome, kind = row
    # DGN-579 (spec A3): presence settles only 'abandoned' (supersession);
    # un-supersede therefore revives 'abandoned' -- but ONLY through the
    # one-presence-at-a-time guard (a revival that re-overlaps the live
    # presence timeline is refused, not silently applied). Actionable rows
    # keep the done-only contract byte-for-byte.
    guard_sql = ""
    guard_params = []
    if kind == "presence":
        if outcome != "abandoned":
            return "refused", None
        srow = conn.execute(
            "SELECT start_at, end_at, open_ended FROM event WHERE id=?;",
            (eid,)).fetchone()
        cee = _cand_eff_end(srow[0], srow[1], srow[2])
        if _presence_overlapping(conn, srow[0], cee, exclude_id=eid):
            return "refused", None   # would violate one-presence-at-a-time
        # atomic belt: same guard inside the UPDATE (race-safe).
        guard_sql = " AND " + _presence_overlap_not_exists(exclude_id=True)
        guard_params = [srow[0], cee, eid]
        revive_outcome = "abandoned"
    elif outcome != "done":
    ...
    def run():
        ...
        try:
            cur = conn.execute(
                ...
                "WHERE id=? AND version=? AND settled_outcome=?" +
                guard_sql + ";",
                [now, eid, version, revive_outcome] + guard_params)
            if cur.rowcount == 1:
                _recompute_and_store(conn, eid, now=now)
                conn.commit()
                return "applied"
...
def bypass_schedule_apply(conn, ulid, new_schedule_kind, new_start, new_end):
    ...
    if travel_ready:
        rec_row = conn.execute(
            ...
            "end_at, kind, slot_exclusive FROM event WHERE id=?;",
            (eid,)).fetchone()
        is_travel_block = rec_row is not None and rec_row[1] == "travel"
        old_sched = (rec_row[2], rec_row[3], rec_row[4]) if rec_row else None
        row_kind = rec_row[5] if rec_row else None
        row_slot_ex = rec_row[6] if rec_row else None
        row_block = rec_row[1] if rec_row else None
    else:
        rec_row = conn.execute(
            "SELECT recurrence_id, kind, slot_exclusive "
            "FROM event WHERE id=?;",
            (eid,)).fetchone()
        is_travel_block = False
        old_sched = None
        row_kind = rec_row[1] if rec_row else None
        row_slot_ex = rec_row[2] if rec_row else None
        row_block = None
    is_recurrence = rec_row is not None and rec_row[0] is not None
    # DGN-579: presence never reaches the mirror (E1-E3), so this inbound
    # lane cannot legitimately address it. Terminal classification (row
    # unchanged), same handling class as rejected_untimed.
    if row_kind == "presence":
        return "rejected_presence", None
    ...
    # DGN-579: time_mode follows the applied shape (single derivation locus;
    # keeps the A1 coherence CHECKs true on the bypass lane too).
    new_time_mode = resolve_time_mode(row_kind, new_schedule_kind,
                                      row_slot_ex, row_block,
                                      rec_row[0] if rec_row else None)
    ...
    def run():
        ...
        try:
            cur = conn.execute(
                ...
                + stamp_sql +
                ", time_mode=?, version=version+1, updated_at=? "
                "WHERE id=? AND version=?;",
                (new_schedule_kind, new_start, new_end,
                 new_end, new_schedule_kind, new_time_mode, now, eid, version))
            if cur.rowcount == 1:
                _recompute_and_store(conn, eid, now=now)
                conn.commit()
                return "applied"
...
def bypass_event_add(conn, kind, title, schedule_kind, start_at, end_at,
    ...
    # DGN-579: the force-adopt lane exists for owner-made SURFACE entries;
    # presence never has a surface (E1-E3) and must go through the guarded
    # presence path (event_add / presence_add).
    if kind == "presence":
        raise ValueError(
            "bypass_event_add refuses kind='presence' (presence never "
            "mirrors; use presence_add)")
    canonical(start_at)
    canonical(end_at)
    validate_interval(start_at, end_at)
    ...
    cols = ("ulid, kind, title, note, schedule_kind, start_at, end_at, "
            "display_tz, open_ended, slot_exclusive, completion_rule, "
            "time_mode, owning_agent, created_by, created_at, updated_at")
    vals = [ulid, kind, title, note, schedule_kind, start_at, end_at,
            "Asia/Seoul", 0, slot_exclusive, "all",
            time_mode, owning_agent, created_by, now, now]
...
def _travel_expected_exclusive(conn, rule, anchor, bs, be):
    """DGN-579 D-C1 (owner-locked): movement blocks are ALWAYS soft
    (slot_exclusive = 0). The DGN-274 3.6 demotion predicate (exclusive
    unless an external live exclusive blocker overlaps, with the seniority
    tie-break) is retired: movement is derived background context and never
    occupies the slot. Overlap visibility moved to the MOVEMENT_OVERLAP
    response contract (B2b). Signature kept so derive() call sites and the
    reconcile expected-value shape stay unchanged."""
    return 0
...
def _travel_create_block(conn, state_conn, anchor, role, start_at, end_at,
    ...
    conn.execute("BEGIN IMMEDIATE;")
    try:
        # DGN-579: movement rows are kind='task', time_mode='anchored'
        # (spec A4) and slot-soft (D-C1). The exclusive param is IGNORED
        # here (kept for call-site/carry shape stability): forcing 0 at
        # this SOLE mint locus makes an exclusive travel row structurally
        # impossible (pinned-carry lanes could still relay a pre-flip 1).
        exclusive = 0
        conn.execute(
            ...
            "derived_delta, time_mode, version, created_at, updated_at) "
...
# ══════════════════════════════════════════════════════════════════════════
# DGN-579 B2b: MOVEMENT_OVERLAP registration response contract (owner D-C1).
# Movement rows (block_class='travel') are slot_exclusive=0 (soft) -- they
# NEVER block a write. In exchange, every write that lands or moves a timed
# non-presence row MUST surface intersecting live movement rows in its
# response. Single choke point: _print_write_result below.
# ══════════════════════════════════════════════════════════════════════════

def movement_overlap(conn, start_at, eff_end, exclude_id=None):
    """Deterministic probe: live movement rows intersecting
    [start_at, eff_end). Returns [(ulid, title, start_at, end_at,
    derived_from, display_tz)] start-ordered."""
    sql = ("SELECT e.ulid, e.title, e.start_at, e.end_at, e.derived_from, "
           "e.display_tz FROM event e "
           "WHERE e.block_class = 'travel' AND e.settled_at IS NULL "
           "AND e.start_at IS NOT NULL "
           "AND e.start_at < ? AND ? < " + EFF_END_SQL +
           " AND (? IS NULL OR e.id != ?) ORDER BY e.start_at;")
    return conn.execute(sql, (eff_end, start_at,
                              exclude_id, exclude_id)).fetchall()


def _print_write_result(conn, eid, primary_line):
    """Shared CLI reporter (B2b, tool-enforced contract): (a) prints the
    verb's existing primary success line VERBATIM, then (b) probes the
    just-written span and, on n > 0 live movement rows, MUST append:

        MOVEMENT_OVERLAP <n>
        <ulid>\\t<title>\\t<start_local>\\t<end_local>\\t<derived_from>

    Exit code stays 0 (the write SUCCEEDED -- notice, not error). Scope:
    schedule_kind='timed', start_at NOT NULL, kind != 'presence'; every
    other row shape prints only the primary line (probe no-op)."""
    print(primary_line)
    row = conn.execute(
        "SELECT kind, schedule_kind, start_at, end_at, open_ended "
        "FROM event WHERE id=?;", (int(eid),)).fetchone()
    if row is None:
        return
    kind, sk, sa, ea, oe = row
    if kind == "presence" or sk != "timed" or sa is None:
        return
    hits = movement_overlap(conn, sa, _cand_eff_end(sa, ea, oe),
                            exclude_id=int(eid))
    if not hits:
        return
    print("MOVEMENT_OVERLAP %d" % len(hits))
    for (u, t, hsa, hea, dfrom, htz) in hits:
        print("%s\t%s\t%s\t%s\t%s" % (
            u, t,
            _render_local_col(hsa, "timed", htz or "Asia/Seoul"),
            _render_local_col(hea, "timed", htz or "Asia/Seoul"),
            dfrom or ""))


# ══════════════════════════════════════════════════════════════════════════
# DGN-579 B4: ONE reconcile probe over the whole event table (subsumes the
# DGN-263 per-verb probes). Tiers, first non-empty wins:
#   T1  same local date AND title EXACT   (any kind)   -- DGN-263 appt key
#   T2  title EXACT, open, any/no date    (any kind)
#   T3  title LIKE %title%, open          (non-presence kinds only, D-12)
# Candidates always exclude settled_outcome='abandoned'. The EXISTS listing
# + exit EXISTS_CODE=3 + --new bypass contract is preserved VERBATIM at the
# calling verbs.
# ══════════════════════════════════════════════════════════════════════════

_FIND_FIRST_COLS = ("id, ulid, kind, title, start_at, end_at, schedule_kind, "
                    "display_tz, settled_outcome, settled_at, version, "
                    "location")


def event_find_first(conn, title, date=None, tz_name="Asia/Seoul"):
    """Unified find-first probe. Returns (tier, rows) with tier in
    ('T1','T2','T3', None); rows are tuples of _FIND_FIRST_COLS."""
    live = "(settled_outcome IS NULL OR settled_outcome <> 'abandoned')"
    if date:
        ws, we = all_day_instants(date, tz_name=tz_name)
        rows = conn.execute(
            "SELECT " + _FIND_FIRST_COLS + " FROM event WHERE " + live +
            " AND title = ? AND start_at >= ? AND start_at < ? "
            "ORDER BY start_at;", (title, ws, we)).fetchall()
        if rows:
            return "T1", rows
    rows = conn.execute(
        "SELECT " + _FIND_FIRST_COLS + " FROM event WHERE " + live +
        " AND settled_at IS NULL AND title = ? "
        "ORDER BY (start_at IS NULL), start_at DESC;", (title,)).fetchall()
    if rows:
        return "T2", rows
    rows = conn.execute(
        "SELECT " + _FIND_FIRST_COLS + " FROM event WHERE " + live +
        " AND settled_at IS NULL AND kind != 'presence' AND title LIKE ? "
...
# DGN-579 B2/A3: structured conflict listing exit code (analog of the
# EXISTS contract): the write did NOT land; the blocker rows were printed.
CONFLICT_CODE = 4


def _ff_task_line(row):
    """Render an event_find_first row in the task-CLI line format."""
    (eid, _u, _k, title, sa, _ea, _sk, tz, outcome, _sat, _v, _loc) = row
    due = str(_local_date_of(sa, tz or "Asia/Seoul")) if sa else ""
    state = "done" if outcome == "done" else "todo"
    return f"{eid}\t{title}\t{due}\t{state}"


def _ff_appt_line(row):
    """Render an event_find_first row in the appt-CLI line format."""
    (eid, _u, _k, title, sa, _ea, sk, tz, _o, _sat, _v, loc) = row
    col2 = _render_local_col(sa, sk, tz or "Asia/Seoul") if sa else ""
    return f"{eid}\t{col2}\t{title}\t{loc or ''}"


def _exists_gate(conn, title, date, line_fn):
    """DGN-579 B4 shared add-verb gate: run the unified probe; on a hit,
    print the EXISTS listing (contract preserved verbatim: 'EXISTS n' +
    rows) and exit EXISTS_CODE without writing. Caller handles --new."""
    _tier, rows = event_find_first(conn, title, date=date)
    if rows:
        print(f"EXISTS {len(rows)}")
        for row in rows:
            print(line_fn(row))
        conn.close()
        sys.exit(EXISTS_CODE)
...
def exclusive_blockers(conn, cand_start, cand_eff_end, exclude_id=None):
    """DGN-579 B2 (locked decision 1): live exclusive actionable rows whose
    [start, eff_end) intersects the candidate span -- the rows the write lost
    the slot to. Companion listing of the _overlap_not_exists guard (same
    LIVE_FILTER + half-open math). Returns
    [(id, ulid, title, start_local, end_local, kind)] start-ordered."""
    sql = ("SELECT e.id, e.ulid, e.title, e.start_at, e.end_at, "
           "e.schedule_kind, e.display_tz, e.kind FROM event e WHERE " +
           LIVE_FILTER + " AND ? < " + EFF_END_SQL + " AND e.start_at < ?")
    params = [cand_start, cand_eff_end]
    if exclude_id is not None:
        sql += " AND e.id != ?"
        params.append(exclude_id)
    rows = conn.execute(sql + " ORDER BY e.start_at;", params).fetchall()
    out = []
    for (eid, ulid, title, sa, ea, sk, tz, kind) in rows:
        out.append((eid, ulid, title,
                    _render_local_col(sa, sk, tz or "Asia/Seoul"),
                    _render_local_col(ea, sk, tz or "Asia/Seoul"), kind))
    return out


def _print_conflict_and_exit(conn, cand_start, cand_eff_end, exclude_id=None):
    """DGN-579 B2: print the structured CONFLICT listing (the blocker rows
    the agent needs to PROMPT the user) and exit CONFLICT_CODE. Nothing was
    written -- distinct from EXISTS (pre-write dup) and from a bare error."""
    blockers = exclusive_blockers(conn, cand_start, cand_eff_end,
                                  exclude_id=exclude_id)
    print(f"CONFLICT {len(blockers)}")
    for (bid, bulid, btitle, bsl, bel, bkind) in blockers:
        print(f"{bid}\t{btitle}\t{bsl}\t{bel}\t{bkind}")
    conn.close()
...
def cli_agg_day(argv):
    ...
    # DGN-669 finality keys: strictly appended AFTER the 10 legacy lines
    # (consumers parse ^key=; legacy lines stay byte-identical).
    print(f"as_of={a['as_of']}")
    print(f"last_entry_at={a['last_entry_at'] or ''}")
    print(f"closed={1 if a['closed'] else 0}")
...
def cli_agg_week(argv):
    ...
    # DGN-669: as_of after the header line (new key; ^key= parsers unaffected).
    print(f"as_of={w['as_of']}")
    ...
    print("--- daily ---")
    for d in w['daily']:
        # DGN-669: daily rows are human-facing (not ^key= lines) -- marker
        # contract applied as a trailing [closed]/[prov] suffix.
        _fin = '[closed]' if d['closed'] else '[prov]'
...
def cli_dump(argv):
    ...
    DGN-672 (contract C4): the dump SELF-CARRIES the schema version. sqlite3's
    .dump never emits PRAGMA user_version, so a restored candidate would land
    at version 0 and any migration catch-up would have to guess. After a
    successful dump we print one trailing line
        PRAGMA user_version=N;
    (after the dump's COMMIT;, outside its transaction). Materializing the
    dump executes that line, so the candidate DB carries its version. N is
    read BEFORE the dump on a raw read-only URI connection (never get_conn:
    _apply_pragmas runs journal_mode=WAL, a write-y pragma). The line changes
    day-to-day only when the schema version changes (byte-stable diffs)."""
    import subprocess
    import shutil
    # PATH-resolved sqlite3 (Linux compat); keep the old absolute fallback.
    ...
    try:
        ...
        try:
            user_version = rconn.execute(
                "PRAGMA user_version;").fetchone()[0]
        finally:
            rconn.close()
    except sqlite3.Error as e:
        print(f"user_version read failed: {e}", file=sys.stderr)
        sys.exit(1)
    r = subprocess.run([sqlite_bin, DB_PATH, '.dump'])
    if r.returncode != 0:
        # Do NOT append the version line to a failed/partial dump.
...
def cli_check(argv):
    """Read-only integrity/version report (DGN-672 contract C5).

    Single locus for "is this lifekit DB healthy and what version is it".
    Consumers: backup-data.sh pre-dump gate, restore-data.sh candidate/post
    verify (via --db), ad-hoc "is the DB ok".

    Connection is a READ-ONLY URI (never get_conn) because:
      (a) a check tool must never mutate its target -- _apply_pragmas executes
          journal_mode=WAL, a write-y pragma (wrong on a possibly corrupt DB);
      (b) DB absence is handled here in the report format (get_conn has its
          own fixed stderr+exit(1) shape).

    Output, one item per line:
        integrity=ok|FAIL
        user_version=N
        table=<name> rows=N     (actual tables, sorted; rows=ERR if unreadable)
        missing=<name>          (in schema.sql's CREATE TABLE set, absent in DB)
    The expected table set is parsed from the instance's database/schema.sql
    (never hardcoded); when schema.sql is absent only the actual set is listed.

    Flags: --db <path> checks a candidate DB with this same logic (used by
    restore-data.sh); default is the live DB_PATH.

    Exit: 0 = integrity ok / 1 = FAIL or DB absent. A version mismatch is a
    REPORT, not an exit reason (backup must keep running on drifted DBs)."""
    p = argparse.ArgumentParser(prog='lifekit.sh check', add_help=False)
    p.add_argument('--db', default=DB_PATH)
    ns, _ = p.parse_known_args(argv)
    db_path = ns.db
    if not os.path.isfile(db_path):
        print(f"integrity=FAIL (db missing: {db_path})")
        sys.exit(1)
    integrity = 'FAIL'
    user_version = None
    tables = []
    ...
    if os.path.isfile(schema_path):
        ...
        for name in sorted(expected - set(tables)):
...
def cli_appt_add(argv):
    ...
    try:
        if not force_new:
            # DGN-579 B4: unified find-first probe (T1 = the DGN-263
            # (local_date, title EXACT) appt key; T2/T3 catch cross-facet
            # duplicates). EXISTS + exit 3 contract preserved verbatim.
            local_date = _local_date_of(start_at, "Asia/Seoul").isoformat()
            _exists_gate(conn, title, local_date, _ff_appt_line)
        try:
        ...
        if eid is None:
            # DGN-579 B2 (locked decision 1): equal-priority exclusive
            # conflict -> structured CONFLICT listing so the agent can
            # PROMPT the user (not a bare error). Nothing was written.
            cee = _cand_eff_end(start_at, end_at, 0)
            _print_conflict_and_exit(conn, start_at, cee)  # E-add-slot (A7)
        _appt_add_ulid = _ulid_for_event_id(conn, eid)
        # DGN-579 B2b: single choke point -- primary line verbatim + the
        # mandatory MOVEMENT_OVERLAP notice on intersecting movement rows.
        _print_write_result(conn, eid, f"{eid}\t{title}")
    finally:
        conn.close()
...
def cli_appt_upd(argv):
    ...
    try:
        ...
        if time_fields:
            ...
            for attempt in range(_UPD_RETRY_MAX):
                ...
                if res == MutationResult.OVERLAP_REJECT:
                    # DGN-579 B2: structured CONFLICT listing (self excluded
                    # -- the moved row must not list itself). E-upd-slot (U7).
                    cee = _cand_eff_end(ns, ne, oe)
                    _print_conflict_and_exit(conn, ns, cee, exclude_id=eid)
        ...
        # DGN-579 B2b: primary line verbatim + mandatory MOVEMENT_OVERLAP
        # notice (time moves land the row on a possibly-new movement span).
        _print_write_result(conn, eid, f"{row[0]}\t{row[1]}\t{row[2] or ''}")
    finally:
...
# ── DGN-579 presence CLI (spec A3/B4). Mirror enqueue: owner D-C3
#    (2026-07-26, presence = GCal all-day banner) REVERSES the E1 producer
#    gate -- every presence CLI verb enqueues post-commit via
#    _outbox_enqueue_best_effort (same DGN-258 pattern as the other verbs).
#    SDK-layer presence verbs still never enqueue (enqueue lives at the
#    CLI/roller producer layer only). ──────────────────────────────────────

def _presence_conflict_line(r):
    """(id, ulid, title, start_at, end_at, recurrence_id, block_class, ver)"""
    return "%s\t%s\t%s\t%s\t%s" % (
        r[0], r[1], r[2],
        _render_local_col(r[3], "timed", "Asia/Seoul"),
        _render_local_col(r[4], "timed", "Asia/Seoul"))


def _presence_parse_span(start_raw, end_raw):
    """Shared span parser: date pair -> all_day instants (inclusive end
    date); datetime pair -> timed canonical instants. Mixed shape / missing
    end -> ValueError (presence end is REQUIRED, grill B6b)."""
    if not end_raw:
        raise ValueError(
            "presence span requires an explicit end (open-ended presence "
            "is forbidden -- extend end later instead)")
    s_shape = _shape_of(start_raw)
    e_shape = _shape_of(end_raw)
    if s_shape == "bad" or e_shape == "bad":
        raise ValueError("bad time format: %s"
                         % (start_raw if s_shape == "bad" else end_raw))
    if s_shape != e_shape:
        raise ValueError("start and end must be the same shape "
                         "(both datetime or both date)")
    if s_shape == "date":
        if end_raw < start_raw:
            raise ValueError("reversed interval refused: end %s < start %s"
                             % (end_raw, start_raw))
        sa, ea = all_day_instants(start_raw, end_raw)
        return "all_day", sa, ea
    return "timed", _to_canonical_utc(start_raw), _to_canonical_utc(end_raw)


def cli_presence_add(argv):
    # presence-add <title> <place> <start> <end> [--new]
    argv, force_new = _pop_flag(argv, '--new')      # B4 find-first bypass
    if len(argv) < 4 or not argv[0] or not argv[1] or not argv[2] or not argv[3]:
        _err("사용법: lifekit.sh presence-add <title> <place> "
             "<start> <end> [--new]")
    title, place_name = argv[0], argv[1]
    try:
        sk, sa, ea = _presence_parse_span(argv[2], argv[3])
    except ValueError as e:
        _err(str(e))
    agent = _facade_agent()
    conn = event_conn()
    _mirror_ulids = []          # D-C3: enqueue after conn closes
    try:
        if not force_new:
            local_date = _local_date_of(sa, "Asia/Seoul").isoformat()
            _exists_gate(conn, title, local_date, _ff_appt_line)
        try:
            pid, pname = place_upsert(conn, place_name)
            res = presence_add(conn, title, pid, sa, ea, schedule_kind=sk,
                               owning_agent=agent, created_by=agent)
        except ValueError as e:
            _err(str(e))
        if res["status"] == "conflict":
            # A3/App.A-8: equal-provenance overlap -> structured PROMPT
            # listing (analog of the CONFLICT contract). Nothing written.
            print(f"PRESENCE_CONFLICT {len(res['rows'])}")
            for r in res["rows"]:
                print(_presence_conflict_line(r))
            sys.exit(CONFLICT_CODE)
        for su in res["superseded"]:
            # cross-provenance supersession: silent + one-line notice.
            print(f"SUPERSEDED {su}")
        # D-C3 mirror enqueue: new banner + superseded banners (their GCal
        # objects need the cancelled tombstone push).
        _mirror_ulids = [res["ulid"]] + list(res["superseded"])
        _print_write_result(conn, res["eid"], f"{res['eid']}\t{title}\t{pname}")
    finally:
        conn.close()
    for _u in _mirror_ulids:
        _outbox_enqueue_best_effort(_u)


def _resolve_presence(conn, token):
    """Accept integer id or ulid; must be a presence row. Returns
    (id, ulid, title, schedule_kind, version) or errors out."""
    row = None
    q = ("SELECT id, ulid, title, schedule_kind, version FROM event "
         "WHERE %s=? AND kind='presence';")
    if str(token).isdigit():
        row = conn.execute(q % "id", (int(token),)).fetchone()
    if row is None:
        row = conn.execute(q % "ulid", (token,)).fetchone()
    if row is None:
        _err(f"presence 없음: {token}")
    return row


def cli_presence_move(argv):
    # presence-move <id|ulid> <start> <end>
    if len(argv) < 3 or not argv[0] or not argv[1] or not argv[2]:
        _err("사용법: lifekit.sh presence-move <id|ulid> <start> <end>")
    try:
        sk, sa, ea = _presence_parse_span(argv[1], argv[2])
    except ValueError as e:
        _err(str(e))
    conn = event_conn()
    _mirror_ulid = None
    try:
        row = _resolve_presence(conn, argv[0])
        eid, _ulid, title, cur_sk, _v = row
        if sk != cur_sk:
            _err("shape mismatch: this presence row is %s "
                 "(give matching start/end shape)" % cur_sk)
        for _attempt in range(_UPD_RETRY_MAX):
            cur = conn.execute(
                "SELECT version FROM event WHERE id=?;", (eid,)).fetchone()
            if cur is None:
                _err(f"presence 없음: {eid}")
            try:
                res = presence_move(conn, eid, cur[0], sa, ea)
            except ValueError as e:
                _err(str(e))
            if res == MutationResult.APPLIED:
                _mirror_ulid = _ulid     # D-C3: banner span changed
                break
            if res == MutationResult.OVERLAP_REJECT:
                # one-presence-at-a-time: moving onto a live presence span
                # is refused loudly with the blocker listing.
                cee = _cand_eff_end(sa, ea, 0)
                rows = _presence_overlapping(conn, sa, cee, exclude_id=eid)
                print(f"PRESENCE_CONFLICT {len(rows)}")
                for r in rows:
                    print(_presence_conflict_line(r))
                sys.exit(CONFLICT_CODE)
            if res == MutationResult.NOT_FOUND:
        ...
        _print_write_result(conn, eid, f"{eid}\t{title}\tmoved")
    finally:
        conn.close()
    if _mirror_ulid:
        _outbox_enqueue_best_effort(_mirror_ulid)


def cli_presence_cancel(argv):
    # presence-cancel <id|ulid>  -- settle a presence 'abandoned' ("안 가게 됐어").
    # appt-cancel/task-* kind-gate presence out, so this is its ONLY tool
    # surface. Reuses the SDK cancel machinery; enqueues post-commit (D-C3:
    # the GCal banner needs its cancelled tombstone push).
    if not argv or not argv[0]:
        _err("사용법: lifekit.sh presence-cancel <id|ulid>")
    conn = event_conn()
    _mirror_ulid = None
    try:
        row = _resolve_presence(conn, argv[0])
        eid, _ulid, title, _sk, ver = row
        outcome = conn.execute(
            "SELECT settled_outcome FROM event WHERE id=?;", (eid,)).fetchone()[0]
        if outcome == 'abandoned':
            _err(f"이미 취소된 presence: {eid}")
        res = cancel(conn, eid, ver, _facade_agent())
        if res != MutationResult.APPLIED:
            _err(f"presence-cancel 실패 ({res}): 이미 종결되었거나 동시 변경")
        _mirror_ulid = _ulid
        print(f"{eid}\t{title}\tcancelled")
    finally:
        conn.close()
    if _mirror_ulid:
        _outbox_enqueue_best_effort(_mirror_ulid)
...
def cli_presence_revive(argv):
    # presence-revive <id|ulid>  -- un-supersede an abandoned presence
    # ("역시 갈래"). Reuses the SDK unsettle lane (guarded: revival that
    # re-overlaps the live presence timeline is refused with the blockers).
    if not argv or not argv[0]:
        _err("사용법: lifekit.sh presence-revive <id|ulid>")
    conn = event_conn()
    _mirror_ulid = None
    try:
        row = _resolve_presence(conn, argv[0])
        eid, ulid, title, _sk, _ver = row
        res, _warn = unsettle(conn, ulid, _facade_agent())
        if res == "refused":
            # guarded refusal: either not abandoned, or the old span is now
            # occupied by another live presence. Surface the blockers.
            srow = conn.execute(
                "SELECT start_at, end_at, settled_outcome FROM event "
                "WHERE id=?;", (eid,)).fetchone()
            if srow[2] != 'abandoned':
                _err(f"presence-revive 실패: abandoned 상태가 아님 ({eid})")
            cee = _cand_eff_end(srow[0], srow[1], 0)
            rows = _presence_overlapping(conn, srow[0], cee, exclude_id=eid)
            print(f"PRESENCE_CONFLICT {len(rows)}")
            for r in rows:
                print(_presence_conflict_line(r))
            sys.exit(CONFLICT_CODE)
        if res != "applied":
            _err(f"presence-revive 실패 ({res})")
        _mirror_ulid = ulid              # D-C3: banner returns to GCal
        print(f"{eid}\t{title}\trevived")
    finally:
        conn.close()
    if _mirror_ulid:
        _outbox_enqueue_best_effort(_mirror_ulid)
...
def _event_day_line(item):
    """Render one event_day partition row for CLI. Columns:
      id<TAB>kind<TAB>title<TAB>start_local<TAB>end_local<TAB>place<TAB>status"""
    (eid, _ulid, kind, title, sl, el, _sk, _tm, _bc, pname, st) = item
    return f"{eid}\t{kind}\t{title}\t{sl}\t{el}\t{pname or ''}\t{st}"


def cli_event_day(argv):
    # event-day <YYYY-MM-DD> [tz]  -- THE day-view query (B1). Prints each
    # partition with a header line so consumers (and humans) can split:
    #   BAND <n> / ANCHORED <n> / DUE <n>, each followed by its rows.
    # floating (untimed backlog) is excluded by contract.
    if not argv or not argv[0] or not _DATE_RE.match(argv[0]):
        _err("사용법: lifekit.sh event-day <YYYY-MM-DD> [tz]")
    tz = argv[1] if len(argv) > 1 and argv[1] else "Asia/Seoul"
    day = event_day(argv[0], tz_name=tz)
    for key, label in (("band", "BAND"), ("anchored", "ANCHORED"),
                       ("due", "DUE")):
        rows = day[key]
        print(f"{label} {len(rows)}")
        for item in rows:
            print(_event_day_line(item))
...
def cli_task_add(argv):
    # task-add <title> [due_date] [note] [--notify <policy>] [--new]
    # (dec-001: event_add kind='task'; --notify = DGN-273 per-event override)
    argv, force_new = _pop_flag(argv, '--new')      # DGN-263
    argv, notify_raw = _pop_opt(argv, '--notify')   # DGN-273
    if not argv or not argv[0]:
    ...
    conn = event_conn()
    if not force_new:
        # DGN-579 B4: unified find-first probe (T2 exact + T3 LIKE keep the
        # DGN-263 partial-match behavior; T1/T2 add cross-facet coverage).
        # EXISTS + exit 3 contract preserved verbatim.
        _exists_gate(conn, title, due, _ff_task_line)
    _task_add_ulid = None
    try:
        if due:
        ...
        # DGN-579 B2b: choke-point reporter (probe is a no-op for the
        # all_day/untimed shapes this verb emits today; timed variants get
        # the MOVEMENT_OVERLAP notice for free).
        _print_write_result(conn, eid, _task_line(_task_fetch(conn, eid)))
    finally:
        conn.close()
    # DGN-258: best-effort mirror outbox enqueue after successful commit.
    if _task_add_ulid:
...
# DGN-564: kind value set; no DB CHECK (source precedent), SDK enforces here.
_SPEND_VALID_KIND = {'expense', 'income'}
...
def cli_spend_add(argv):
    ...
    kind = kind_val or 'expense'
    if kind not in _SPEND_VALID_KIND:
        _err(f"kind 값 오류: {kind} (expense/income)")
    conn = get_conn()
    try:
        if not force_new:
            # DGN-231 reconcile-before-write: match key = (date, amount, kind).
            # DGN-564: same (date, amount) but different kind is NOT a dup.
            matches = conn.execute(
                "SELECT e.id, e.name, e.amount, COALESCE(c.name,'') "
                "FROM spend_entry e "
                "LEFT JOIN spend_category c ON c.id = e.category_id "
                "WHERE e.date=? AND e.amount=? AND e.kind=?;",
                (date, amount, kind)).fetchall()
            if matches:
                print(f"EXISTS {len(matches)}")
        ...
        nid = spend_add(date, amount, name, category=category, method=method,
                        ...
                        source=source, kind=kind, conn=conn)
        row = conn.execute(
            "SELECT e.id, e.name, e.amount, COALESCE(c.name,'') "
            "FROM spend_entry e "
            "LEFT JOIN spend_category c ON c.id = e.category_id "
...
def cli_spend_upd(argv):
    ...
    # fields: date name amount method note is_fixed scope source kind category
    _u = ("사용법: lifekit.sh spend-upd <id> field=value [field=value ...]\n"
          "  field: date name amount method note is_fixed scope source kind "
          "category\n"
          "  지정한 필드만 바뀐다(부분 수정).")
    if len(argv) < 2 or not str(argv[0]).isdigit():
        _err(_u)
...
def cli_spend_week(argv):
    ...
    # DGN-564: income subtotal + net (income - expense) appended; the
    # expense lines above stay untouched.
    net_sign = '+' if w['net'] >= 0 else ''
    print(f"income={w['income_total']:,}원 ({w['income_count']}건)")
    print(f"net={net_sign}{w['net']:,}원")
...
def cli_spend_month(argv):
    ...
    print(f"income={m['income_total']:,}원 ({m['income_count']}건)")
    print(f"net={net_sign}{m['net']:,}원")
...
_DISPATCH = {
    ...
    'dump': cli_dump,
    # DGN-672 C5: read-only integrity/version report (backup gate, restore verify)
    'check': cli_check,
    ...
    # DGN-579: presence verbs (never enqueue to the mirror outbox, E1)
    'presence-add': cli_presence_add,
    'presence-move': cli_presence_move,
    'presence-cancel': cli_presence_cancel,
    'presence-revive': cli_presence_revive,
    # DGN-579 B1: THE day-view query (presence-visible)
    'event-day': cli_event_day,
